var storyContent = {
  inkVersion: 21,
  root: [
    [
      "\n",
      "\n",
      "\n",
      "\n",
      "\n",
      "\n",
      "\n",
      "\n",
      "\n",
      "\n",
      "\n",
      "\n",
      { "->": "scene_1_impact" },
      ["done", { "#f": 5, "#n": "g-0" }],
      null,
    ],
    "done",
    {
      scene_1_impact: [
        "^Darkness. A deep, dreamless void that is suddenly, violently, interrupted.",
        "\n",
        "^Your first sensation is cold. A sharp, biting cold on your cheek, pressed against a metal panel that hums with a low, unfamiliar thrum. Your body is held fast, strapped into a seat that feels more like a cage. The air is stale, tasting of ozone and recycled oxygen.",
        "\n",
        "^A sudden, violent lurch throws you against the restraints, your head snapping forward. A single, angry red light flashes in the cramped space, strobing across the interior of what you now realize is a drop pod. It's a coffin-sized container, all sharp angles and exposed wiring, with no comfort or ceremony to it. This was not built for passengers; it was built for cargo.",
        "\n",
        "^The red light illuminates a small, grimy viewport. Through it, you see a sight that makes your blood run cold: a shattered planet, pieces of its continents hanging silently in the black void of space, wreathed in a sickly green energy. It's a graveyard floating in the cosmos.",
        "\n",
        "^An alarm blares, a deafening, high-pitched shriek that cuts through the hum. The thrumming intensifies, becoming a bone-jarring roar that vibrates through your teeth. The tiny pod shudders as it hits the upper atmosphere, the viewport glowing a brilliant, terrifying white-hot.",
        "\n",
        "^This isn't a rescue. This is a re-entry.",
        "\n",
        "^The impact is absolute. Metal screams, your vision whites out, and then... nothing.",
        "\n",
        "^Silence.",
        "\n",
        { "->": "scene_1a_half_awake" },
        { "#f": 1 },
      ],
      scene_1a_half_awake: [
        [
          "^Pain. A brief window of searing clarity in a sea of darkness. You're slumped forward in the wreckage of the drop pod, the emergency lights flickering erratically. An automated medical panel is open, its contents scattered. You see a blinking auto-injector with a green vial. A Neuro-Stim. Your vision is already starting to swim. You have just enough strength for one action.",
          "\n",
          "ev",
          "str",
          "^Grab the Neuro-Stim.",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          "ev",
          "str",
          "^Try to brace yourself.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 20 },
          {
            "c-0": [
              "\n",
              "^Your fingers close around the cold metal of the injector just as you lose consciousness again. You managed to hold on to it.",
              "\n",
              "ev",
              "str",
              "^AVAILABLE",
              "/str",
              "/ev",
              { "VAR=": "neuro_stim_state", re: true },
              { "->": "scene_2_awakening" },
              { "#f": 5 },
            ],
            "c-1": [
              "\n",
              "^You instinctively try to curl into a defensive position, but the effort is too much. The darkness claims you completely.",
              "\n",
              { "->": "scene_2_awakening" },
              { "#f": 5 },
            ],
          },
        ],
        { "#f": 1 },
      ],
      scene_2_awakening: [
        [
          "^Your head throbs.",
          "\n",
          "^The AI implant in your brain comes online with a soft chime, its voice a calm whisper against the panic in your mind.",
          "\n",
          '^<i>"Implant activated. Vital signs are... suboptimal, but stable. Welcome to the Arena. You have been assigned to Test Site Echo-7."</i>',
          "\n",
          "^You try to remember who you are.",
          "\n",
          "^The name comes first, then the title.",
          "\n",
          "ev",
          "str",
          '^I am Kaelen "Rook" Vance, the Soldier.',
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          "ev",
          "str",
          "^I am Dr. Aris Thorne, the Bio-Hacker.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 20 },
          "ev",
          "str",
          '^I am Lena "Ghost" Petrova, the Infiltrator.',
          "/str",
          "/ev",
          { "*": ".^.c-2", flg: 20 },
          {
            "c-0": [
              "\n",
              "ev",
              "str",
              "^Kaelen",
              "/str",
              "/ev",
              { "VAR=": "character_name", re: true },
              "ev",
              "str",
              "^Xander",
              "/str",
              "/ev",
              { "VAR=": "rival_name", re: true },
              "ev",
              7,
              "/ev",
              { "VAR=": "strength", re: true },
              "ev",
              3,
              "/ev",
              { "VAR=": "intelligence", re: true },
              "ev",
              4,
              "/ev",
              { "VAR=": "agility", re: true },
              "ev",
              5,
              "/ev",
              { "VAR=": "perception", re: true },
              "ev",
              25,
              "/ev",
              { "VAR=": "max_hp", re: true },
              "ev",
              25,
              "/ev",
              { "VAR=": "hp", re: true },
              "ev",
              6,
              "/ev",
              { "VAR=": "atk", re: true },
              "ev",
              3,
              "/ev",
              { "VAR=": "def", re: true },
              "ev",
              { list: { "AllSkills.Survivalist": 1 } },
              "/ev",
              { "VAR=": "player_skills", re: true },
              "ev",
              { "f()": "update_combat_stats" },
              "pop",
              "/ev",
              "\n",
              "ev",
              { "VAR?": "max_hp" },
              "/ev",
              { "VAR=": "hp", re: true },
              { "->": "scene_3_the_first_room" },
              { "#f": 5 },
            ],
            "c-1": [
              "\n",
              "ev",
              "str",
              "^Aris",
              "/str",
              "/ev",
              { "VAR=": "character_name", re: true },
              "ev",
              "str",
              "^Jinx",
              "/str",
              "/ev",
              { "VAR=": "rival_name", re: true },
              "ev",
              3,
              "/ev",
              { "VAR=": "strength", re: true },
              "ev",
              8,
              "/ev",
              { "VAR=": "intelligence", re: true },
              "ev",
              4,
              "/ev",
              { "VAR=": "agility", re: true },
              "ev",
              6,
              "/ev",
              { "VAR=": "perception", re: true },
              "ev",
              18,
              "/ev",
              { "VAR=": "max_hp", re: true },
              "ev",
              18,
              "/ev",
              { "VAR=": "hp", re: true },
              "ev",
              3,
              "/ev",
              { "VAR=": "atk", re: true },
              "ev",
              5,
              "/ev",
              { "VAR=": "def", re: true },
              "ev",
              { list: { "AllSkills.BioScan": 2 } },
              "/ev",
              { "VAR=": "player_skills", re: true },
              "ev",
              { "f()": "update_combat_stats" },
              "pop",
              "/ev",
              "\n",
              "ev",
              { "VAR?": "max_hp" },
              "/ev",
              { "VAR=": "hp", re: true },
              { "->": "scene_3_the_first_room" },
              { "#f": 5 },
            ],
            "c-2": [
              "\n",
              "ev",
              "str",
              "^Lena",
              "/str",
              "/ev",
              { "VAR=": "character_name", re: true },
              "ev",
              "str",
              "^Isha",
              "/str",
              "/ev",
              { "VAR=": "rival_name", re: true },
              "ev",
              4,
              "/ev",
              { "VAR=": "strength", re: true },
              "ev",
              5,
              "/ev",
              { "VAR=": "intelligence", re: true },
              "ev",
              7,
              "/ev",
              { "VAR=": "agility", re: true },
              "ev",
              8,
              "/ev",
              { "VAR=": "perception", re: true },
              "ev",
              20,
              "/ev",
              { "VAR=": "max_hp", re: true },
              "ev",
              20,
              "/ev",
              { "VAR=": "hp", re: true },
              "ev",
              5,
              "/ev",
              { "VAR=": "atk", re: true },
              "ev",
              2,
              "/ev",
              { "VAR=": "def", re: true },
              "ev",
              { list: { "AllSkills.DiscerningEye": 3 } },
              "/ev",
              { "VAR=": "player_skills", re: true },
              "ev",
              { "f()": "update_combat_stats" },
              "pop",
              "/ev",
              "\n",
              "ev",
              { "VAR?": "max_hp" },
              "/ev",
              { "VAR=": "hp", re: true },
              { "->": "scene_3_the_first_room" },
              { "#f": 5 },
            ],
          },
        ],
        { "#f": 1 },
      ],
      scene_3_the_first_room: [
        "^The door of your drop pod hisses open, dumping you onto a floor of cracked concrete slick with rainwater and alien moss. You pick yourself up, turning to look back at the source of your rude awakening. The pod is a wreck, its hull scorched and dented from the impact. Wires spark feebly from a shattered console, and the air smells of burnt insulation.",
        "\n",
        "^You look down at yourself, taking stock.",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Kaelen",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^You're wearing a set of worn, grey military fatigues, torn at the knee and stained with grime. It's not a uniform you recognize, but it feels familiar.",
              "\n",
              { "->": ".^.^.^.12" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Aris",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^Your clothes are simple—a plain shirt and trousers—but they're covered by a tattered, knee-length lab coat, singed at the cuffs and splattered with something you hope is just mud.",
              "\n",
              { "->": ".^.^.^.22" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Lena",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^You're clad in a form-fitting, dark grey utility suit. It's designed for stealth and movement, though a fresh tear along the arm reminds you that it's not indestructible.",
              "\n",
              { "->": ".^.^.^.32" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        '^<i>AI: "Location confirmed. Maintenance Bay. Zone designation: Ruined City-Isle."</i>',
        "\n",
        "^You're in a cavernous, ruined maintenance bay. The air is thick with the smell of ozone and decay. A single emergency light casts long, dancing shadows across shattered computer terminals and the skeletal remains of what might have been a maintenance worker. Across the room, you see a patch of faintly glowing moss clinging to a damp wall. Next to it is a heavy, rusted metal locker. The only way out is a collapsed doorway to the north, choked with rubble but passable.",
        "\n",
        { "->": "scene_3_choices" },
        { "#f": 1 },
      ],
      scene_4_the_first_obstacle: [
        [
          "^To reach the plaza, you must navigate a short but unstable transit tunnel. The floor is a mess of twisted metal and rubble, and the ceiling groans under the strain.",
          "\n",
          "ev",
          "str",
          "^BRUTE FORCE: Shove a large piece of debris to create a stable, direct path.",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          "ev",
          "str",
          "^ANALYZE: Scan the structure for the most stable route.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 20 },
          "ev",
          "str",
          "^NAVIGATE: Nimbly climb through the quickest, most dangerous path.",
          "/str",
          "/ev",
          { "*": ".^.c-2", flg: 20 },
          {
            "c-0": [
              "\n",
              "ev",
              { "VAR?": "character_name" },
              "str",
              "^Kaelen",
              "/str",
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^You use your natural talents to pass through the tunnel unharmed.",
                    "\n",
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "^Your attempt is clumsy. A piece of rubble gives way, and the strain affects your core skills.",
                    "\n",
                    "ev",
                    { "VAR?": "character_name" },
                    "str",
                    "^Aris",
                    "/str",
                    "==",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "^You receive a -1 debuff to Intelligence.",
                          "\n",
                          "ev",
                          { "VAR?": "intelligence" },
                          1,
                          "-",
                          { "VAR=": "intelligence", re: true },
                          "/ev",
                          "ev",
                          "str",
                          "^Intelligence",
                          "/str",
                          "/ev",
                          { "VAR=": "scene_4_debuff_stat", re: true },
                          "ev",
                          { "f()": "update_combat_stats" },
                          "pop",
                          "/ev",
                          "\n",
                          { "->": ".^.^.^.12" },
                          null,
                        ],
                      },
                    ],
                    [
                      { "->": ".^.b" },
                      {
                        b: [
                          "\n",
                          "^You receive a -1 debuff to Agility.",
                          "\n",
                          "ev",
                          { "VAR?": "agility" },
                          1,
                          "-",
                          { "VAR=": "agility", re: true },
                          "/ev",
                          "ev",
                          "str",
                          "^Agility",
                          "/str",
                          "/ev",
                          { "VAR=": "scene_4_debuff_stat", re: true },
                          "ev",
                          { "f()": "update_combat_stats" },
                          "pop",
                          "/ev",
                          "\n",
                          { "->": ".^.^.^.12" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": "scene_5_crossroads" },
              { "#f": 5 },
            ],
            "c-1": [
              "\n",
              "ev",
              { "VAR?": "character_name" },
              "str",
              "^Aris",
              "/str",
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^You use your natural talents to pass through the tunnel unharmed.",
                    "\n",
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "^Your attempt is ill-suited to your skills, and the mental effort affects your core abilities.",
                    "\n",
                    "ev",
                    { "VAR?": "character_name" },
                    "str",
                    "^Kaelen",
                    "/str",
                    "==",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "^You receive a -1 debuff to Strength.",
                          "\n",
                          "ev",
                          { "VAR?": "strength" },
                          1,
                          "-",
                          { "VAR=": "strength", re: true },
                          "/ev",
                          "ev",
                          "str",
                          "^Strength",
                          "/str",
                          "/ev",
                          { "VAR=": "scene_4_debuff_stat", re: true },
                          "ev",
                          { "f()": "update_combat_stats" },
                          "pop",
                          "/ev",
                          "\n",
                          { "->": ".^.^.^.12" },
                          null,
                        ],
                      },
                    ],
                    [
                      { "->": ".^.b" },
                      {
                        b: [
                          "\n",
                          "^You receive a -1 debuff to Agility.",
                          "\n",
                          "ev",
                          { "VAR?": "agility" },
                          1,
                          "-",
                          { "VAR=": "agility", re: true },
                          "/ev",
                          "ev",
                          "str",
                          "^Agility",
                          "/str",
                          "/ev",
                          { "VAR=": "scene_4_debuff_stat", re: true },
                          "ev",
                          { "f()": "update_combat_stats" },
                          "pop",
                          "/ev",
                          "\n",
                          { "->": ".^.^.^.12" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": "scene_5_crossroads" },
              { "#f": 5 },
            ],
            "c-2": [
              "\n",
              "ev",
              { "VAR?": "character_name" },
              "str",
              "^Lena",
              "/str",
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^You use your natural talents to pass through the tunnel unharmed.",
                    "\n",
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "^Your attempt is clumsy, and the physical toll weakens your primary strengths.",
                    "\n",
                    "ev",
                    { "VAR?": "character_name" },
                    "str",
                    "^Kaelen",
                    "/str",
                    "==",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "^You receive a -1 debuff to Strength.",
                          "\n",
                          "ev",
                          { "VAR?": "strength" },
                          1,
                          "-",
                          { "VAR=": "strength", re: true },
                          "/ev",
                          "ev",
                          "str",
                          "^Strength",
                          "/str",
                          "/ev",
                          { "VAR=": "scene_4_debuff_stat", re: true },
                          "ev",
                          { "f()": "update_combat_stats" },
                          "pop",
                          "/ev",
                          "\n",
                          { "->": ".^.^.^.12" },
                          null,
                        ],
                      },
                    ],
                    [
                      { "->": ".^.b" },
                      {
                        b: [
                          "\n",
                          "^You receive a -1 debuff to Intelligence.",
                          "\n",
                          "ev",
                          { "VAR?": "intelligence" },
                          1,
                          "-",
                          { "VAR=": "intelligence", re: true },
                          "/ev",
                          "ev",
                          "str",
                          "^Intelligence",
                          "/str",
                          "/ev",
                          { "VAR=": "scene_4_debuff_stat", re: true },
                          "ev",
                          { "f()": "update_combat_stats" },
                          "pop",
                          "/ev",
                          "\n",
                          { "->": ".^.^.^.12" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": "scene_5_crossroads" },
              { "#f": 5 },
            ],
          },
        ],
        { "#f": 1 },
      ],
      scene_5_crossroads: [
        [
          "^You emerge into a larger area with two clear paths: one leading up towards the rooftops, the other down into a subway entrance.",
          "\n",
          "^Suddenly, a booming, disembodied voice echoes through the plaza.",
          "\n",
          "^<br><br>",
          "\n",
          '^<i>PROCTOR: "Major Event initiated: The Drop. A high-tier supply cache will arrive at the central plaza in 90 seconds. Competition is encouraged."</i>',
          "\n",
          "^<br><br>",
          "\n",
          "ev",
          "str",
          "^Go for the Supply Drop.",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          "ev",
          "str",
          "^Ignore the Drop and find a safer path.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 20 },
          "ev",
          "str",
          "^Check Status.",
          "/str",
          "/ev",
          { "*": ".^.c-2", flg: 4 },
          "ev",
          "str",
          "^Query the AI.",
          "/str",
          "/ev",
          { "*": ".^.c-3", flg: 20 },
          {
            "c-0": ["^ ", { "->": "scene_5a_the_cache" }, "\n", { "#f": 5 }],
            "c-1": ["^ ", { "->": "scene_6_first_test" }, "\n", { "#f": 5 }],
            "c-2": [
              "\n",
              "ev",
              { "^->": "scene_5_crossroads" },
              "/ev",
              { "->": "check_status" },
              { "#f": 5 },
            ],
            "c-3": ["\n", { "->": "scene_5_ai_query" }, { "#f": 5 }],
          },
        ],
        { "#f": 3 },
      ],
      scene_6_first_test: [
        [
          "ev",
          { "CNT?": "scene_5a_the_cache" },
          0,
          ">",
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "^You move on from the plaza, the encounter with your rival leaving a sour taste in your mouth.",
                "\n",
                { "->": ".^.^.^.7" },
                null,
              ],
            },
          ],
          [
            { "->": ".^.b" },
            {
              b: [
                "\n",
                "^You decide to avoid the confrontation at the plaza, heading for a less obvious route.",
                "\n",
                { "->": ".^.^.^.7" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "^You have a choice of paths ahead, one leading up, the other down.",
          "\n",
          "ev",
          "str",
          "^Take the high ground. Head for the rooftops.",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          "ev",
          "str",
          "^Descend into the darkness. Take the subway.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 20 },
          {
            "c-0": ["\n", { "->": "scene_6a_rooftops" }, { "#f": 5 }],
            "c-1": ["\n", { "->": "scene_6b_subway" }, { "#f": 5 }],
          },
        ],
        { "#f": 1 },
      ],
      scene_7_the_fragment: [
        "^You now possess your first Data Fragment. It's a small, crystalline object that hums with a faint energy and feels strangely warm to the touch. As you inspect it, the AI's voice chimes in your head.",
        "\n",
        { "->": ".^.fragment_dialogue" },
        {
          fragment_dialogue: [
            [
              "ev",
              { "VAR?": "character_name" },
              "str",
              "^Kaelen",
              "/str",
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    '^<i>AI: "First Data Fragment acquired. Two remaining."</i>',
                    "\n",
                    "ev",
                    "str",
                    '^"Cut the chatter. Where\'s the next one?"',
                    "/str",
                    "/ev",
                    { "*": ".^.c-0", flg: 20 },
                    "ev",
                    "str",
                    '^"What is this thing, really?"',
                    "/str",
                    "/ev",
                    { "*": ".^.c-1", flg: 20 },
                    { "->": ".^.^.^.8" },
                    {
                      "c-0": [
                        "\n",
                        '^<i>AI: "Triangulating... Energy signatures are inconsistent. Further data is required to pinpoint the next fragment\'s exact location."</i>',
                        "\n",
                        { "->": ".^.^.^.^.^" },
                        { "#f": 5 },
                      ],
                      "c-1": [
                        "\n",
                        '^<i>AI: "A data key. It contains positioning algorithms. Useless individually, but invaluable when combined."</i>',
                        "\n",
                        { "->": ".^.^.^.^.^" },
                        { "#f": 5 },
                      ],
                    },
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              { "VAR?": "character_name" },
              "str",
              "^Aris",
              "/str",
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    '^<i>AI: "First Data Fragment acquired. Analysis suggests it is a complex bio-organic data storage medium."</i>',
                    "\n",
                    "ev",
                    "str",
                    '^"Fascinating. Is the crystalline structure naturally occurring?"',
                    "/str",
                    "/ev",
                    { "*": ".^.c-0", flg: 20 },
                    "ev",
                    "str",
                    "^\"So these 'fragments' are the keys to progression?\"",
                    "/str",
                    "/ev",
                    { "*": ".^.c-1", flg: 20 },
                    { "->": ".^.^.^.18" },
                    {
                      "c-0": [
                        "\n",
                        '^<i>AI: "Negative. It is a manufactured lattice, grown around a quantum processing core. The technology is far beyond your civilization\'s understanding."</i>',
                        "\n",
                        { "->": ".^.^.^.^.^" },
                        { "#f": 5 },
                      ],
                      "c-1": [
                        "\n",
                        '^<i>AI: "A logical deduction. Mastery of the environment is the metric for success. Acquiring all fragments is the primary indicator of mastery for this phase."</i>',
                        "\n",
                        { "->": ".^.^.^.^.^" },
                        { "#f": 5 },
                      ],
                    },
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              { "VAR?": "character_name" },
              "str",
              "^Lena",
              "/str",
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    '^<i>AI: "First Data Fragment acquired. Be advised, its energy signature may be detectable by other subjects."</i>',
                    "\n",
                    "ev",
                    "str",
                    '^"Can I mask the signature?"',
                    "/str",
                    "/ev",
                    { "*": ".^.c-0", flg: 20 },
                    "ev",
                    "str",
                    '^"What aren\'t you telling me about these things?"',
                    "/str",
                    "/ev",
                    { "*": ".^.c-1", flg: 20 },
                    { "->": ".^.^.^.28" },
                    {
                      "c-0": [
                        "\n",
                        '^<i>AI: "Possible, with the correct dampening materials. However, such materials are not currently in your possession."</i>',
                        "\n",
                        { "->": ".^.^.^.^.^" },
                        { "#f": 5 },
                      ],
                      "c-1": [
                        "\n",
                        '^<i>AI: "My function is to provide necessary data for the experiment. The purpose of the experiment itself is data available only to the Proctor."</i>',
                        "\n",
                        { "->": ".^.^.^.^.^" },
                        { "#f": 5 },
                      ],
                    },
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              "str",
              "^Got it. Let's move.",
              "/str",
              "/ev",
              { "*": ".^.c-0", flg: 20 },
              {
                "c-0": [
                  "\n",
                  "^The AI gives you a general direction for the next fragment's energy signature. It's time to move on.",
                  "\n",
                  { "->": "scene_7_scavenge" },
                  { "#f": 5 },
                ],
              },
            ],
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      scene_8_the_tower: [
        [
          "ev",
          { "VAR?": "resolve" },
          50,
          "<",
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "^As you approach the tower, a wave of doubt washes over you. The climb seems impossible, the other contestants faster and stronger. Your wavering confidence affects your physical readiness.",
                "\n",
                '^<i>AI: "Warning: Subject\'s resolve is below optimal parameters. Combat and traversal efficiency may be compromised."</i>',
                "\n",
                "^Your Strength and Agility are lowered by the psychological strain.",
                "\n",
                "ev",
                { "VAR?": "strength" },
                1,
                "-",
                { "VAR=": "strength", re: true },
                "/ev",
                "ev",
                { "VAR?": "agility" },
                1,
                "-",
                { "VAR=": "agility", re: true },
                "/ev",
                { "->": ".^.^.^.6" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "^Your path forward leads you into a vast, open plaza, a graveyard of rusted vehicles and shattered ferrocrete. At its center, a colossal communications spire claws its way towards the sky, its metallic latticework draped in thick, alien vines. Much of its structure has collapsed, but the main tower still stands, impossibly tall. A single red emergency light pulses at its peak, a lonely star in the decaying cityscape.",
          "\n",
          '^<i>AI: "Structure identified: Primary Communications Relay Spire-3. High-value Data Fragment detected at the apex. Be advised: multiple other subjects are converging on this location. A competitive trial is imminent."</i>',
          "\n",
          "^You see them now, scattered around the base of the tower. A handful of other contestants, their faces grim, checking their gear and eyeing each other with suspicion. No alliances here. Just a race to the top. The main entrance is a crater of twisted metal, but a service ladder is still intact, leading up the first few stories.",
          "\n",
          "ev",
          "str",
          "^Begin the ascent.",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          "ev",
          "str",
          "^Query the AI.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 4 },
          "ev",
          "str",
          "^Check Status.",
          "/str",
          "/ev",
          { "*": ".^.c-2", flg: 4 },
          {
            "c-0": ["\n", { "->": ".^.^.^.tower_floor_1" }, { "#f": 5 }],
            "c-1": ["\n", { "->": "scene_8_ai_query" }, { "#f": 5 }],
            "c-2": [
              "\n",
              "ev",
              { "^->": "scene_8_the_tower" },
              "/ev",
              { "->": "check_status" },
              { "#f": 5 },
            ],
          },
        ],
        {
          tower_floor_1: [
            [
              '^You haul yourself onto the first-floor landing, a wide platform of rusted metal grating. A lithe contestant, the "Scrambler," is already there. They see you, grin, and make a dash for the stairs to the next level.',
              "\n",
              '^<i>PROCTOR: "First subject to ascend demonstrates superior traversal capability."</i>',
              "\n",
              "ev",
              "str",
              "^Beat them to the stairs - Agility Check",
              "/str",
              "/ev",
              { "*": ".^.c-0", flg: 20 },
              {
                "c-0": [
                  "\n",
                  "ev",
                  1,
                  6,
                  "rnd",
                  "/ev",
                  { "temp=": "roll" },
                  "\n",
                  "ev",
                  { "VAR?": "agility" },
                  { "VAR?": "roll" },
                  "+",
                  "/ev",
                  { "temp=": "total_skill" },
                  "ev",
                  { "VAR?": "total_skill" },
                  12,
                  ">=",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        "^You're faster. You easily bound over the obstacles and cut them off, claiming the path upwards. They scowl and retreat to find another way.",
                        "\n",
                        { "->": ".^.^.^.21" },
                        null,
                      ],
                    },
                  ],
                  [
                    { "->": ".^.b" },
                    {
                      b: [
                        "\n",
                        "^They're too quick, vaulting over a railing while you're still navigating the debris. They're gone before you can catch them. The path is clear, but your pride is stung.",
                        "\n",
                        "ev",
                        { "VAR?": "resolve" },
                        3,
                        "-",
                        { "VAR=": "resolve", re: true },
                        "/ev",
                        { "->": ".^.^.^.21" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  "ev",
                  { "^->": "tower_floor_2" },
                  "/ev",
                  { "->": "tower_buffer_room" },
                  { "#f": 5 },
                ],
              },
            ],
            { "#f": 1 },
          ],
          "#f": 3,
        },
      ],
      scene_9_the_bargain: [
        "ev",
        { "VAR?": "data_fragments" },
        2,
        "<",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^Having failed to secure the fragment at the tower, your AI directs you to another faint energy signature. It leads you to a dilapidated hab-block, lit by a single emergency light.",
              "\n",
              { "->": "find_jed_for_fragment" },
              { "->": ".^.^.^.7" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "^As you search for a path forward from the tower, you come across a small, fortified room in a hab-block. It seems recently occupied.",
              "\n",
              { "->": "meet_jed_friendly" },
              { "->": ".^.^.^.7" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      scene_10_the_lair: [
        "^With your preparations complete, the AI directs you into a massive, cavernous subway nexus. Several tunnels converge here, and a derelict subway train rests in the center. Atop the main platform, a shimmering, inactive Archive Gate stands, but it is not unguarded.",
        "\n",
        "^Nine other contestants are gathered here, a tense and hostile silence hanging in the air. You see your rival ",
        "ev",
        { "VAR?": "rival_name" },
        "out",
        "/ev",
        "^ among them, and you also spot Jed, who gives you a nod or a glare depending on your history.",
        "\n",
        "^The Proctor's voice echoes through the cavern, cold and final.",
        "\n",
        '^<i>PROCTOR: "The final test for this zone has begun. The Archive Gate is guarded by the zone\'s Apex Predator: the Alpha Skulker Matriarch. The five subjects who contribute most to its defeat will be awarded the final Data Fragment. All others will be deemed... substandard. Good luck."</i>',
        "\n",
        "^As if on cue, a wave of smaller Skulkers floods from the tunnels, a living tide of claws and teeth between you and the main platform.",
        "\n",
        { "->": "lair_horde_battle" },
        { "#f": 1 },
      ],
      update_combat_stats: [
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Kaelen",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              15,
              { "VAR?": "strength" },
              2,
              "*",
              "+",
              "/ev",
              { "VAR=": "max_hp", re: true },
              "ev",
              2,
              { "VAR?": "strength" },
              "+",
              "/ev",
              { "VAR=": "atk", re: true },
              "ev",
              { "VAR?": "agility" },
              "/ev",
              { "VAR=": "def", re: true },
              { "->": ".^.^.^.8" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Aris",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              12,
              { "VAR?": "strength" },
              "+",
              "/ev",
              { "VAR=": "max_hp", re: true },
              "ev",
              1,
              { "VAR?": "intelligence" },
              "+",
              "/ev",
              { "VAR=": "atk", re: true },
              "ev",
              { "VAR?": "agility" },
              1,
              "+",
              "/ev",
              { "VAR=": "def", re: true },
              { "->": ".^.^.^.18" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Lena",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              14,
              { "VAR?": "strength" },
              "+",
              "/ev",
              { "VAR=": "max_hp", re: true },
              "ev",
              2,
              { "VAR?": "agility" },
              "+",
              "/ev",
              { "VAR=": "atk", re: true },
              "ev",
              { "VAR?": "perception" },
              2,
              "/",
              "INT",
              "/ev",
              { "VAR=": "def", re: true },
              "\n",
              { "->": ".^.^.^.28" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "emitter_equipped" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "studied_emitter" },
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    { "VAR?": "character_name" },
                    "str",
                    "^Aris",
                    "/str",
                    "==",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "ev",
                          { "VAR?": "atk" },
                          4,
                          "+",
                          { "VAR=": "atk", re: true },
                          "/ev",
                          { "->": ".^.^.^.10" },
                          null,
                        ],
                      },
                    ],
                    [
                      { "->": ".^.b" },
                      {
                        b: [
                          "\n",
                          "ev",
                          { "VAR?": "atk" },
                          3,
                          "+",
                          { "VAR=": "atk", re: true },
                          "/ev",
                          { "->": ".^.^.^.10" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    { "->": ".^.^.^.6" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "ev",
                    { "VAR?": "atk" },
                    2,
                    "+",
                    { "VAR=": "atk", re: true },
                    "/ev",
                    { "->": ".^.^.^.6" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.34" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "club_equipped" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "atk" },
              2,
              "+",
              { "VAR=": "atk", re: true },
              "/ev",
              "ev",
              { "VAR?": "club_is_upgraded" },
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    { "VAR?": "atk" },
                    1,
                    "+",
                    { "VAR=": "atk", re: true },
                    "/ev",
                    { "->": ".^.^.^.11" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.40" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "bow_equipped" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "atk" },
              2,
              "+",
              { "VAR=": "atk", re: true },
              "/ev",
              { "->": ".^.^.^.46" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "agility" },
        { "VAR?": "perception" },
        2,
        "/",
        "+",
        "/ev",
        { "VAR=": "dodge_chance", re: true },
        "ev",
        true,
        "/ev",
        "~ret",
        { "#f": 1 },
      ],
      use_emitter_charge: [
        "ev",
        { "VAR?": "has_kinetic_emitter" },
        { "VAR?": "emitter_charges" },
        0,
        ">",
        "&&",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "emitter_charges" },
              1,
              "-",
              { "VAR=": "emitter_charges", re: true },
              "/ev",
              "^The Kinetic Field Emitter discharges with a powerful hum.",
              "\n",
              "ev",
              { "VAR?": "emitter_charges" },
              0,
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^A final surge of power leaves the device inert, its internal mechanisms fused.",
                    "\n",
                    "^It's broken for good.",
                    "\n",
                    { "->": ".^.^.^.15" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              true,
              "/ev",
              "~ret",
              { "->": ".^.^.^.9" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          { b: ["\n", "ev", false, "/ev", "~ret", { "->": ".^.^.^.9" }, null] },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      display_status: [
        [
          [
            "^Character Status --",
            "\n",
            "^Name: ",
            "ev",
            { "VAR?": "character_name" },
            "out",
            "/ev",
            "\n",
            "^HP: ",
            "ev",
            { "VAR?": "hp" },
            "out",
            "/ev",
            "^ / ",
            "ev",
            { "VAR?": "max_hp" },
            "out",
            "/ev",
            "\n",
            "^Attack: ",
            "ev",
            { "VAR?": "atk" },
            "out",
            "/ev",
            "\n",
            "^Defense: ",
            "ev",
            { "VAR?": "def" },
            "out",
            "/ev",
            "\n",
            [
              "^Skills --",
              "\n",
              "ev",
              { "VAR?": "player_skills" },
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    { "VAR?": "player_skills" },
                    "out",
                    "/ev",
                    "\n",
                    { "->": ".^.^.^.7" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "^No skills learned.",
                    "\n",
                    { "->": ".^.^.^.7" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              [
                "^Attributes --",
                "\n",
                "^Strength: ",
                "ev",
                { "VAR?": "strength" },
                "out",
                "/ev",
                "\n",
                "^Intelligence: ",
                "ev",
                { "VAR?": "intelligence" },
                "out",
                "/ev",
                "\n",
                "^Agility: ",
                "ev",
                { "VAR?": "agility" },
                "out",
                "/ev",
                "\n",
                "^Perception: ",
                "ev",
                { "VAR?": "perception" },
                "out",
                "/ev",
                "\n",
                "^Resolve: ",
                "ev",
                { "VAR?": "resolve" },
                "out",
                "/ev",
                "\n",
                [
                  "^Condition --",
                  "\n",
                  "ev",
                  { "VAR?": "is_injured" },
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    { b: ["\n", "^INJURED", "\n", { "->": ".^.^.^.6" }, null] },
                  ],
                  "nop",
                  "\n",
                  "ev",
                  { "VAR?": "is_fatigued" },
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: ["\n", "^FATIGUED", "\n", { "->": ".^.^.^.12" }, null],
                    },
                  ],
                  "nop",
                  "\n",
                  "ev",
                  { "VAR?": "is_injured" },
                  "!",
                  { "VAR?": "is_fatigued" },
                  "!",
                  "&&",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: ["\n", "^HEALTHY", "\n", { "->": ".^.^.^.22" }, null],
                    },
                  ],
                  "nop",
                  "\n",
                  [
                    "^Inventory --",
                    "\n",
                    "ev",
                    { "VAR?": "power_cell_stack" },
                    0,
                    "==",
                    { "VAR?": "glimmer_moss_stack" },
                    0,
                    "==",
                    "&&",
                    { "VAR?": "has_kinetic_emitter" },
                    "!",
                    "&&",
                    { "VAR?": "neuro_stim_state" },
                    "str",
                    "^AVAILABLE",
                    "/str",
                    "!=",
                    "&&",
                    { "VAR?": "has_reinforced_club" },
                    "!",
                    "&&",
                    { "VAR?": "has_recurve_bow" },
                    "!",
                    "&&",
                    { "VAR?": "has_emp_grenade" },
                    "!",
                    "&&",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "^Your pockets are empty.",
                          "\n",
                          { "->": ".^.^.^.30" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "power_cell_stack" },
                    0,
                    ">",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Degraded Power Cell (x",
                          "ev",
                          { "VAR?": "power_cell_stack" },
                          "out",
                          "/ev",
                          "^) ",
                          { "->": ".^.^.^.38" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "glimmer_moss_stack" },
                    0,
                    ">",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Glimmer Moss Sample (x",
                          "ev",
                          { "VAR?": "glimmer_moss_stack" },
                          "out",
                          "/ev",
                          "^) ",
                          { "->": ".^.^.^.46" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_kinetic_emitter" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Kinetic Field Emitter (",
                          "ev",
                          { "VAR?": "emitter_charges" },
                          "out",
                          "/ev",
                          "^ charges) ",
                          { "->": ".^.^.^.52" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "neuro_stim_state" },
                    "str",
                    "^AVAILABLE",
                    "/str",
                    "==",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Neuro-Stim - Single Use ",
                          { "->": ".^.^.^.62" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_reinforced_club" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "^Reinforced Club ",
                          "ev",
                          { "VAR?": "club_is_upgraded" },
                          "/ev",
                          [
                            { "->": ".^.b", c: true },
                            {
                              b: [
                                "^(Upgraded, ",
                                "ev",
                                { "VAR?": "club_power_slots" },
                                "out",
                                "/ev",
                                "^ slots left)",
                                { "->": ".^.^.^.6" },
                                null,
                              ],
                            },
                          ],
                          "nop",
                          "\n",
                          { "->": ".^.^.^.68" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_recurve_bow" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      { b: ["^ - Recurve Bow ", { "->": ".^.^.^.74" }, null] },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_emp_grenade" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - EMP Grenade (Single Use) ",
                          { "->": ".^.^.^.80" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_metal_pipe" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Sturdy Metal Pipe ",
                          { "->": ".^.^.^.86" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_thick_wiring" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      { b: ["^ - Thick Wiring ", { "->": ".^.^.^.92" }, null] },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_flexible_polymer" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Flexible Polymer ",
                          { "->": ".^.^.^.98" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_tensile_cable" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - High-Tensile Cable ",
                          { "->": ".^.^.^.104" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_copper_wiring" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: ["^ - Copper Wiring ", { "->": ".^.^.^.110" }, null],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "skulker_venom_gland_stack" },
                    0,
                    ">",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Skulker Venom Gland (x",
                          "ev",
                          { "VAR?": "skulker_venom_gland_stack" },
                          "out",
                          "/ev",
                          "^) ",
                          { "->": ".^.^.^.118" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_moss_poison_vial" },
                    0,
                    ">",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "^Moss Poison Vial (x",
                          "ev",
                          { "VAR?": "has_moss_poison_vial" },
                          "out",
                          "/ev",
                          "^)",
                          "\n",
                          { "->": ".^.^.^.126" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "poison_bomb_stack" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Poison Gas Bomb (x",
                          "ev",
                          { "VAR?": "poison_bomb_stack" },
                          "out",
                          "/ev",
                          "^) ",
                          { "->": ".^.^.^.132" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_club_upgrade_kit" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Club Upgrade Kit ",
                          { "->": ".^.^.^.138" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "knows_calming_poultice_recipe" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Recipe: Calming Poultice ",
                          { "->": ".^.^.^.144" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "silent_arrow_count" },
                    0,
                    ">",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Silent Arrows (x",
                          "ev",
                          { "VAR?": "silent_arrow_count" },
                          "out",
                          "/ev",
                          "^) ",
                          { "->": ".^.^.^.152" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "scrap_arrow_count" },
                    0,
                    ">",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Scrap Arrows (x",
                          "ev",
                          { "VAR?": "scrap_arrow_count" },
                          "out",
                          "/ev",
                          "^) ",
                          { "->": ".^.^.^.160" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "shock_arrow_count" },
                    0,
                    ">",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Shock Arrows (x",
                          "ev",
                          { "VAR?": "shock_arrow_count" },
                          "out",
                          "/ev",
                          "^) ",
                          { "->": ".^.^.^.168" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_titan_beetle_carapace" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Titan-Beetle Carapace ",
                          { "->": ".^.^.^.174" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_adrenaline_shot" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Adrenaline Shot (Single Use) ",
                          { "->": ".^.^.^.180" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_regen_serum" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ - Regen Serum (Single Use) ",
                          { "->": ".^.^.^.186" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "has_magnetic_coil" },
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: ["^ - Magnetic Coil ", { "->": ".^.^.^.192" }, null],
                      },
                    ],
                    "nop",
                    "\n",
                    [
                      "^Logs & Intel --",
                      "\n",
                      "ev",
                      { "VAR?": "found_first_log" },
                      "!",
                      "/ev",
                      [
                        { "->": ".^.b", c: true },
                        {
                          b: [
                            "\n",
                            "^No intel recovered.",
                            "\n",
                            { "->": ".^.^.^.7" },
                            null,
                          ],
                        },
                      ],
                      "nop",
                      "\n",
                      "ev",
                      { "VAR?": "found_first_log" },
                      { "VAR?": "know_skulker_weakness" },
                      "!",
                      "&&",
                      "/ev",
                      [
                        { "->": ".^.b", c: true },
                        {
                          b: [
                            "\n",
                            "^Archivist Log #77-B (Encrypted)",
                            "\n",
                            { "->": ".^.^.^.16" },
                            null,
                          ],
                        },
                      ],
                      "nop",
                      "\n",
                      "ev",
                      { "VAR?": "know_skulker_weakness" },
                      "/ev",
                      [
                        { "->": ".^.b", c: true },
                        {
                          b: [
                            "\n",
                            "^Intel: Skulkers are vulnerable to high-frequency sonics.",
                            "\n",
                            { "->": ".^.^.^.22" },
                            null,
                          ],
                        },
                      ],
                      "nop",
                      "\n",
                      { "#f": 5, "#n": "g-5" },
                    ],
                    { "#f": 5, "#n": "g-4" },
                  ],
                  { "#f": 5, "#n": "g-3" },
                ],
                { "#f": 5, "#n": "g-2" },
              ],
              { "#f": 5, "#n": "g-1" },
            ],
            { "#f": 5, "#n": "g-0" },
          ],
          null,
        ],
        { "#f": 1 },
      ],
      check_status: [
        { "temp=": "return_to" },
        [
          "ev",
          { "f()": "display_status" },
          "pop",
          "/ev",
          "\n",
          "ev",
          "str",
          "^Return.",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 4 },
          { "c-0": ["\n", { "->": "return_to", var: true }, { "#f": 5 }] },
        ],
        { "#f": 1 },
      ],
      crafting_options: [
        { "temp=": "return_point" },
        [
          "^You lay out your scavenged materials.",
          "\n",
          "ev",
          "str",
          "^Fashion a Reinforced Club.",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Kaelen",
          "/str",
          "==",
          { "VAR?": "has_reinforced_club" },
          "!",
          "&&",
          { "VAR?": "has_metal_pipe" },
          "&&",
          { "VAR?": "has_thick_wiring" },
          "&&",
          "/ev",
          { "*": ".^.c-0", flg: 21 },
          "ev",
          "str",
          "^Assemble an EMP Grenade.",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Aris",
          "/str",
          "==",
          { "VAR?": "power_cell_stack" },
          0,
          ">",
          "&&",
          { "VAR?": "has_emp_grenade" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-1", flg: 5 },
          "ev",
          "str",
          "^Refine Glimmer Moss into Poison.",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Aris",
          "/str",
          "==",
          { "VAR?": "glimmer_moss_stack" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-2", flg: 5 },
          "ev",
          "str",
          "^Create a Poison Gas Bomb.",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Aris",
          "/str",
          "==",
          { "VAR?": "skulker_venom_gland_stack" },
          0,
          ">",
          "&&",
          { "VAR?": "power_cell_stack" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-3", flg: 5 },
          "ev",
          "str",
          "^Construct a Recurve Bow.",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Lena",
          "/str",
          "==",
          { "VAR?": "has_recurve_bow" },
          "!",
          "&&",
          { "VAR?": "has_flexible_polymer" },
          "&&",
          { "VAR?": "has_tensile_cable" },
          "&&",
          "/ev",
          { "*": ".^.c-4", flg: 21 },
          "ev",
          "str",
          "^Fletch Scrap Arrows.",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Lena",
          "/str",
          "==",
          { "VAR?": "has_recurve_bow" },
          "&&",
          "/ev",
          { "*": ".^.c-5", flg: 5 },
          "ev",
          "str",
          "^Craft Shock Arrows.",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Lena",
          "/str",
          "==",
          { "VAR?": "has_recurve_bow" },
          "&&",
          { "VAR?": "scrap_arrow_count" },
          0,
          ">",
          "&&",
          { "VAR?": "power_cell_stack" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-6", flg: 5 },
          "ev",
          "str",
          "^Equip the Reinforced Club.",
          "/str",
          { "VAR?": "has_reinforced_club" },
          { "VAR?": "club_equipped" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-7", flg: 21 },
          "ev",
          "str",
          "^Equip the Recurve Bow.",
          "/str",
          { "VAR?": "has_recurve_bow" },
          { "VAR?": "bow_equipped" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-8", flg: 21 },
          "ev",
          "str",
          "^That's all for now.",
          "/str",
          "/ev",
          { "*": ".^.c-9", flg: 4 },
          {
            "c-0": [
              "\n",
              "^You take the sturdy metal pipe and thick wiring you found. Using your strength, you wrap the wiring tightly around one end, creating a weighted, brutal-looking club. It feels solid in your hands.",
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "has_reinforced_club", re: true },
              "ev",
              { "VAR?": "atk" },
              2,
              "+",
              { "VAR=": "atk", re: true },
              "/ev",
              "ev",
              { "VAR?": "return_point" },
              "/ev",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-1": [
              "\n",
              "^You carefully pry open the casing of the Degraded Power Cell. Bypassing the safety regulators, you rig it to overload on impact. It's a volatile, single-use weapon, perfect for disabling electronics... or stunning biological targets.",
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "has_emp_grenade", re: true },
              "ev",
              { "VAR?": "power_cell_stack" },
              1,
              "-",
              { "VAR=": "power_cell_stack", re: true },
              "/ev",
              "ev",
              { "VAR?": "return_point" },
              "/ev",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-2": [
              "\n",
              "^You crush the Glimmer Moss, carefully isolating the coagulant you discovered earlier. By mixing it with a mild solvent from your kit, you refine it into a sticky, paralytic poison. You place it in an empty vial, ready for use.",
              "\n",
              "ev",
              { "VAR?": "has_moss_poison_vial" },
              1,
              "+",
              { "VAR=": "has_moss_poison_vial", re: true },
              "/ev",
              "ev",
              { "VAR?": "glimmer_moss_stack" },
              1,
              "-",
              { "VAR=": "glimmer_moss_stack", re: true },
              "/ev",
              "ev",
              { "VAR?": "return_point" },
              "/ev",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-3": [
              "\n",
              "^This is a dangerous idea... but a brilliant one. You carefully puncture the Skulker's venom gland, siphoning the potent neurotoxin into the casing of the Degraded Power Cell. You rig the cell to overload, not with an EMP, but with a thermal charge that will aerosolize the venom on impact. A devastating biological weapon.",
              "\n",
              "ev",
              { "VAR?": "poison_bomb_stack" },
              1,
              "+",
              { "VAR=": "poison_bomb_stack", re: true },
              "/ev",
              "ev",
              { "VAR?": "skulker_venom_gland_stack" },
              1,
              "-",
              { "VAR=": "skulker_venom_gland_stack", re: true },
              "/ev",
              "ev",
              { "VAR?": "power_cell_stack" },
              1,
              "-",
              { "VAR=": "power_cell_stack", re: true },
              "/ev",
              "ev",
              { "VAR?": "return_point" },
              "/ev",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-4": [
              "\n",
              "^You take the length of flexible polymer and the high-tensile cable. With your deft hands, you shape the polymer and string the cable, creating a makeshift but deadly silent bow. You'll need to find arrows, but the frame is perfect.",
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "has_recurve_bow", re: true },
              "ev",
              { "VAR?": "return_point" },
              "/ev",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-5": [
              "\n",
              "^You find a few thin pieces of rebar for shafts and some sharpened scrap metal for heads. With some tattered fabric for fletching, you can assemble a small quiver's worth of arrows.",
              "\n",
              "ev",
              2,
              4,
              "rnd",
              "/ev",
              { "temp=": "arrows_crafted" },
              "\n",
              "ev",
              { "VAR?": "scrap_arrow_count" },
              { "VAR?": "arrows_crafted" },
              "+",
              { "VAR=": "scrap_arrow_count", re: true },
              "/ev",
              "^You craft ",
              "ev",
              { "VAR?": "arrows_crafted" },
              "out",
              "/ev",
              "^ scrap arrows.",
              "\n",
              "ev",
              { "VAR?": "return_point" },
              "/ev",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-6": [
              "\n",
              "^You carefully attach the live wire from a Degraded Power Cell to the head of a scrap arrow. It's a crude but effective way to deliver a powerful electrical jolt on impact. You manage to create a small bundle of them before the power cell is drained.",
              "\n",
              "ev",
              1,
              3,
              "rnd",
              "/ev",
              { "temp=": "arrows_crafted2" },
              "\n",
              "ev",
              { "VAR?": "shock_arrow_count" },
              { "VAR?": "arrows_crafted2" },
              "+",
              { "VAR=": "shock_arrow_count", re: true },
              "/ev",
              "ev",
              { "VAR?": "scrap_arrow_count" },
              { "VAR?": "arrows_crafted2" },
              "-",
              { "VAR=": "scrap_arrow_count", re: true },
              "/ev",
              "ev",
              { "VAR?": "power_cell_stack" },
              1,
              "-",
              { "VAR=": "power_cell_stack", re: true },
              "/ev",
              "^You craft ",
              "ev",
              { "VAR?": "arrows_crafted" },
              "out",
              "/ev",
              "^ shock arrows.",
              "\n",
              "ev",
              { "VAR?": "return_point" },
              "/ev",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-7": [
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "club_equipped", re: true },
              "^You grip the club tightly. It's a crude but effective weapon. Your Attack has increased.",
              "\n",
              "ev",
              { "f()": "update_combat_stats" },
              "pop",
              "/ev",
              "\n",
              "ev",
              { "VAR?": "return_point" },
              "/ev",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-8": [
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "bow_equipped", re: true },
              "^You sling the bow over your shoulder. You'll be able to strike from the shadows. Your Attack has increased.",
              "\n",
              "ev",
              { "f()": "update_combat_stats" },
              "pop",
              "/ev",
              "\n",
              "ev",
              { "VAR?": "return_point" },
              "/ev",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-9": ["\n", { "->": "return_point", var: true }, { "#f": 5 }],
          },
        ],
        { "#f": 1 },
      ],
      use_glimmer_moss_tunnel: [
        { "temp=": "is_safe" },
        "ev",
        { "VAR?": "glimmer_moss_stack" },
        1,
        "-",
        { "VAR=": "glimmer_moss_stack", re: true },
        "/ev",
        "ev",
        0,
        "/ev",
        { "temp=": "heal_amount" },
        "^You pull out the pouch of glowing moss.",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Aris",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^Knowing its properties, you create a crude but effective poultice. The mild coagulant works quickly, and you feel a surge of vitality as the bioluminescent enzymes knit your tissues back together. You recover a significant amount of health.",
              "\n",
              "ev",
              { "VAR?": "max_hp" },
              0.5,
              "*",
              "INT",
              "/ev",
              { "temp=": "heal_amount", re: true },
              "\n",
              { "->": ".^.^.^.22" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "^You crush the moss and apply it to your wounds, hoping for the best. It feels cool and soothing, and you notice your bruises fading slightly. It's not a miracle cure, but it helps. You recover a small amount of health.",
              "\n",
              "ev",
              { "VAR?": "max_hp" },
              0.2,
              "*",
              "INT",
              "/ev",
              { "temp=": "heal_amount", re: true },
              "\n",
              { "->": ".^.^.^.22" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "hp" },
        { "VAR?": "heal_amount" },
        "+",
        { "VAR=": "hp", re: true },
        "/ev",
        "ev",
        { "VAR?": "hp" },
        { "VAR?": "max_hp" },
        ">",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "max_hp" },
              "/ev",
              { "VAR=": "hp", re: true },
              { "->": ".^.^.^.36" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "^The moss dissolves into a faint, glowing dust, its regenerative properties spent.",
        "\n",
        "ev",
        false,
        "/ev",
        { "VAR=": "is_injured", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "is_fatigued", re: true },
        "ev",
        { "VAR?": "is_safe" },
        "!",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              1,
              4,
              "rnd",
              "/ev",
              { "temp=": "attack_chance" },
              "\n",
              "ev",
              { "VAR?": "attack_chance" },
              1,
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^As the glow fades, you hear a frantic chittering sound. A small, rat-like creature with pale skin darts out from a crack in the wall, attracted by the moss's sweet scent. It nips at your boot before you can react, then vanishes back into the darkness. It didn't hurt, but the startling encounter has left you on edge.",
                    "\n",
                    "ev",
                    { "VAR?": "resolve" },
                    3,
                    "-",
                    { "VAR=": "resolve", re: true },
                    "/ev",
                    { "->": ".^.^.^.14" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.53" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        "void",
        "/ev",
        "->->",
        { "#f": 1 },
      ],
      scene_3_choices: [
        [
          "ev",
          "str",
          "^Investigate the rusted locker.",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          "ev",
          "str",
          "^Investigate the glowing moss.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 20 },
          "ev",
          "str",
          "^Query the AI.",
          "/str",
          "/ev",
          { "*": ".^.c-2", flg: 20 },
          "ev",
          "str",
          "^Use Skill",
          "/str",
          "/ev",
          { "*": ".^.c-3", flg: 20 },
          "ev",
          "str",
          "^Check Status.",
          "/str",
          "/ev",
          { "*": ".^.c-4", flg: 4 },
          "ev",
          "str",
          "^Analyze Items.",
          "/str",
          { "VAR?": "power_cell_stack" },
          0,
          ">",
          { "VAR?": "glimmer_moss_stack" },
          0,
          ">",
          "||",
          { "VAR?": "found_first_log" },
          "||",
          { "VAR?": "analyzed_first_log" },
          "!",
          "||",
          "/ev",
          { "*": ".^.c-5", flg: 5 },
          "ev",
          "str",
          "^Leave through the collapsed doorway.",
          "/str",
          "/ev",
          { "*": ".^.c-6", flg: 20 },
          {
            "c-0": ["\n", { "->": "locker_encounter" }, { "#f": 5 }],
            "c-1": ["\n", { "->": "moss_encounter" }, { "#f": 5 }],
            "c-2": ["\n", { "->": "scene_3_ai_query" }, { "#f": 5 }],
            "c-3": ["\n", { "->": "use_skill" }, { "#f": 5 }],
            "c-4": [
              "\n",
              "ev",
              { "^->": "scene_3_choices" },
              "/ev",
              { "->": "check_status" },
              { "#f": 5 },
            ],
            "c-5": ["\n", { "->": "analyze_items" }, { "#f": 5 }],
            "c-6": ["\n", { "->": "scene_4_the_first_obstacle" }, { "#f": 5 }],
          },
        ],
        { "#f": 3 },
      ],
      use_skill: [
        "ev",
        { "VAR?": "found_first_log" },
        "!",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              { "->": "find_first_log_event" },
              { "->": ".^.^.^.5" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      find_first_log_event: [
        [
          "ev",
          true,
          "/ev",
          { "VAR=": "found_first_log", re: true },
          "^As you concentrate, your unique skill reveals something unexpected.",
          "\n",
          "ev",
          { "VAR?": "character_name" },
          "str",
          "^Kaelen",
          "/str",
          "==",
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "^While scanning the debris for anything useful, your hand brushes against a flat, metallic object wedged beneath a twisted girder.",
                "\n",
                { "->": ".^.^.^.14" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "ev",
          { "VAR?": "character_name" },
          "str",
          "^Aris",
          "/str",
          "==",
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "^Your bio-scan sweeps the room, but it also registers a faint, encrypted energy signature from a small datapad tucked inside a corroded wall panel.",
                "\n",
                { "->": ".^.^.^.24" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "ev",
          { "VAR?": "character_name" },
          "str",
          "^Lena",
          "/str",
          "==",
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "^Your eyes, trained to spot things that don't belong, catch the unnatural straight edge of a datapad almost perfectly concealed in the shadows of the ceiling.",
                "\n",
                { "->": ".^.^.^.34" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "^You retrieve the device. It's an **Archivist Log**, its screen displaying a single, heavily encrypted file.",
          "\n",
          '^<i>AI: "Log from a previous test cycle detected. Decryption is possible, but the file is protected by a high-level algorithm."</i>',
          "\n",
          "ev",
          "str",
          "^Attempt to decrypt the log - Intelligence Check",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          "ev",
          "str",
          "^Leave it for now.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 20 },
          {
            "c-0": ["\n", { "->": "decrypt_first_log" }, { "#f": 5 }],
            "c-1": ["\n", { "->": "scene_3_choices" }, { "#f": 5 }],
          },
        ],
        { "#f": 1 },
      ],
      decrypt_first_log: [
        "ev",
        true,
        "/ev",
        { "VAR=": "analyzed_first_log", re: true },
        "ev",
        { "VAR?": "intelligence" },
        "/ev",
        { "temp=": "original_intelligence" },
        "ev",
        { "VAR?": "neuro_stim_state" },
        "str",
        "^ACTIVE",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "intelligence" },
              4,
              "+",
              { "VAR=": "intelligence", re: true },
              "/ev",
              { "->": ".^.^.^.16" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "^You focus on the complex encryption, trying to find a flaw in the code.",
        "\n",
        "ev",
        { "VAR?": "intelligence" },
        7,
        ">=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^With a surge of insight, you find a recursive loop in the encryption key. You bypass it, and a fragment of the log becomes readable. It's a combat record.",
              "\n",
              "^<b>LOG FRAGMENT 77-B:</b> \"...subject cornered by local fauna (Designation: 'Skulker'). Auditory sensitivity is a critical vulnerability. High-frequency sonics cause neural cascade failure...\"",
              "\n",
              "^You've learned a critical weakness of the Skulker creatures.",
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "know_skulker_weakness", re: true },
              "ev",
              { "VAR?": "neuro_stim_state" },
              "str",
              "^ACTIVE",
              "/str",
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^The intense focus fades as the Neuro-Stim wears off, leaving a dull ache behind your eyes.",
                    "\n",
                    "ev",
                    "str",
                    "^USED",
                    "/str",
                    "/ev",
                    { "VAR=": "neuro_stim_state", re: true },
                    { "->": ".^.^.^.19" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              { "VAR?": "original_intelligence" },
              1,
              "+",
              "/ev",
              { "VAR=": "intelligence", re: true },
              "ev",
              { "VAR?": "perception" },
              1,
              "+",
              { "VAR=": "perception", re: true },
              "/ev",
              { "->": "scene_3_choices" },
              { "->": ".^.^.^.27" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "^The code is a nonsensical wall of alien symbols. It's beyond your ability to comprehend right now.",
              "\n",
              '^<i>AI: "Decryption failed. Cognitive processing power is insufficient."</i>',
              "\n",
              "ev",
              { "VAR?": "neuro_stim_state" },
              "str",
              "^AVAILABLE",
              "/str",
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    '^<i>AI: "A neuro-stimulant is detected in your possession. Administering it may temporarily enhance cognitive function enough to bypass the encryption. This action will consume the device."</i>',
                    "\n",
                    "ev",
                    "str",
                    "^Use the Neuro-Stim and try again.",
                    "/str",
                    "/ev",
                    { "*": ".^.c-0", flg: 20 },
                    "ev",
                    "str",
                    "^Save it for later.",
                    "/str",
                    "/ev",
                    { "*": ".^.c-1", flg: 20 },
                    { "->": ".^.^.^.14" },
                    {
                      "c-0": [
                        "\n",
                        "^You press the Neuro-Stim against your neck. With a sharp hiss, it injects its contents. A cold fire spreads through your mind, clearing the fog of the crash.",
                        "\n",
                        "ev",
                        "str",
                        "^ACTIVE",
                        "/str",
                        "/ev",
                        { "VAR=": "neuro_stim_state", re: true },
                        { "->": ".^.^.^.^.^.^" },
                        { "#f": 5 },
                      ],
                      "c-1": ["\n", { "->": "scene_3_choices" }, { "#f": 5 }],
                    },
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    { "->": "scene_3_choices" },
                    { "->": ".^.^.^.14" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.27" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      locker_encounter: [
        "^The locker is old and heavy. The locking mechanism is a simple electronic keypad, now dark and corroded. The door itself is dented and sealed shut with rust.",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Kaelen",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^You plant your feet, grip the edge of the locker door, and pull with everything you've got.",
              "\n",
              "ev",
              { "VAR?": "strength" },
              7,
              ">=",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^The metal groans, shrieks, and finally tears open with a deafening CRUNCH. Inside, you find a **Degraded Power Cell**. It's heavy, but it might be useful.",
                    "\n",
                    "ev",
                    { "VAR?": "power_cell_stack" },
                    1,
                    "+",
                    { "VAR=": "power_cell_stack", re: true },
                    "/ev",
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "^You throw your shoulder against the door, but it only groans in protest. The rust holds it fast. You've only succeeded in bruising your shoulder and making a lot of noise.",
                    "\n",
                    "ev",
                    { "VAR?": "resolve" },
                    1,
                    "-",
                    { "VAR=": "resolve", re: true },
                    "/ev",
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.10" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Aris",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^You spot a frayed power conduit hanging from the ceiling. Rerouting the cable, you attempt to send a jolt of power to the corroded keypad.",
              "\n",
              "ev",
              { "VAR?": "intelligence" },
              8,
              ">=",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^It sparks to life for just a moment, long enough for the lock to disengage with a loud THUNK. Inside, you find a **Degraded Power Cell**. Fascinating.",
                    "\n",
                    "ev",
                    { "VAR?": "power_cell_stack" },
                    1,
                    "+",
                    { "VAR=": "power_cell_stack", re: true },
                    "/ev",
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "^You try to create a circuit, but the components are too decayed. A final spark from the conduit singes your fingers and the keypad goes completely dead. It's useless now.",
                    "\n",
                    "ev",
                    { "VAR?": "resolve" },
                    1,
                    "-",
                    { "VAR=": "resolve", re: true },
                    "/ev",
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.20" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Lena",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^The keypad is dead, but the manual override is still intact. Your deft fingers search for the tumblers.",
              "\n",
              "ev",
              { "VAR?": "agility" },
              7,
              ">=",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^With a series of satisfying clicks, the lock disengages. The door swings open silently. Inside, you find a **Degraded Power Cell**. A valuable find.",
                    "\n",
                    "ev",
                    { "VAR?": "power_cell_stack" },
                    1,
                    "+",
                    { "VAR=": "power_cell_stack", re: true },
                    "/ev",
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "^You work at the lock, but the internal mechanism is rusted solid. A lockpick snaps with a sharp *tink*, leaving the lock hopelessly jammed. There's no getting in that way now.",
                    "\n",
                    "ev",
                    { "VAR?": "resolve" },
                    1,
                    "-",
                    { "VAR=": "resolve", re: true },
                    "/ev",
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.30" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "->": "scene_3_choices" },
        { "#f": 1 },
      ],
      moss_encounter: [
        "^You move closer to the wall. The moss gives off a soft, ethereal green light. It pulses gently, like a slow heartbeat.",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Aris",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              '^<i>AI: "Glimmer Moss. Bioluminescent fungus. Mildly regenerative properties. Caution: Spores are a known attractant for subterranean fauna."</i>',
              "\n",
              { "->": ".^.^.^.11" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "perception" },
              6,
              ">=",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^You notice the ground beneath the moss is disturbed with small tracks, and you can smell a strange, sweet scent. This moss is more than just a pretty light; it's an active part of the ecosystem here.",
                    "\n",
                    { "->": ".^.^.^.8" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "^It's a weird glowing plant. Looks cool, but you don't know anything else about it.",
                    "\n",
                    { "->": ".^.^.^.8" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.11" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "->": ".^.harvest_moss" },
        {
          harvest_moss: [
            [
              "ev",
              "str",
              "^Take a sample.",
              "/str",
              "/ev",
              { "*": ".^.c-0", flg: 4 },
              "ev",
              "str",
              "^Leave it be.",
              "/str",
              "/ev",
              { "*": ".^.c-1", flg: 4 },
              {
                "c-0": [
                  "\n",
                  "^You scrape a handful of the glowing moss off the wall and store it in a pouch. It feels cool and damp to the touch.",
                  "\n",
                  "ev",
                  { "VAR?": "glimmer_moss_stack" },
                  1,
                  "+",
                  { "VAR=": "glimmer_moss_stack", re: true },
                  "/ev",
                  { "->": "scene_3_choices" },
                  { "#f": 5 },
                ],
                "c-1": ["\n", { "->": "scene_3_choices" }, { "#f": 5 }],
              },
            ],
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      scene_5a_the_cache: [
        "^You sprint towards the center of the plaza. A large, metallic crate is half-buried in a smoking crater. Standing over it is your rival. They turn as you approach, a hostile glint in their eyes.",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Kaelen",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^It's Xander. The last time you saw him was during the siege of Cygnus X-1. He was the one who gave the order to collapse the mining tunnels to halt the enemy's advance, burying hundreds of civilians to save a handful of soldiers. You never forgot the cold, pragmatic look in his eyes. For him, people are just pieces on a board.",
              "\n",
              { "->": ".^.^.^.10" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Aris",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              '^Jinx. Of course, it\'s her. You remember her from the Tycho Symposium. She presented a paper on "unstable xenomorphic adaptation," then demonstrated it by releasing a creature that nearly ate the entire bio-dome. You were the one who synthesized the counter-agent, shutting down her "experiment." She\'s never forgiven you for spoiling her fun.',
              "\n",
              { "->": ".^.^.^.20" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Lena",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^Isha. The hunter. You've only seen her once before, a fleeting shadow in the neon-drenched towers of Neo-Kyoto. You were extracting a data core; she was the security they sent when all else failed. You barely escaped, the sound of her high-caliber rifle echoing in your ears. She's the only person who has ever come close to catching you.",
              "\n",
              { "->": ".^.^.^.30" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "^There's no talking your way out of this.",
        "\n",
        { "->": ".^.setup_rival_battle" },
        {
          setup_rival_battle: [
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Kaelen",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  22,
                  "/ev",
                  { "VAR=": "rival_max_hp", re: true },
                  "ev",
                  8,
                  "/ev",
                  { "VAR=": "rival_atk", re: true },
                  "ev",
                  3,
                  "/ev",
                  { "VAR=": "rival_def", re: true },
                  { "->": ".^.^.^.8" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Aris",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  18,
                  "/ev",
                  { "VAR=": "rival_max_hp", re: true },
                  "ev",
                  6,
                  "/ev",
                  { "VAR=": "rival_atk", re: true },
                  "ev",
                  2,
                  "/ev",
                  { "VAR=": "rival_def", re: true },
                  { "->": ".^.^.^.18" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Lena",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  20,
                  "/ev",
                  { "VAR=": "rival_max_hp", re: true },
                  "ev",
                  7,
                  "/ev",
                  { "VAR=": "rival_atk", re: true },
                  "ev",
                  4,
                  "/ev",
                  { "VAR=": "rival_def", re: true },
                  { "->": ".^.^.^.28" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "rival_max_hp" },
            "/ev",
            { "VAR=": "rival_hp", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "used_skill_in_battle", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "rival_will_miss_next_turn", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "rival_is_defending", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "enemy_is_poisoned", re: true },
            "ev",
            0,
            "/ev",
            { "VAR=": "poison_turns_remaining", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "is_overcharging", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "is_countering", re: true },
            { "->": ".^.^.rival_battle_loop" },
            { "#f": 1 },
          ],
          rival_battle_loop: [
            [
              "^You have ",
              "ev",
              { "VAR?": "hp" },
              "out",
              "/ev",
              "^/",
              "ev",
              { "VAR?": "max_hp" },
              "out",
              "/ev",
              "^ HP. Your rival has ",
              "ev",
              { "VAR?": "rival_hp" },
              "out",
              "/ev",
              "^/",
              "ev",
              { "VAR?": "rival_max_hp" },
              "out",
              "/ev",
              "^ HP.",
              "\n",
              "ev",
              "str",
              "^Attack!",
              "/str",
              "/ev",
              { "*": ".^.c-0", flg: 4 },
              "ev",
              "str",
              "^Use Moss Poison (",
              "ev",
              { "VAR?": "has_moss_poison_vial" },
              "out",
              "/ev",
              "^ left)",
              "/str",
              { "VAR?": "character_name" },
              "str",
              "^Aris",
              "/str",
              "==",
              { "VAR?": "has_moss_poison_vial" },
              0,
              ">",
              "&&",
              "/ev",
              { "*": ".^.c-1", flg: 5 },
              "ev",
              "str",
              "^Use Poison Bomb",
              "/str",
              { "VAR?": "character_name" },
              "str",
              "^Aris",
              "/str",
              "==",
              { "VAR?": "poison_bomb_stack" },
              0,
              ">",
              "&&",
              "/ev",
              { "*": ".^.c-2", flg: 5 },
              "ev",
              "str",
              "^Defend",
              "/str",
              "/ev",
              { "*": ".^.c-3", flg: 4 },
              "ev",
              "str",
              "^Use Skill",
              "/str",
              { "VAR?": "used_skill_in_battle" },
              "!",
              "/ev",
              { "*": ".^.c-4", flg: 5 },
              "ev",
              "str",
              "^Give Up",
              "/str",
              "/ev",
              { "*": ".^.c-5", flg: 20 },
              {
                "c-0": [
                  "\n",
                  { "->": ".^.^.^.^.rival_player_attack" },
                  { "#f": 5 },
                ],
                "c-1": [
                  "\n",
                  { "->": ".^.^.^.^.rival_use_moss_poison" },
                  { "#f": 5 },
                ],
                "c-2": [
                  "\n",
                  { "->": ".^.^.^.^.rival_use_poison_bomb" },
                  { "#f": 5 },
                ],
                "c-3": [
                  "\n",
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "is_defending", re: true },
                  "^You anticipate your rival's next move and prepare to block.",
                  "\n",
                  { "->": ".^.^.^.^.rival_enemy_turn" },
                  { "#f": 5 },
                ],
                "c-4": [
                  "\n",
                  { "->": ".^.^.^.^.rival_use_skill" },
                  { "#f": 5 },
                ],
                "c-5": [
                  "\n",
                  { "->": ".^.^.^.^.rival_battle_give_up" },
                  { "#f": 5 },
                ],
              },
            ],
            { "#f": 1 },
          ],
          rival_use_moss_poison: [
            "ev",
            { "VAR?": "has_moss_poison_vial" },
            1,
            "-",
            { "VAR=": "has_moss_poison_vial", re: true },
            "/ev",
            "^You discreetly coat a small dart with the paralytic poison and fling it at your rival. It finds its mark. Your rival is now poisoned!",
            "\n",
            "ev",
            true,
            "/ev",
            { "VAR=": "enemy_is_poisoned", re: true },
            "ev",
            3,
            "/ev",
            { "VAR=": "poison_turns_remaining", re: true },
            { "->": ".^.^.rival_enemy_turn" },
            { "#f": 1 },
          ],
          rival_use_poison_bomb: [
            "ev",
            { "VAR?": "poison_bomb_stack" },
            1,
            "-",
            { "VAR=": "poison_bomb_stack", re: true },
            "/ev",
            "^You hurl the bomb at your rival's feet. It shatters, releasing a cloud of thick, green gas. Your rival coughs and staggers back, weakened and disoriented by the neurotoxin. They're in no condition to continue the fight.",
            "\n",
            "ev",
            1,
            "/ev",
            { "VAR=": "rival_hp", re: true },
            { "->": ".^.^.rival_battle_win" },
            { "#f": 1 },
          ],
          rival_use_skill: [
            "ev",
            true,
            "/ev",
            { "VAR=": "used_skill_in_battle", re: true },
            "ev",
            { "VAR?": "player_skills" },
            { "VAR?": "Survivalist" },
            "?",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You kick a cloud of dust and rubble into your rival's face, momentarily blinding them and giving you an opening. You feel a surge of adrenaline. Your Attack increases for this battle!",
                  "\n",
                  "ev",
                  { "VAR?": "atk" },
                  2,
                  "+",
                  { "VAR=": "atk", re: true },
                  "/ev",
                  { "->": ".^.^.^.^.rival_enemy_turn" },
                  { "->": ".^.^.^.10" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "player_skills" },
            { "VAR?": "BioScan" },
            "?",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You activate your bio-scanner, analyzing your rival's physiology in real-time. You spot a minor strain in their shoulder... a weak point. Their Defense is permanently lowered!",
                  "\n",
                  "ev",
                  { "VAR?": "rival_def" },
                  2,
                  "-",
                  { "VAR=": "rival_def", re: true },
                  "/ev",
                  "ev",
                  { "VAR?": "rival_def" },
                  0,
                  "<",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        "ev",
                        0,
                        "/ev",
                        { "VAR=": "rival_def", re: true },
                        { "->": ".^.^.^.15" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  { "->": ".^.^.^.^.rival_enemy_turn" },
                  { "->": ".^.^.^.18" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "player_skills" },
            { "VAR?": "DiscerningEye" },
            "?",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^Ignoring the direct threat, you watch your rival's footwork. You spot a subtle tell, a shift in weight just before they strike. You know exactly how to evade their next move. Your rival will miss their next attack.",
                  "\n",
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "rival_will_miss_next_turn", re: true },
                  { "->": ".^.^.^.^.rival_enemy_turn" },
                  { "->": ".^.^.^.26" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "#f": 1 },
          ],
          rival_player_attack: [
            "ev",
            { "VAR?": "rival_def" },
            "/ev",
            { "temp=": "current_rival_def" },
            "ev",
            { "VAR?": "rival_is_defending" },
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  { "VAR?": "rival_def" },
                  3,
                  "+",
                  "/ev",
                  { "temp=": "current_rival_def", re: true },
                  { "->": ".^.^.^.8" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            8,
            12,
            "rnd",
            10.0,
            "/",
            "/ev",
            { "temp=": "p_multiplier" },
            "\n",
            "ev",
            { "VAR?": "atk" },
            { "VAR?": "current_rival_def" },
            "-",
            "/ev",
            { "temp=": "p_base_dmg" },
            "ev",
            { "VAR?": "p_base_dmg" },
            1,
            "<",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  1,
                  "/ev",
                  { "temp=": "p_base_dmg", re: true },
                  { "->": ".^.^.^.31" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "p_base_dmg" },
            { "VAR?": "p_multiplier" },
            "*",
            "INT",
            "/ev",
            { "temp=": "p_final_dmg" },
            "\n",
            "ev",
            { "VAR?": "rival_hp" },
            { "VAR?": "p_final_dmg" },
            "-",
            { "VAR=": "rival_hp", re: true },
            "/ev",
            "ev",
            false,
            "/ev",
            { "VAR=": "rival_is_defending", re: true },
            "^You attack your rival, dealing ",
            "ev",
            { "VAR?": "p_final_dmg" },
            "out",
            "/ev",
            "^ damage!",
            "\n",
            "ev",
            { "VAR?": "rival_hp" },
            { "VAR?": "rival_max_hp" },
            3,
            "/",
            "INT",
            "<=",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  { "->": ".^.^.^.^.rival_battle_win" },
                  { "->": ".^.^.^.68" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  { "->": ".^.^.^.^.rival_enemy_turn" },
                  { "->": ".^.^.^.68" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "#f": 1 },
          ],
          rival_enemy_turn: [
            "ev",
            { "VAR?": "rival_used_non_combat_skill" },
            "!",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "rival_used_non_combat_skill", re: true },
                  [
                    "ev",
                    { "VAR?": "rival_name" },
                    "str",
                    "^Xander",
                    "/str",
                    "==",
                    "/ev",
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        '^Xander notices your exhaustion from the tunnel. "Already tired, Rook?" he taunts. He lunges, striking with brutal precision at your weak point.',
                        "\n",
                        "ev",
                        { "VAR?": "hp" },
                        4,
                        "-",
                        { "VAR=": "hp", re: true },
                        "/ev",
                        { "->": ".^.^.^.8" },
                        null,
                      ],
                    },
                  ],
                  [
                    "ev",
                    { "VAR?": "rival_name" },
                    "str",
                    "^Jinx",
                    "/str",
                    "==",
                    "/ev",
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        '^"Let\'s make this more interesting!" Jinx cackles, throwing a small, sparking device at your feet. It erupts in a flash of blinding light, leaving you disoriented. Your Defense is lowered for this fight.',
                        "\n",
                        "ev",
                        { "VAR?": "def" },
                        2,
                        "-",
                        { "VAR=": "def", re: true },
                        "/ev",
                        { "->": ".^.^.^.8" },
                        null,
                      ],
                    },
                  ],
                  [
                    "ev",
                    { "VAR?": "rival_name" },
                    "str",
                    "^Isha",
                    "/str",
                    "==",
                    "/ev",
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        '^Isha fires a single, precise arrow that thuds into the ground by your foot, pinning your bootlace to the metal grating. "Stay put," she says calmly. The move throws off your balance. Your Agility is temporarily lowered.',
                        "\n",
                        "ev",
                        { "VAR?": "agility" },
                        2,
                        "-",
                        { "VAR=": "agility", re: true },
                        "/ev",
                        { "->": ".^.^.^.8" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  { "->": ".^.^.^.^.rival_battle_loop" },
                  { "->": ".^.^.^.5" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "rival_will_miss_next_turn" },
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  false,
                  "/ev",
                  { "VAR=": "rival_will_miss_next_turn", re: true },
                  "^Using the opening you spotted, you easily sidestep your rival's clumsy attack. It misses completely.",
                  "\n",
                  { "->": ".^.^.^.^.rival_battle_loop" },
                  { "->": ".^.^.^.12" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  "ev",
                  1,
                  4,
                  "rnd",
                  "/ev",
                  { "temp=": "rival_action_roll" },
                  "\n",
                  [
                    "ev",
                    { "VAR?": "rival_action_roll" },
                    3,
                    "<=",
                    "/ev",
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        "ev",
                        8,
                        12,
                        "rnd",
                        10.0,
                        "/",
                        "/ev",
                        { "temp=": "r_multiplier" },
                        "\n",
                        "ev",
                        { "VAR?": "rival_atk" },
                        { "VAR?": "def" },
                        "-",
                        "/ev",
                        { "temp=": "r_base_dmg" },
                        "ev",
                        { "VAR?": "r_base_dmg" },
                        1,
                        "<",
                        "/ev",
                        [
                          { "->": ".^.b", c: true },
                          {
                            b: [
                              "\n",
                              "ev",
                              1,
                              "/ev",
                              { "temp=": "r_base_dmg", re: true },
                              { "->": ".^.^.^.22" },
                              null,
                            ],
                          },
                        ],
                        "nop",
                        "\n",
                        "ev",
                        { "VAR?": "r_base_dmg" },
                        { "VAR?": "r_multiplier" },
                        "*",
                        "INT",
                        "/ev",
                        { "temp=": "r_final_dmg" },
                        "\n",
                        "ev",
                        { "VAR?": "hp" },
                        { "VAR?": "r_final_dmg" },
                        "-",
                        { "VAR=": "hp", re: true },
                        "/ev",
                        "^Your rival counters, hitting you for ",
                        "ev",
                        { "VAR?": "r_final_dmg" },
                        "out",
                        "/ev",
                        "^ damage!",
                        "\n",
                        { "->": ".^.^.^.10" },
                        null,
                      ],
                    },
                  ],
                  [
                    { "->": ".^.b" },
                    {
                      b: [
                        "\n",
                        "ev",
                        true,
                        "/ev",
                        { "VAR=": "rival_is_defending", re: true },
                        "^Your rival anticipates your next move and takes a defensive stance.",
                        "\n",
                        { "->": ".^.^.^.10" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  "ev",
                  { "VAR?": "enemy_is_poisoned" },
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        "ev",
                        { "VAR?": "poison_turns_remaining" },
                        1,
                        "-",
                        { "VAR=": "poison_turns_remaining", re: true },
                        "/ev",
                        "^Your rival winces as the poison takes effect, dealing 4 damage.",
                        "\n",
                        "ev",
                        { "VAR?": "rival_hp" },
                        4,
                        "-",
                        { "VAR=": "rival_hp", re: true },
                        "/ev",
                        "ev",
                        { "VAR?": "poison_turns_remaining" },
                        0,
                        "<=",
                        "/ev",
                        [
                          { "->": ".^.b", c: true },
                          {
                            b: [
                              "\n",
                              "ev",
                              false,
                              "/ev",
                              { "VAR=": "enemy_is_poisoned", re: true },
                              "^The poison wears off.",
                              "\n",
                              { "->": ".^.^.^.21" },
                              null,
                            ],
                          },
                        ],
                        "nop",
                        "\n",
                        { "->": ".^.^.^.16" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  [
                    "ev",
                    { "VAR?": "rival_hp" },
                    { "VAR?": "rival_max_hp" },
                    3,
                    "/",
                    "<=",
                    "/ev",
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        { "->": ".^.^.^.^.^.^.rival_battle_win" },
                        { "->": ".^.^.^.21" },
                        null,
                      ],
                    },
                  ],
                  [
                    "ev",
                    { "VAR?": "hp" },
                    { "VAR?": "max_hp" },
                    3,
                    "/",
                    "<=",
                    "/ev",
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        { "->": ".^.^.^.^.^.^.rival_battle_lose_choice" },
                        { "->": ".^.^.^.21" },
                        null,
                      ],
                    },
                  ],
                  [
                    { "->": ".^.b" },
                    {
                      b: [
                        "\n",
                        { "->": ".^.^.^.^.^.^.rival_battle_loop" },
                        { "->": ".^.^.^.21" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  { "->": ".^.^.^.12" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "#f": 1 },
          ],
          rival_battle_give_up: [
            '^"Enough!" you pant, lowering your arms. "It\'s yours. I yield."',
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Kaelen",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  '^Xander stops, an eyebrow raised in amusement. He saunters over to the cache and pulls out the Emitter. "Wise choice, Rook. I was getting bored anyway." He looks at you, a calculating glint in his eye. "For being so... cooperative, I\'ll remember this. Consider it a favour." His tone makes it sound more like a threat.',
                  "\n",
                  { "->": ".^.^.^.10" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Aris",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  '^Jinx freezes mid-lunge, her manic grin faltering into a pout. "Awww, you\'re giving up? But the sparks were just getting pretty!" She bounces over and grabs the Emitter. "Fine, be a party pooper. But hey!" She points a greasy finger at you. "Since you didn\'t break all the way, I owe you one! It\'ll be something... flashy!"',
                  "\n",
                  { "->": ".^.^.^.20" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Lena",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  '^Isha lowers her weapon instantly, her predatory focus softening into a neutral calm. She retrieves the Emitter from the cache. "A tactical retreat. There is no dishonor in it," she says, her voice even. "You knew you were outmatched. For this, I owe you a debt. I will repay it." She gives a respectful nod before disappearing.',
                  "\n",
                  { "->": ".^.^.^.30" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            true,
            "/ev",
            { "VAR=": "rival_owes_favour", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "has_kinetic_emitter", re: true },
            "ev",
            true,
            "/ev",
            { "VAR=": "rival_has_emitter", re: true },
            "ev",
            3,
            "/ev",
            { "VAR=": "rival_emitter_charges", re: true },
            "ev",
            { "VAR?": "resolve" },
            5,
            "-",
            { "VAR=": "resolve", re: true },
            "/ev",
            "ev",
            { "VAR?": "rival_relationship" },
            15,
            "+",
            { "VAR=": "rival_relationship", re: true },
            "/ev",
            { "->": "post_rival_encounter" },
            { "#f": 1 },
          ],
          rival_battle_lose_choice: [
            [
              "^The blow sends you staggering back. You're injured and losing the fight, but there might be a way out of this.",
              "\n",
              "ev",
              "str",
              "^Use the Degraded Power Cell!",
              "/str",
              { "VAR?": "analyzed_power_cell" },
              "/ev",
              { "*": ".^.c-0", flg: 21 },
              "ev",
              "str",
              "^You're beaten. Surrender the drop.",
              "/str",
              "/ev",
              { "*": ".^.c-1", flg: 20 },
              {
                "c-0": [
                  "\n",
                  "^You pull out the unstable power cell. It's a desperate gambit. You hurl it at the supply cache. The resulting explosion is deafening, frying the crate's contents and forcing your rival to dive for cover. The prize is gone, but you've denied them the victory.",
                  "\n",
                  "ev",
                  { "VAR?": "power_cell_stack" },
                  1,
                  "-",
                  { "VAR=": "power_cell_stack", re: true },
                  "/ev",
                  "ev",
                  false,
                  "/ev",
                  { "VAR=": "has_kinetic_emitter", re: true },
                  "ev",
                  false,
                  "/ev",
                  { "VAR=": "rival_has_emitter", re: true },
                  "ev",
                  { "VAR?": "rival_relationship" },
                  15,
                  "-",
                  { "VAR=": "rival_relationship", re: true },
                  "/ev",
                  "ev",
                  { "VAR?": "resolve" },
                  5,
                  "-",
                  { "VAR=": "resolve", re: true },
                  "/ev",
                  { "->": "post_rival_encounter" },
                  { "#f": 5 },
                ],
                "c-1": [
                  "\n",
                  "ev",
                  { "VAR?": "character_name" },
                  "str",
                  "^Kaelen",
                  "/str",
                  "==",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        '^You collapse to one knee, unable to continue the fight. Xander looms over you, snatching the Emitter from the cache. "Just as I thought," he sneers, his voice dripping with contempt. "All that talk of honor, and you\'re just another piece of meat for the grinder. Stay down." He kicks dust in your face before turning his back and walking away.',
                        "\n",
                        { "->": ".^.^.^.9" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  "ev",
                  { "VAR?": "character_name" },
                  "str",
                  "^Aris",
                  "/str",
                  "==",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        '^You stumble, your calculations failing you in the heat of the moment. Jinx zips past you and pulls the Emitter from the crate, holding it up like a new toy. "Awww, is the little experiment over already?" she pouts, her manic grin never faltering. "Don\'t worry, I\'ll find a much... *louder* use for this than you ever would! Toodles!" With a chaotic laugh, she\'s gone.',
                        "\n",
                        { "->": ".^.^.^.19" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  "ev",
                  { "VAR?": "character_name" },
                  "str",
                  "^Lena",
                  "/str",
                  "==",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        '^You misjudge her speed, and her counter-attack leaves you breathless and exposed. Isha calmly walks to the cache and retrieves the Emitter. She gives it a cursory glance, then looks back at you with the patient eyes of a hunter. "You rely too much on the shadows," she says, her voice devoid of malice. "They can\'t save you when you\'re caught in the open. You were good prey, but the hunt is over." She nods once, then vanishes.',
                        "\n",
                        { "->": ".^.^.^.29" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  "ev",
                  false,
                  "/ev",
                  { "VAR=": "has_kinetic_emitter", re: true },
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "rival_has_emitter", re: true },
                  "ev",
                  3,
                  "/ev",
                  { "VAR=": "rival_emitter_charges", re: true },
                  "ev",
                  { "VAR?": "resolve" },
                  10,
                  "-",
                  { "VAR=": "resolve", re: true },
                  "/ev",
                  "ev",
                  { "VAR?": "rival_relationship" },
                  5,
                  "-",
                  { "VAR=": "rival_relationship", re: true },
                  "/ev",
                  { "->": "scene_6_first_test" },
                  { "#f": 5 },
                ],
              },
            ],
            { "#f": 1 },
          ],
          rival_battle_win: [
            "^Your final blow connects, and your rival stumbles back, winded and defeated. The fight is over. You've won. You claim the **Kinetic Field Emitter** from the supply cache.",
            "\n",
            "ev",
            true,
            "/ev",
            { "VAR=": "has_kinetic_emitter", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "rival_has_emitter", re: true },
            "ev",
            3,
            "/ev",
            { "VAR=": "emitter_charges", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "used_skill_in_battle", re: true },
            { "->": "post_rival_encounter" },
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      post_rival_encounter: [
        [
          "^The adrenaline fades, leaving you panting in the quiet plaza.",
          "\n",
          [
            "ev",
            { "VAR?": "hp" },
            { "VAR?": "max_hp" },
            4,
            "/",
            "<=",
            "/ev",
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "ev",
                true,
                "/ev",
                { "VAR=": "is_injured", re: true },
                "ev",
                false,
                "/ev",
                { "VAR=": "is_fatigued", re: true },
                "^You're badly wounded, and every movement is a struggle.",
                "\n",
                { "->": ".^.^.^.5" },
                null,
              ],
            },
          ],
          [
            "ev",
            { "VAR?": "hp" },
            { "VAR?": "max_hp" },
            2,
            "/",
            "<=",
            "/ev",
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "ev",
                false,
                "/ev",
                { "VAR=": "is_injured", re: true },
                "ev",
                true,
                "/ev",
                { "VAR=": "is_fatigued", re: true },
                "^You're bruised and exhausted from the fight.",
                "\n",
                { "->": ".^.^.^.5" },
                null,
              ],
            },
          ],
          [
            { "->": ".^.b" },
            {
              b: [
                "\n",
                "ev",
                false,
                "/ev",
                { "VAR=": "is_injured", re: true },
                "ev",
                false,
                "/ev",
                { "VAR=": "is_fatigued", re: true },
                { "->": ".^.^.^.5" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "^You take a moment to catch your breath before moving on.",
          "\n",
          "ev",
          "str",
          "^Rest for a moment.",
          "/str",
          { "VAR?": "is_fatigued" },
          "/ev",
          { "*": ".^.c-0", flg: 21 },
          "ev",
          "str",
          "^Look for something to treat your wounds.",
          "/str",
          { "VAR?": "is_injured" },
          { "VAR?": "glimmer_moss_stack" },
          0,
          "==",
          "&&",
          "/ev",
          { "*": ".^.c-1", flg: 21 },
          "ev",
          "str",
          "^Use the Glimmer Moss to tend to your wounds.",
          "/str",
          { "VAR?": "glimmer_moss_stack" },
          0,
          ">",
          { "VAR?": "is_injured" },
          "&&",
          "/ev",
          { "*": ".^.c-2", flg: 21 },
          "ev",
          "str",
          "^Study the Kinetic Field Emitter.",
          "/str",
          { "VAR?": "has_kinetic_emitter" },
          { "VAR?": "studied_emitter" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-3", flg: 21 },
          "ev",
          "str",
          "^Equip the Kinetic Field Emitter.",
          "/str",
          { "VAR?": "has_kinetic_emitter" },
          { "VAR?": "emitter_equipped" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-4", flg: 21 },
          "ev",
          "str",
          "^Continue on.",
          "/str",
          "/ev",
          { "*": ".^.c-5", flg: 20 },
          "ev",
          "str",
          "^Check Status.",
          "/str",
          "/ev",
          { "*": ".^.c-6", flg: 4 },
          {
            "c-0": ["\n", { "->": ".^.^.^.rest_a_moment" }, { "#f": 5 }],
            "c-1": ["\n", { "->": ".^.^.^.look_for_moss" }, { "#f": 5 }],
            "c-2": [
              "\n",
              "ev",
              false,
              "/ev",
              { "->": "use_glimmer_moss_tunnel" },
              { "#f": 5 },
            ],
            "c-3": ["\n", { "->": ".^.^.^.study_emitter" }, { "#f": 5 }],
            "c-4": ["\n", { "->": ".^.^.^.equip_emitter" }, { "#f": 5 }],
            "c-5": ["\n", { "->": "scene_6_first_test" }, { "#f": 5 }],
            "c-6": [
              "\n",
              "ev",
              { "^->": "post_rival_encounter" },
              "/ev",
              { "->": "check_status" },
              { "#f": 5 },
            ],
          },
        ],
        {
          rest_a_moment: [
            "^You find a relatively dry piece of rubble to sit on, closing your eyes for a moment to let the exhaustion wash over you. After a few minutes, you feel a bit more clear-headed. The fatigue subsides.",
            "\n",
            "ev",
            false,
            "/ev",
            { "VAR=": "is_fatigued", re: true },
            { "->": ".^.^" },
            { "#f": 1 },
          ],
          look_for_moss: [
            "^You scan the damp, shadowy corners of the plaza, looking for any signs of the glowing fungus you saw in the maintenance bay.",
            "\n",
            "ev",
            { "VAR?": "perception" },
            6,
            ">=",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^Your sharp eyes spot a small patch clinging to the underside of a collapsed archway. You carefully scrape a sample into a pouch.",
                  "\n",
                  "ev",
                  { "VAR?": "glimmer_moss_stack" },
                  1,
                  "+",
                  { "VAR=": "glimmer_moss_stack", re: true },
                  "/ev",
                  { "->": ".^.^.^.9" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  "^The plaza is picked clean. There's nothing here that can help you. You'll have to push on despite your injuries.",
                  "\n",
                  { "->": ".^.^.^.9" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "->": ".^.^" },
            { "#f": 1 },
          ],
          equip_emitter: [
            "ev",
            true,
            "/ev",
            { "VAR=": "emitter_equipped", re: true },
            "^You strap the Kinetic Field Emitter to your forearm. It feels heavy, but hums with a responsive energy. You feel more dangerous.",
            "\n",
            "ev",
            { "f()": "update_combat_stats" },
            "pop",
            "/ev",
            "\n",
            '^<i>AI: "Emitter equipped. Unit has ',
            "ev",
            { "VAR?": "emitter_charges" },
            "out",
            "/ev",
            "^ charges remaining. Attack is now ",
            "ev",
            { "VAR?": "atk" },
            "out",
            "/ev",
            '^."</i>',
            "\n",
            { "->": ".^.^" },
            { "#f": 1 },
          ],
          study_emitter: [
            "ev",
            true,
            "/ev",
            { "VAR=": "studied_emitter", re: true },
            "^You take a closer look at the device you won from your rival. It's a heavy, metallic object that hums with a low, contained energy.",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Aris",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You carefully pry open a maintenance hatch, your multi-tool tracing the alien circuitry. The design is elegant, almost organic. Understanding a fraction of its power source feels like unlocking a new law of physics. Your mind expands with the possibilities.",
                  "\n",
                  "ev",
                  { "VAR?": "intelligence" },
                  1,
                  "+",
                  { "VAR=": "intelligence", re: true },
                  "/ev",
                  "ev",
                  { "f()": "update_combat_stats" },
                  "pop",
                  "/ev",
                  "\n",
                  '^<i>AI: "Intelligence increased to ',
                  "ev",
                  { "VAR?": "intelligence" },
                  "out",
                  "/ev",
                  '^."</i>',
                  "\n",
                  { "->": ".^.^.^.14" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Lena",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You handle the Emitter like a new weapon, testing its weight and grip. You spot a subtle calibration dial near the power conduit. A few careful adjustments, and the hum changes pitch, sounding more stable and efficient. You've learned to read its subtle cues.",
                  "\n",
                  "ev",
                  { "VAR?": "perception" },
                  1,
                  "+",
                  { "VAR=": "perception", re: true },
                  "/ev",
                  "ev",
                  { "f()": "update_combat_stats" },
                  "pop",
                  "/ev",
                  "\n",
                  '^<i>AI: "Perception increased to ',
                  "ev",
                  { "VAR?": "perception" },
                  "out",
                  "/ev",
                  '^."</i>',
                  "\n",
                  { "->": ".^.^.^.24" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Kaelen",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^This thing is beyond you, but you understand tools of war. You run it through a series of power-on and power-off tests, getting a feel for its activation time and recoil. It's not a gun, but it is a weapon. Knowing you can master it settles your nerves.",
                  "\n",
                  "ev",
                  { "VAR?": "resolve" },
                  5,
                  "+",
                  { "VAR=": "resolve", re: true },
                  "/ev",
                  '^<i>AI: "Resolve has stabilized."</i>',
                  "\n",
                  { "->": ".^.^.^.34" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "->": ".^.^" },
            { "#f": 1 },
          ],
          "#f": 3,
        },
      ],
      scene_6a_rooftops: [
        [
          "^You follow a treacherous path across rusted gantries to the base of a massive, dilapidated communications tower clinging to the side of a skyscraper. The AI's voice chimes in.",
          "\n",
          '^<i>AI: "Archivist Test Chamber detected. Objective: Retrieve Data Fragment from the tower\'s broadcast antenna. Warning: Structural integrity is compromised."</i>',
          "\n",
          "^The wind howls around you. It's a long, dangerous climb.",
          "\n",
          "ev",
          "str",
          "^Begin the Climb - Agility Check",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          {
            "c-0": [
              "\n",
              "ev",
              1,
              6,
              "rnd",
              "/ev",
              { "temp=": "roll" },
              "\n",
              "ev",
              { "VAR?": "agility" },
              { "VAR?": "roll" },
              "+",
              "/ev",
              { "temp=": "total_skill" },
              "ev",
              { "VAR?": "total_skill" },
              10,
              ">=",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^You move with grace and speed, the dizzying height feeling like home. You easily navigate the broken ladders and exposed rebar, retrieving the **first Data Fragment** from the antenna array at the top.",
                    "\n",
                    "ev",
                    { "VAR?": "data_fragments" },
                    1,
                    "+",
                    { "VAR=": "data_fragments", re: true },
                    "/ev",
                    { "->": "scene_7_the_fragment" },
                    { "->": ".^.^.^.21" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "^A handhold crumbles under your grip, and a gust of wind throws you off balance. You fall a short distance, slamming into a lower platform. Wounded and aching, you realize you can't reach the top from here.",
                    "\n",
                    '^<i>AI: "Subject has failed the test. Data Fragment unretrievable."</i>',
                    "\n",
                    "ev",
                    { "VAR?": "resolve" },
                    10,
                    "-",
                    { "VAR=": "resolve", re: true },
                    "/ev",
                    "ev",
                    true,
                    "/ev",
                    { "VAR=": "is_injured", re: true },
                    { "->": "scene_7_scavenge" },
                    { "->": ".^.^.^.21" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "#f": 5 },
            ],
          },
        ],
        { "#f": 1 },
      ],
      scene_6b_subway: [
        [
          "^You descend into a flooded subway station, lit by the eerie green glow of moss. In the center of the platform is a functioning Archivist terminal, humming with power.",
          "\n",
          '^<i>AI: "Archivist Test Chamber detected. Objective: Access the terminal to download one Data Fragment."</i>',
          "\n",
          "^A single, powerful Slick-skinned Skulker guards the terminal, its eyeless head twitching at every sound.",
          "\n",
          "ev",
          "str",
          "^Use your knowledge of its weakness.",
          "/str",
          { "VAR?": "know_skulker_weakness" },
          "/ev",
          { "*": ".^.c-0", flg: 21 },
          "ev",
          "str",
          "^Use the Emitter's concussive blast (",
          "ev",
          { "VAR?": "emitter_charges" },
          "out",
          "/ev",
          "^ left).",
          "/str",
          { "VAR?": "has_kinetic_emitter" },
          { "VAR?": "emitter_charges" },
          0,
          ">",
          "&&",
          { "VAR?": "emitter_equipped" },
          "&&",
          "/ev",
          { "*": ".^.c-1", flg: 21 },
          "ev",
          "str",
          "^Attempt to use the broken Emitter.",
          "/str",
          { "VAR?": "has_kinetic_emitter" },
          { "VAR?": "emitter_charges" },
          0,
          "<=",
          "&&",
          "/ev",
          { "*": ".^.c-2", flg: 21 },
          "ev",
          "str",
          "^Engage the Skulker",
          "/str",
          "/ev",
          { "*": ".^.c-3", flg: 20 },
          "ev",
          "str",
          "^Sneak to the Terminal",
          "/str",
          "/ev",
          { "*": ".^.c-4", flg: 20 },
          "ev",
          "str",
          "^Analyze the Environment",
          "/str",
          "/ev",
          { "*": ".^.c-5", flg: 20 },
          {
            "c-0": [
              "\n",
              { "->": ".^.^.^.exploit_skulker_weakness" },
              { "#f": 5 },
            ],
            "c-1": [
              "\n",
              { "->": ".^.^.^.use_emitter_on_skulker" },
              { "#f": 5 },
            ],
            "c-2": [
              "\n",
              "^You raise the Emitter and try to activate it, but it remains silent and cold. The power is completely spent. It's useless.",
              "\n",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-3": ["\n", { "->": "engage_skulker_setup" }, { "#f": 5 }],
            "c-4": ["\n", { "->": "sneak_past_skulker" }, { "#f": 5 }],
            "c-5": ["\n", { "->": "analyze_skulker_env" }, { "#f": 5 }],
          },
        ],
        {
          exploit_skulker_weakness: [
            "^Remembering the Archivist Log, you tell your implant to emit a high-frequency sonic pulse. The AI complies. A piercing, silent-to-you shriek fills the subway. The Skulker instantly recoils, thrashing in agony as its sensitive auditory organs overload. It collapses, twitching, completely incapacitated.",
            "\n",
            "^The path to the terminal is clear. Your intel paid off.",
            "\n",
            { "->": "skulker_defeated_hub" },
            { "#f": 1 },
          ],
          use_emitter_on_skulker: [
            "ev",
            { "f()": "use_emitter_charge" },
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^The blast hits the Skulker, sending it flying backwards into the tunnel wall with a wet smack. It's stunned and incapacitated. The path to the terminal is clear, and you easily download the **first Data Fragment**.",
                  "\n",
                  { "->": "skulker_defeated_hub" },
                  { "->": ".^.^.^.4" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      engage_skulker_setup: [
        "ev",
        "str",
        "^Slick-Skinned Skulker",
        "/str",
        "/ev",
        { "VAR=": "current_enemy_name", re: true },
        "ev",
        15,
        "/ev",
        { "VAR=": "current_enemy_hp", re: true },
        "ev",
        8,
        "/ev",
        { "VAR=": "current_enemy_atk", re: true },
        "ev",
        2,
        "/ev",
        { "VAR=": "current_enemy_def", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "used_skill_in_battle", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "rival_will_miss_next_turn", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "enemy_is_poisoned", re: true },
        "ev",
        0,
        "/ev",
        { "VAR=": "poison_turns_remaining", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "enemy2_is_poisoned", re: true },
        "ev",
        0,
        "/ev",
        { "VAR=": "poison2_turns_remaining", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "is_overcharging", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "is_countering", re: true },
        "^The Skulker lets out a piercing shriek and lunges!",
        "\n",
        { "->": "battle_loop" },
        {
          skulker_battle_loop: [
            [
              "^You have ",
              "ev",
              { "VAR?": "hp" },
              "out",
              "/ev",
              "^/",
              "ev",
              { "VAR?": "max_hp" },
              "out",
              "/ev",
              "^ HP. The ",
              "ev",
              { "VAR?": "current_enemy_name" },
              "out",
              "/ev",
              "^ has ",
              "ev",
              { "VAR?": "current_enemy_hp" },
              "out",
              "/ev",
              "^ HP.",
              "\n",
              "ev",
              "str",
              "^Attack!",
              "/str",
              "/ev",
              { "*": ".^.c-0", flg: 4 },
              "ev",
              "str",
              "^Defend",
              "/str",
              "/ev",
              { "*": ".^.c-1", flg: 20 },
              {
                "c-0": [
                  "\n",
                  { "->": ".^.^.^.^.skulker_player_attack" },
                  { "#f": 5 },
                ],
                "c-1": [
                  "\n",
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "is_defending", re: true },
                  "^You brace for the Skulker's attack, readying your defenses.",
                  "\n",
                  { "->": ".^.^.^.^.skulker_enemy_turn" },
                  { "#f": 5 },
                ],
              },
            ],
            { "#f": 1 },
          ],
          skulker_player_attack: [
            "ev",
            8,
            12,
            "rnd",
            10.0,
            "/",
            "/ev",
            { "temp=": "p_multiplier" },
            "\n",
            "ev",
            { "VAR?": "atk" },
            { "VAR?": "current_enemy_def" },
            "-",
            "/ev",
            { "temp=": "p_base_dmg" },
            "ev",
            { "VAR?": "p_base_dmg" },
            1,
            "<",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  1,
                  "/ev",
                  { "temp=": "p_base_dmg", re: true },
                  { "->": ".^.^.^.21" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "p_base_dmg" },
            { "VAR?": "p_multiplier" },
            "*",
            "INT",
            "/ev",
            { "temp=": "p_final_dmg" },
            "\n",
            "ev",
            { "VAR?": "current_enemy_hp" },
            { "VAR?": "p_final_dmg" },
            "-",
            { "VAR=": "current_enemy_hp", re: true },
            "/ev",
            "^You attack the Skulker, dealing ",
            "ev",
            { "VAR?": "p_final_dmg" },
            "out",
            "/ev",
            "^ damage!",
            "\n",
            "ev",
            { "VAR?": "current_enemy_hp" },
            0,
            "<=",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: ["\n", { "->": "skulker_win" }, { "->": ".^.^.^.51" }, null],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  { "->": ".^.^.^.^.skulker_enemy_turn" },
                  { "->": ".^.^.^.51" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "#f": 1 },
          ],
          skulker_enemy_turn: [
            "ev",
            { "VAR?": "def" },
            "/ev",
            { "temp=": "current_def" },
            "ev",
            { "VAR?": "is_defending" },
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  { "VAR?": "def" },
                  3,
                  "+",
                  "/ev",
                  { "temp=": "current_def", re: true },
                  { "->": ".^.^.^.8" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            8,
            12,
            "rnd",
            10.0,
            "/",
            "/ev",
            { "temp=": "r_multiplier" },
            "\n",
            "ev",
            { "VAR?": "current_enemy_atk" },
            { "VAR?": "current_def" },
            "-",
            "/ev",
            { "temp=": "r_base_dmg" },
            "ev",
            { "VAR?": "r_base_dmg" },
            1,
            "<",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  1,
                  "/ev",
                  { "temp=": "r_base_dmg", re: true },
                  { "->": ".^.^.^.31" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "r_base_dmg" },
            { "VAR?": "r_multiplier" },
            "*",
            "INT",
            "/ev",
            { "temp=": "r_final_dmg" },
            "\n",
            "ev",
            { "VAR?": "hp" },
            { "VAR?": "r_final_dmg" },
            "-",
            { "VAR=": "hp", re: true },
            "/ev",
            "^The Skulker counters, hitting you for ",
            "ev",
            { "VAR?": "r_final_dmg" },
            "out",
            "/ev",
            "^ damage!",
            "\n",
            "ev",
            false,
            "/ev",
            { "VAR=": "is_defending", re: true },
            [
              "ev",
              { "VAR?": "hp" },
              0,
              "<=",
              "/ev",
              { "->": ".^.b", c: true },
              {
                b: ["\n", { "->": "battle_lost" }, { "->": ".^.^.^.61" }, null],
              },
            ],
            [
              "ev",
              { "VAR?": "hp" },
              { "VAR?": "max_hp" },
              2,
              "/",
              "<=",
              "/ev",
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  { "->": ".^.^.^.^.skulker_battle_run_or_fight" },
                  { "->": ".^.^.^.61" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  { "->": ".^.^.^.^.skulker_battle_loop" },
                  { "->": ".^.^.^.61" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "#f": 1 },
          ],
          skulker_battle_run_or_fight: [
            [
              "^The creature's blow wounds you badly. You're hurt, but you see a chance to disengage and escape.",
              "\n",
              "ev",
              "str",
              "^Continue Fighting",
              "/str",
              "/ev",
              { "*": ".^.c-0", flg: 20 },
              "ev",
              "str",
              "^Run Away",
              "/str",
              "/ev",
              { "*": ".^.c-1", flg: 20 },
              {
                "c-0": [
                  "\n",
                  "^You grit your teeth and press the attack, ignoring the pain.",
                  "\n",
                  { "->": ".^.^.^.^.skulker_battle_loop" },
                  { "#f": 5 },
                ],
                "c-1": [
                  "\n",
                  "^You take the opening and scramble back into the flooded tunnels. The Skulker shrieks in frustration but doesn't follow. You've escaped, but failed the test.",
                  "\n",
                  '^<i>AI: "Subject has failed the test. Data Fragment unretrievable."</i>',
                  "\n",
                  "ev",
                  { "VAR?": "resolve" },
                  10,
                  "-",
                  { "VAR=": "resolve", re: true },
                  "/ev",
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "is_injured", re: true },
                  { "->": "scene_8_the_tower" },
                  { "#f": 5 },
                ],
              },
            ],
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      sneak_past_skulker: [
        "ev",
        1,
        6,
        "rnd",
        "/ev",
        { "temp=": "roll" },
        "\n",
        "ev",
        { "VAR?": "agility" },
        { "VAR?": "roll" },
        "+",
        "/ev",
        { "temp=": "total_skill" },
        "ev",
        { "VAR?": "total_skill" },
        10,
        ">=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^You slip through the shadows, your footsteps silent in the shallow water. The creature never senses you as you reach the terminal, download the **first Data Fragment**, and slip away.",
              "\n",
              "ev",
              { "VAR?": "data_fragments" },
              1,
              "+",
              { "VAR=": "data_fragments", re: true },
              "/ev",
              { "->": "scene_7_the_fragment" },
              { "->": ".^.^.^.20" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "^A loose piece of debris clatters under your foot. The Skulker shrieks and lunges. You barely manage to escape its claws, retreating with your heart pounding in your chest.",
              "\n",
              '^<i>AI: "Subject has failed the test. Data Fragment unretrievable."</i>',
              "\n",
              "ev",
              { "VAR?": "resolve" },
              10,
              "-",
              { "VAR=": "resolve", re: true },
              "/ev",
              "ev",
              { "VAR?": "hp" },
              5,
              "-",
              { "VAR=": "hp", re: true },
              "/ev",
              "ev",
              true,
              "/ev",
              { "VAR=": "is_injured", re: true },
              { "->": "scene_8_the_tower" },
              { "->": ".^.^.^.20" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      analyze_skulker_env: [
        "ev",
        1,
        6,
        "rnd",
        "/ev",
        { "temp=": "roll" },
        "\n",
        "ev",
        { "VAR?": "intelligence" },
        { "VAR?": "roll" },
        "+",
        "/ev",
        { "temp=": "total_skill" },
        "ev",
        { "VAR?": "total_skill" },
        11,
        ">=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^You notice a leaking water pipe on the ceiling directly above a sparking, exposed power conduit near the creature. You throw a piece of rubble, shattering the pipe. Water gushes onto the conduit, creating a massive electrical surge that incapacitates the Skulker. You download the **first Data Fragment**.",
              "\n",
              "ev",
              { "VAR?": "data_fragments" },
              1,
              "+",
              { "VAR=": "data_fragments", re: true },
              "/ev",
              { "->": "scene_7_the_fragment" },
              { "->": ".^.^.^.20" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "^You see a potential environmental advantage but miscalculate. Your attempt to create a distraction only succeeds in making a loud noise, drawing the Skulker's immediate, aggressive attention. You are forced to flee.",
              "\n",
              '^<i>AI: "Subject has failed the test. Data Fragment unretrievable."</i>',
              "\n",
              "ev",
              { "VAR?": "resolve" },
              10,
              "-",
              { "VAR=": "resolve", re: true },
              "/ev",
              { "->": "scene_8_the_tower" },
              { "->": ".^.^.^.20" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        {
          setup_skulker_battle: [
            "ev",
            "str",
            "^Slick-Skinned Skulker",
            "/str",
            "/ev",
            { "VAR=": "current_enemy_name", re: true },
            "ev",
            15,
            "/ev",
            { "VAR=": "current_enemy_hp", re: true },
            "ev",
            5,
            "/ev",
            { "VAR=": "current_enemy_atk", re: true },
            "ev",
            2,
            "/ev",
            { "VAR=": "current_enemy_def", re: true },
            { "->": "battle_loop" },
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      scene_6c_skulker_lair: [
        [
          "^Darkness... and the stench of decay and damp earth. You wake with a gasp, every muscle screaming in protest. Your head throbs where the Skulker struck you.",
          "\n",
          "^You're in a narrow cavern, barely lit by patches of glowing moss. It seems to be the creature's nest, built from scavenged metal and a strange, hardened resin. The Skulker that defeated you is here, its pale back to you, noisily chewing on some unidentifiable piece of carrion. You are weak, and your gear is scattered just out of reach.",
          "\n",
          "^You see one clear exit, a narrow tunnel leading back up into the subway system. This is your only chance.",
          "\n",
          "ev",
          "str",
          "^Try to sneak past it - Agility Check",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          "ev",
          "str",
          "^Create a diversion - Intelligence Check",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 20 },
          {
            "c-0": ["\n", { "->": ".^.^.^.escape_lair_sneak" }, { "#f": 5 }],
            "c-1": [
              "\n",
              { "->": ".^.^.^.escape_lair_diversion" },
              { "#f": 5 },
            ],
          },
        ],
        {
          escape_lair_sneak: [
            "ev",
            1,
            6,
            "rnd",
            "/ev",
            { "temp=": "roll" },
            "\n",
            "ev",
            { "VAR?": "agility" },
            { "VAR?": "roll" },
            "+",
            "/ev",
            { "temp=": "total_skill" },
            "ev",
            { "VAR?": "total_skill" },
            12,
            ">=",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You move with absolute silence, your body hugging the shadows. The creature is too engrossed in its meal to notice as you slip past and into the tunnel, escaping back into the subway proper. You've survived, but you failed to get the Data Fragment.",
                  "\n",
                  { "->": "scene_8_the_tower" },
                  { "->": ".^.^.^.20" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  "^A loose rock shifts under your foot. The Skulker's head whips around, its eyeless face emitting a piercing shriek. It lunges. This time, there is no escape.",
                  "\n",
                  { "->": "game_over_death" },
                  { "->": ".^.^.^.20" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "#f": 1 },
          ],
          escape_lair_diversion: [
            "ev",
            1,
            6,
            "rnd",
            "/ev",
            { "temp=": "roll" },
            "\n",
            "ev",
            { "VAR?": "intelligence" },
            { "VAR?": "roll" },
            "+",
            "/ev",
            { "temp=": "total_skill" },
            "ev",
            { "VAR?": "total_skill" },
            11,
            ">=",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You spot a loose chunk of metal propped precariously above a far corner of the nest. You grab a small stone and toss it with perfect aim. The metal sheet clatters to the ground, and the Skulker screeches, darting towards the sound. The distraction gives you the precious seconds you need to scramble into the exit tunnel.",
                  "\n",
                  { "->": "scene_8_the_tower" },
                  { "->": ".^.^.^.20" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  "^Your throw is off. The stone clinks uselessly against the wall, only serving to draw the creature's immediate attention directly to you. It shrieks and attacks.",
                  "\n",
                  { "->": "game_over_death" },
                  { "->": ".^.^.^.20" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      scene_7_scavenge: [
        [
          "^Knowing you'll need more than just wits to survive, you decide to scavenge the immediate area before moving on.",
          "\n",
          "ev",
          { "CNT?": "scene_6a_rooftops" },
          0,
          ">",
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "^The wreckage of the comms tower is a tangled mess of metal supports and snapped cables.",
                "\n",
                { "->": ".^.^.^.8" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "ev",
          { "CNT?": "scene_6b_subway" },
          0,
          ">",
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "^The derelict subway cars and dark maintenance tunnels are full of old, decaying equipment.",
                "\n",
                { "->": ".^.^.^.16" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "ev",
          "str",
          "^Search for useful components.",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          "ev",
          "str",
          "^Ignore scavenging and move on to crafting.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 20 },
          {
            "c-0": ["\n", { "->": ".^.^.^.scavenge_action" }, { "#f": 5 }],
            "c-1": ["\n", { "->": "scene_7a_gearing_up" }, { "#f": 5 }],
          },
        ],
        {
          scavenge_action: [
            "^You spend some time carefully picking through the debris.",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Kaelen",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^Your strength allows you to pry open a sealed maintenance hatch that others couldn't budge. Inside, you find a sturdy metal pipe and a spool of thick wiring, perfect for a weapon.",
                  "\n",
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "has_metal_pipe", re: true },
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "has_thick_wiring", re: true },
                  { "->": ".^.^.^.10" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Lena",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^Your sharp eyes spot a high-tensile cable still intact amidst the chaos, and you deftly cut it free. Nearby, a shattered wall panel contains a sheet of flexible polymer, exactly what you need for a bow limb.",
                  "\n",
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "has_flexible_polymer", re: true },
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "has_tensile_cable", re: true },
                  { "->": ".^.^.^.20" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Aris",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^Your trained eyes spot useful components that others would dismiss as junk. You salvage some intact copper wires and a magnetic coil from a ruined transformer.",
                  "\n",
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "has_copper_wiring", re: true },
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "has_magnetic_coil", re: true },
                  "ev",
                  { "VAR?": "power_cell_stack" },
                  0,
                  "==",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        "^Tucked away inside the transformer's casing is another Degraded Power Cell, apparently undamaged by the overload. A lucky find.",
                        "\n",
                        "ev",
                        { "VAR?": "power_cell_stack" },
                        1,
                        "+",
                        { "VAR=": "power_cell_stack", re: true },
                        "/ev",
                        { "->": ".^.^.^.17" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  { "->": ".^.^.^.30" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "^You've gathered what you can.",
            "\n",
            { "->": "scene_7a_gearing_up" },
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      scene_7a_gearing_up: [
        [
          "^Before you proceed, you find a relatively sheltered alcove in the ruins to catch your breath and take stock of what you have. This is a good opportunity to prepare for what's ahead.",
          "\n",
          "ev",
          "str",
          "^Use your resources to craft something useful.",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 4 },
          "ev",
          "str",
          "^Scour the area for more resources.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 4 },
          "ev",
          "str",
          "^Save your resources and move on.",
          "/str",
          { "VAR?": "has_reinforced_club" },
          "!",
          { "VAR?": "has_recurve_bow" },
          "!",
          "&&",
          { "VAR?": "has_emp_grenade" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-2", flg: 21 },
          "ev",
          "str",
          "^You are prepared. Move on.",
          "/str",
          { "VAR?": "has_reinforced_club" },
          { "VAR?": "has_recurve_bow" },
          "||",
          { "VAR?": "has_emp_grenade" },
          "||",
          { "VAR?": "has_moss_poison_vial" },
          "||",
          { "VAR?": "poison_bomb_stack" },
          0,
          ">",
          "||",
          "/ev",
          { "*": ".^.c-3", flg: 5 },
          "ev",
          "str",
          "^Heal Up",
          "/str",
          { "VAR?": "glimmer_moss_stack" },
          0,
          ">",
          "/ev",
          { "*": ".^.c-4", flg: 5 },
          "ev",
          "str",
          "^Check Status",
          "/str",
          "/ev",
          { "*": ".^.c-5", flg: 4 },
          {
            "c-0": [
              "\n",
              "ev",
              { "^->": "scene_7a_gearing_up" },
              "/ev",
              { "->": "crafting_options" },
              { "#f": 5 },
            ],
            "c-1": ["\n", { "->": "scene_7b_grinding" }, { "#f": 5 }],
            "c-2": ["\n", { "->": "scene_8_the_tower" }, { "#f": 5 }],
            "c-3": ["\n", { "->": "scene_8_the_tower" }, { "#f": 5 }],
            "c-4": [
              "\n",
              "ev",
              false,
              "/ev",
              { "->t->": "use_glimmer_moss_tunnel" },
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-5": [
              "\n",
              "ev",
              { "^->": "scene_7a_gearing_up" },
              "/ev",
              { "->": "check_status" },
              { "#f": 5 },
            ],
          },
        ],
        { "#f": 3 },
      ],
      scene_7b_grinding: [
        [
          "^You scan the immediate vicinity for any useful materials you may have missed.",
          "\n",
          "ev",
          "str",
          "^Search the damp crevices for Glimmer Moss.",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 4 },
          "ev",
          "str",
          "^Sift through the wreckage for Power Cells.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 4 },
          "ev",
          "str",
          "^Stop searching and return.",
          "/str",
          "/ev",
          { "*": ".^.c-2", flg: 4 },
          {
            "c-0": ["\n", { "->": ".^.^.^.grind_moss" }, { "#f": 5 }],
            "c-1": ["\n", { "->": ".^.^.^.grind_power_cells" }, { "#f": 5 }],
            "c-2": ["\n", { "->": "scene_7a_gearing_up" }, { "#f": 5 }],
          },
        ],
        {
          grind_moss: [
            "ev",
            1,
            3,
            "rnd",
            "/ev",
            { "temp=": "roll" },
            "\n",
            "ev",
            { "VAR?": "roll" },
            2,
            "<=",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You find another small patch of the glowing fungus. You add it to your collection.",
                  "\n",
                  "ev",
                  { "VAR?": "glimmer_moss_stack" },
                  1,
                  "+",
                  { "VAR=": "glimmer_moss_stack", re: true },
                  "/ev",
                  { "->": ".^.^.^.14" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  "^You search for a while but find nothing. This area seems picked clean for now.",
                  "\n",
                  { "->": ".^.^.^.14" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "->": ".^.^" },
            { "#f": 1 },
          ],
          grind_power_cells: [
            "ev",
            1,
            4,
            "rnd",
            "/ev",
            { "temp=": "roll" },
            "\n",
            "ev",
            { "VAR?": "roll" },
            1,
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^Your persistence pays off. Wedged inside a damaged console, you find another Degraded Power Cell.",
                  "\n",
                  "ev",
                  { "VAR?": "power_cell_stack" },
                  1,
                  "+",
                  { "VAR=": "power_cell_stack", re: true },
                  "/ev",
                  { "->": ".^.^.^.14" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  "^You find plenty of scrap, but no functional power sources.",
                  "\n",
                  { "->": ".^.^.^.14" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "->": ".^.^" },
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      tower_floor_2: [
        '^The second floor is a cramped server room. A hulking figure, the "Brute," blocks the only doorway, cracking their knuckles. "Tower\'s mine," they grunt. "Get lost or get broken." This will be a fight.',
        "\n",
        { "->": "setup_brute_battle" },
        { "#f": 3 },
      ],
      tower_floor_3: [
        [
          '^After defeating the Brute, you ascend to the third floor, a darkened observation deck. You don\'t see anyone, but your instincts scream "trap."',
          "\n",
          '^<i>PROCTOR: "First subject to bypass the obstacle demonstrates superior situational awareness."</i>',
          "\n",
          "ev",
          "str",
          "^Scan for the trap - Perception/Intelligence Check",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          {
            "c-0": [
              "\n",
              "ev",
              1,
              6,
              "rnd",
              "/ev",
              { "temp=": "roll" },
              "\n",
              "ev",
              { "VAR?": "perception" },
              { "VAR?": "intelligence" },
              "+",
              { "VAR?": "roll" },
              "+",
              "/ev",
              { "temp=": "total_skill" },
              "ev",
              { "VAR?": "total_skill" },
              18,
              ">=",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    { "VAR?": "character_name" },
                    "str",
                    "^Aris",
                    "/str",
                    "==",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "^ Your AI implant flags a hidden pressure plate wired to a jury-rigged arc welder. A nasty trap. ",
                          { "->": ".^.^.^.10" },
                          null,
                        ],
                      },
                    ],
                    [
                      { "->": ".^.b" },
                      {
                        b: [
                          "^ Your eyes catch the faint shimmer of a tripwire connected to a weighted net. Amateurish, but deadly.",
                          { "->": ".^.^.^.10" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    '^ You easily disable it. A shadowy figure, the "Ghost," curses from the far side of the room and flees upwards.',
                    "\n",
                    { "->": ".^.^.^.23" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "^You step forward and a trap springs! A heavy cargo net drops from the ceiling, entangling you. It takes you precious moments to cut yourself free. You've lost time, and the Ghost is long gone.",
                    "\n",
                    "ev",
                    true,
                    "/ev",
                    { "VAR=": "is_fatigued", re: true },
                    { "->": ".^.^.^.23" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              { "^->": "tower_floor_3.tower_floor_4" },
              "/ev",
              { "->": "tower_buffer_room" },
              { "#f": 5 },
            ],
          },
        ],
        {
          tower_floor_4: [
            '^The fourth floor is a chaotic workshop, filled with sparking contraptions. A contestant in a customized tech-suit, the "Tinkerer," is working on a device. "Ah, a new test subject!" they exclaim, leveling a whirring gadget at you.',
            "\n",
            { "->": "setup_tinkerer_battle" },
            { "#f": 3 },
          ],
          "#f": 3,
        },
      ],
      tower_floor_5: [
        '^You reach the final floor before the spire\'s peak. A contestant with cold, professional eyes, the "Veteran," stands waiting for you. They hold a sharpened piece of rebar like a short sword. "Only one of us gets that fragment," they say calmly. "Let\'s make this quick."',
        "\n",
        { "->": "setup_veteran_battle" },
        { "#f": 3 },
      ],
      tower_top: [
        "^Panting and bruised, you finally reach the open-air platform at the peak of the tower. The emergency light flashes on the console where the **second Data Fragment** is waiting. You take it.",
        "\n",
        "ev",
        { "VAR?": "data_fragments" },
        1,
        "+",
        { "VAR=": "data_fragments", re: true },
        "/ev",
        '^<i>AI: "Second Data Fragment acquired. Subject has demonstrated superior combat and traversal capabilities. Proceeding to next objective."</i>',
        "\n",
        { "->": "descend_from_tower" },
        { "#f": 1 },
      ],
      descend_from_tower: [
        "^You stand at the apex of the spire, the wind whipping at your clothes. For a moment, you can see the shattered world in its entirety—a graveyard of floating islands hanging in the green-tinged void. The climb was brutal, but the way down looks easier. You spot what looks like a functioning service lift on a lower level.",
        "\n",
        "^The descent is quiet and jarringly peaceful after the chaos of the climb. The lift carries you down into the heart of the ruined hab-block district that surrounds the tower's base. These crumbling residential buildings are a maze of collapsed corridors and dark, empty homes.",
        "\n",
        '^Your AI pings a location nearby. <i>AI: "Life sign detected. Biorhythms indicate stress and injury, but subject is not hostile."</i>',
        "\n",
        "^Following the signal, you navigate the treacherous streets until you come across a small, fortified room in one of the less damaged hab-blocks. The door is slightly ajar, and you can see a dim light from within. It seems recently occupied.",
        "\n",
        { "->": "scene_9_the_bargain" },
        { "#f": 1 },
      ],
      tower_buffer_room: [
        { "temp=": "next_floor" },
        [
          "^You find a small, cramped access corridor between the floors. The sounds of combat have faded, giving you a brief moment to recover before ascending further.",
          "\n",
          "ev",
          "str",
          "^Shake off the fatigue.",
          "/str",
          { "VAR?": "is_fatigued" },
          "/ev",
          { "*": ".^.c-0", flg: 21 },
          "ev",
          "str",
          "^Use Glimmer Moss (",
          "ev",
          { "VAR?": "glimmer_moss_stack" },
          "out",
          "/ev",
          "^ left).",
          "/str",
          { "VAR?": "glimmer_moss_stack" },
          0,
          ">",
          { "VAR?": "is_injured" },
          "&&",
          "/ev",
          { "*": ".^.c-1", flg: 5 },
          "ev",
          "str",
          "^Proceed to the next floor.",
          "/str",
          "/ev",
          { "*": ".^.c-2", flg: 4 },
          "ev",
          "str",
          "^Check Status",
          "/str",
          "/ev",
          { "*": ".^.c-3", flg: 4 },
          {
            "c-0": [
              "\n",
              "^You stretch your aching muscles and take a long drink of water. The weariness recedes, and you feel ready to continue.",
              "\n",
              "ev",
              false,
              "/ev",
              { "VAR=": "is_fatigued", re: true },
              "ev",
              { "VAR?": "next_floor" },
              "/ev",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-1": [
              "\n",
              "ev",
              true,
              "/ev",
              { "->t->": "use_glimmer_moss_tunnel" },
              "ev",
              { "VAR?": "next_floor" },
              "/ev",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-2": [
              "\n",
              "^Steeling yourself, you continue your ascent.",
              "\n",
              { "->": "next_floor", var: true },
              { "#f": 5 },
            ],
            "c-3": [
              "\n",
              "ev",
              { "^->": "tower_buffer_room" },
              "/ev",
              { "->": "check_status" },
              { "#f": 5 },
            ],
          },
        ],
        { "#f": 3 },
      ],
      setup_brute_battle: [
        "ev",
        "str",
        "^The Brute",
        "/str",
        "/ev",
        { "VAR=": "current_enemy_name", re: true },
        "ev",
        30,
        "/ev",
        { "VAR=": "current_enemy_hp", re: true },
        "ev",
        8,
        "/ev",
        { "VAR=": "current_enemy_atk", re: true },
        "ev",
        4,
        "/ev",
        { "VAR=": "current_enemy_def", re: true },
        "ev",
        0,
        "/ev",
        { "VAR=": "enemy2_hp", re: true },
        { "->": "battle_loop" },
        { "#f": 1 },
      ],
      setup_tinkerer_battle: [
        "ev",
        "str",
        "^The Tinkerer",
        "/str",
        "/ev",
        { "VAR=": "current_enemy_name", re: true },
        "ev",
        20,
        "/ev",
        { "VAR=": "current_enemy_hp", re: true },
        "ev",
        8,
        "/ev",
        { "VAR=": "current_enemy_atk", re: true },
        "ev",
        2,
        "/ev",
        { "VAR=": "current_enemy_def", re: true },
        "ev",
        0,
        "/ev",
        { "VAR=": "enemy2_hp", re: true },
        { "->": "battle_loop" },
        { "#f": 1 },
      ],
      setup_veteran_battle: [
        "ev",
        "str",
        "^The Veteran",
        "/str",
        "/ev",
        { "VAR=": "current_enemy_name", re: true },
        "ev",
        25,
        "/ev",
        { "VAR=": "current_enemy_hp", re: true },
        "ev",
        7,
        "/ev",
        { "VAR=": "current_enemy_atk", re: true },
        "ev",
        3,
        "/ev",
        { "VAR=": "current_enemy_def", re: true },
        "ev",
        0,
        "/ev",
        { "VAR=": "enemy2_hp", re: true },
        { "->": "battle_loop" },
        { "#f": 1 },
      ],
      find_jed_for_fragment: [
        [
          "^Inside, you find an older, weathered man slumped against a wall with a makeshift splint on his leg. He has a Data Fragment clutched in his hand, but his face is pale with pain. This must be Jed.",
          "\n",
          '^He looks up at you, not with fear, but with weary resignation. "Well, look what the cat dragged in," he rasps. "Don\'t suppose you\'ve got a working power cell on you? This old diagnostic tool is dead, and I need to see how bad this leg is." He gestures to the fragment. "It\'s yours if you can help."',
          "\n",
          "ev",
          "str",
          "^Trade a Power Cell for the fragment.",
          "/str",
          { "VAR?": "power_cell_stack" },
          0,
          ">",
          "/ev",
          { "*": ".^.c-0", flg: 21 },
          "ev",
          "str",
          "^Intimidate him for the fragment - Strength Check",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 20 },
          "ev",
          "str",
          "^Leave him.",
          "/str",
          "/ev",
          { "*": ".^.c-2", flg: 20 },
          {
            "c-0": [
              "\n",
              "^You nod, pulling a Degraded Power Cell from your pack. Jed's eyes light up with relief. \"Thank you, friend. A deal's a deal.\" He tosses you the fragment.",
              "\n",
              "ev",
              { "VAR?": "power_cell_stack" },
              1,
              "-",
              { "VAR=": "power_cell_stack", re: true },
              "/ev",
              "ev",
              { "VAR?": "data_fragments" },
              1,
              "+",
              { "VAR=": "data_fragments", re: true },
              "/ev",
              "ev",
              "str",
              "^HELPED",
              "/str",
              "/ev",
              { "VAR=": "jed_status", re: true },
              { "->": "check_for_jed_gift" },
              { "#f": 5 },
            ],
            "c-1": ["\n", { "->": ".^.^.^.intimidate_jed" }, { "#f": 5 }],
            "c-2": [
              "\n",
              "^You decide not to get involved and back out of the room, leaving the old man to his fate. The fragment remains out of reach.",
              "\n",
              { "->": "check_for_jed_gift" },
              { "#f": 5 },
            ],
          },
        ],
        {
          intimidate_jed: [
            "ev",
            1,
            6,
            "rnd",
            "/ev",
            { "temp=": "roll" },
            "\n",
            "ev",
            { "VAR?": "strength" },
            { "VAR?": "roll" },
            "+",
            "/ev",
            { "temp=": "total_skill" },
            "ev",
            { "VAR?": "total_skill" },
            13,
            ">=",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  '^"I don\'t have time for this. Give me the fragment," you growl. Jed sizes you up, then scoffs and throws the fragment at your feet. "Fine. Take it. You\'re just like the others."',
                  "\n",
                  "ev",
                  { "VAR?": "data_fragments" },
                  1,
                  "+",
                  { "VAR=": "data_fragments", re: true },
                  "/ev",
                  "ev",
                  "str",
                  "^HOSTILE",
                  "/str",
                  "/ev",
                  { "VAR=": "jed_status", re: true },
                  { "->": "scene_9c_final_preparations" },
                  { "->": ".^.^.^.20" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  '^You try to look threatening, but Jed just gives you a tired smile. "Save your breath, kid. I\'ve seen scarier things in a mirror. No cell, no fragment."',
                  "\n",
                  "ev",
                  "str",
                  "^Attack him.",
                  "/str",
                  "/ev",
                  { "*": ".^.c-0", flg: 20 },
                  "ev",
                  "str",
                  "^Back down.",
                  "/str",
                  "/ev",
                  { "*": ".^.c-1", flg: 20 },
                  { "->": ".^.^.^.20" },
                  {
                    "c-0": [
                      "\n",
                      { "->": ".^.^.^.^.^.attack_jed" },
                      { "#f": 5 },
                    ],
                    "c-1": [
                      "\n",
                      { "->": "scene_9c_final_preparations" },
                      { "#f": 5 },
                    ],
                  },
                ],
              },
            ],
            "nop",
            "\n",
            { "#f": 1 },
          ],
          attack_jed: [
            "^You lunge at the old man, deciding to take the fragment by force. Jed isn't surprised. With a weary sigh, he pulls a hidden trigger on the floor.",
            "\n",
            "ev",
            1,
            10,
            "rnd",
            "/ev",
            { "temp=": "trap_roll" },
            "\n",
            "ev",
            { "VAR?": "trap_roll" },
            1,
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You see the trap mechanism a split second before it springs—a powerful stun baton swinging out from a wall panel. You manage to dodge it. Jed looks genuinely surprised. You snatch the fragment from his hand before he can react further.",
                  "\n",
                  "ev",
                  { "VAR?": "data_fragments" },
                  1,
                  "+",
                  { "VAR=": "data_fragments", re: true },
                  "/ev",
                  "ev",
                  "str",
                  "^DEAD",
                  "/str",
                  "/ev",
                  { "VAR=": "jed_status", re: true },
                  { "->": "scene_9c_final_preparations" },
                  { "->": ".^.^.^.16" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  "^A powerful stun baton swings out from a hidden wall panel, slamming into your side with a crackle of electricity. The shock is immense, leaving you gasping on the floor as your vision whites out.",
                  "\n",
                  "ev",
                  { "VAR?": "hp" },
                  15,
                  "-",
                  { "VAR=": "hp", re: true },
                  "/ev",
                  "ev",
                  { "VAR?": "hp" },
                  0,
                  "<=",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        { "->": "game_over_death" },
                        { "->": ".^.^.^.16" },
                        null,
                      ],
                    },
                  ],
                  [
                    { "->": ".^.b" },
                    {
                      b: [
                        "\n",
                        "ev",
                        true,
                        "/ev",
                        { "VAR=": "is_injured", re: true },
                        "ev",
                        "str",
                        "^HOSTILE",
                        "/str",
                        "/ev",
                        { "VAR=": "jed_status", re: true },
                        '^Jed shakes his head. "Told you. Scarier things in a mirror." He waits for you to recover before pointing to the door. "Now get out."',
                        "\n",
                        "^You drag yourself out of the room, empty-handed and in agony.",
                        "\n",
                        { "->": "scene_9c_final_preparations" },
                        { "->": ".^.^.^.16" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  { "->": ".^.^.^.16" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      meet_jed_friendly: [
        [
          '^Inside, you find an older, weathered man slumped against a wall with a makeshift splint on his leg. He eyes you warily as you enter. "Just passin\' through," he says. "Don\'t mind me." This is your first encounter with another non-hostile contestant.',
          "\n",
          "ev",
          "str",
          "^Offer him a Glimmer Moss sample for his leg.",
          "/str",
          { "VAR?": "glimmer_moss_stack" },
          0,
          ">",
          "/ev",
          { "*": ".^.c-0", flg: 21 },
          "ev",
          "str",
          "^Leave him be.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 20 },
          {
            "c-0": [
              "\n",
              '^You offer him one of your moss samples. He looks surprised, then accepts it with a grateful nod. "Well, I\'ll be. Thanks. Here, take this in return."',
              "\n",
              "ev",
              { "VAR?": "glimmer_moss_stack" },
              1,
              "-",
              { "VAR=": "glimmer_moss_stack", re: true },
              "/ev",
              "ev",
              "str",
              "^HELPED",
              "/str",
              "/ev",
              { "VAR=": "jed_status", re: true },
              "ev",
              { "VAR?": "character_name" },
              "str",
              "^Kaelen",
              "/str",
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^He hands you a roll of thick, resin-soaked wrappings. \"Saw you eyein' that pipe you carry. This'll make the grip better, add some weight to your swing. Should make your club a proper weapon.\"",
                    "\n",
                    "ev",
                    true,
                    "/ev",
                    { "VAR=": "has_club_upgrade_kit", re: true },
                    { "->": ".^.^.^.23" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              { "VAR?": "character_name" },
              "str",
              "^Aris",
              "/str",
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^He points to a common-looking weed growing in a crack in the floor. \"See that? Most folks think it's useless. But if you crush the leaves and mix 'em with a bit of moss, makes a real fine poultice. Calms the nerves.\" You've learned the recipe for a Calming Poultice.",
                    "\n",
                    "ev",
                    true,
                    "/ev",
                    { "VAR=": "knows_calming_poultice_recipe", re: true },
                    { "->": ".^.^.^.33" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              { "VAR?": "character_name" },
              "str",
              "^Lena",
              "/str",
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    '^He reaches into his pack and pulls out a small, tightly wrapped bundle. "You look like you know how to stay quiet. These might help." He hands you a bundle of five arrows, fletched with a dark, soft feather that makes them almost silent in flight.',
                    "\n",
                    "ev",
                    { "VAR?": "silent_arrow_count" },
                    5,
                    "+",
                    { "VAR=": "silent_arrow_count", re: true },
                    "/ev",
                    { "->": ".^.^.^.43" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": "check_for_jed_gift" },
              { "#f": 5 },
            ],
            "c-1": [
              "\n",
              "^You give a nod and back out of the room, leaving him in peace.",
              "\n",
              { "->": "scene_9c_final_preparations" },
              { "#f": 5 },
            ],
          },
        ],
        { "#f": 1 },
      ],
      check_for_jed_gift: [
        "ev",
        { "VAR?": "jed_status" },
        "str",
        "^HELPED",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: ["\n", { "->": "jed_parting_gift" }, { "->": ".^.^.^.9" }, null],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              { "->": "scene_9c_final_preparations" },
              { "->": ".^.^.^.9" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      jed_parting_gift: [
        '^As you head for the collapsed doorway, a voice calls out from the shadows. "Hold on a minute, friend." Jed limps towards you, leaning on a makeshift crutch.',
        "\n",
        '^"You did me a real kindness back there," he says, his eyes grateful. "Most folks in this place would\'ve just put a knife in my back. The Arena... it changes people."',
        "\n",
        "^He holds out a slightly damaged datapad. \"I've been here a while. Found a few of these logs and cracked 'em. Nothing that'll tell you how to win, but... it helps to know what you're up against. You should have them.\"",
        "\n",
        "^You take the datapad. He's unlocked several entries for you.",
        "\n",
        "^<b>UNLOCKED LOG 14-G:</b> \"Test Site Echo-7's primary function is to observe species' adaptation to catastrophic environmental collapse. The Shattered World provides an ideal crucible. Survival is the only metric.\"",
        "\n",
        "ev",
        true,
        "/ev",
        { "VAR=": "log_knows_site_purpose", re: true },
        "^<b>UNLOCKED LOG 29-A:</b> \"Note on planetary destruction: Previous experiment in terraforming using resonant frequencies resulted in cascading geological failure. The Archivists deemed the outcome 'sub-optimal but informative'.\"",
        "\n",
        "ev",
        true,
        "/ev",
        { "VAR=": "log_knows_shattered_world_cause", re: true },
        '^<b>UNLOCKED LOG 55-C:</b> "The Proctor AI administrates the trials with perfect efficiency. It is unconstrained by ethics or empathy, operating solely on the parameters of the experiment. Do not attempt to reason with it."',
        "\n",
        "ev",
        true,
        "/ev",
        { "VAR=": "log_knows_proctor_ethics", re: true },
        '^Jed gives you a final nod. "Good luck out there. Try not to die." He turns and limps back into the shadows of the maintenance bay.',
        "\n",
        { "->": "scene_9c_final_preparations" },
        { "#f": 1 },
      ],
      scene_9c_final_preparations: [
        [
          "^The AI indicates that the final Data Fragment is located in a heavily defended nest nearby. This is likely the last major confrontation in this zone. You find a defensible spot to make your final preparations before the assault.",
          "\n",
          "ev",
          "str",
          "^Manage Equipment and Crafting.",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 4 },
          "ev",
          "str",
          "^Scavenge the area one last time.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 4 },
          "ev",
          "str",
          "^Check Status.",
          "/str",
          "/ev",
          { "*": ".^.c-2", flg: 4 },
          "ev",
          "str",
          "^Use the Emitter to clear the blocked passage.",
          "/str",
          { "VAR?": "has_kinetic_emitter" },
          { "VAR?": "emitter_charges" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-3", flg: 5 },
          "ev",
          "str",
          "^Heal Up",
          "/str",
          { "VAR?": "glimmer_moss_stack" },
          0,
          ">",
          { "VAR?": "is_injured" },
          "&&",
          { "VAR?": "is_fatigued" },
          "||",
          "/ev",
          { "*": ".^.c-4", flg: 5 },
          "ev",
          "str",
          "^Proceed to the final location.",
          "/str",
          "/ev",
          { "*": ".^.c-5", flg: 4 },
          {
            "c-0": ["\n", { "->": ".^.^.^.manage_equipment" }, { "#f": 5 }],
            "c-1": ["\n", { "->": "final_scavenge" }, { "#f": 5 }],
            "c-2": [
              "\n",
              "ev",
              { "^->": "scene_9c_final_preparations" },
              "/ev",
              { "->": "check_status" },
              { "#f": 5 },
            ],
            "c-3": [
              "\n",
              { "->": ".^.^.^.clear_passage_with_emitter" },
              { "#f": 5 },
            ],
            "c-4": [
              "\n",
              "ev",
              false,
              "/ev",
              { "->t->": "use_glimmer_moss_tunnel" },
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-5": ["\n", { "->": "scene_10_the_lair" }, { "#f": 5 }],
          },
        ],
        {
          clear_passage_with_emitter: [
            [
              "^You notice a side passage that seems to lead towards the lair, but it's completely blocked by a massive pile of collapsed girders.",
              "\n",
              "ev",
              "str",
              "^Use an Emitter charge to clear it.",
              "/str",
              "/ev",
              { "*": ".^.c-0", flg: 4 },
              "ev",
              "str",
              "^Save your charges.",
              "/str",
              "/ev",
              { "*": ".^.c-1", flg: 4 },
              {
                "c-0": [
                  "\n",
                  "ev",
                  { "f()": "use_emitter_charge" },
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        "^You unleash a focused blast of kinetic energy. The girders groan and shift, pushed aside by the immense force, revealing a hidden path. Beyond the rubble, you find a small supply cache left by a previous contestant.",
                        "\n",
                        "ev",
                        { "VAR?": "glimmer_moss_stack" },
                        2,
                        "+",
                        { "VAR=": "glimmer_moss_stack", re: true },
                        "/ev",
                        "ev",
                        { "VAR?": "power_cell_stack" },
                        1,
                        "+",
                        { "VAR=": "power_cell_stack", re: true },
                        "/ev",
                        "^You've found some extra supplies! The path is now clear.",
                        "\n",
                        { "->": ".^.^.^.5" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  { "->": ".^.^.^.^" },
                  { "#f": 5 },
                ],
                "c-1": ["\n", { "->": ".^.^.^.^" }, { "#f": 5 }],
              },
            ],
            { "#f": 1 },
          ],
          manage_equipment: [
            [
              "^You lay out your gear, checking everything is in order.",
              "\n",
              "ev",
              "str",
              "^Use a Power Cell to recharge the Emitter.",
              "/str",
              { "VAR?": "has_kinetic_emitter" },
              { "VAR?": "emitter_charges" },
              0,
              "<=",
              "&&",
              { "VAR?": "power_cell_stack" },
              0,
              ">",
              "&&",
              "/ev",
              { "*": ".^.c-0", flg: 5 },
              "ev",
              "str",
              "^Use the Adrenaline Shot.",
              "/str",
              { "VAR?": "has_adrenaline_shot" },
              "/ev",
              { "*": ".^.c-1", flg: 21 },
              "ev",
              "str",
              "^Apply the Upgrade Kit to the Club.",
              "/str",
              { "VAR?": "has_reinforced_club" },
              { "VAR?": "has_club_upgrade_kit" },
              "&&",
              { "VAR?": "club_is_upgraded" },
              "!",
              "&&",
              "/ev",
              { "*": ".^.c-2", flg: 21 },
              "ev",
              "str",
              "^Craft Titan-Beetle Armor.",
              "/str",
              { "VAR?": "has_titan_beetle_carapace" },
              { "VAR?": "character_name" },
              "str",
              "^Kaelen",
              "/str",
              "==",
              "&&",
              "/ev",
              { "*": ".^.c-3", flg: 21 },
              "ev",
              "str",
              "^View Crafting Options.",
              "/str",
              "/ev",
              { "*": ".^.c-4", flg: 4 },
              "ev",
              "str",
              "^Return.",
              "/str",
              "/ev",
              { "*": ".^.c-5", flg: 4 },
              {
                "c-0": [
                  "\n",
                  "^You connect the unstable Power Cell to the Emitter's charging port. The cell sputters and whines as its energy is drained, but the Emitter's internal lights hum back to life. It's fully charged.",
                  "\n",
                  "ev",
                  { "VAR?": "power_cell_stack" },
                  1,
                  "-",
                  { "VAR=": "power_cell_stack", re: true },
                  "/ev",
                  "ev",
                  3,
                  "/ev",
                  { "VAR=": "emitter_charges", re: true },
                  { "->": ".^.^.^" },
                  { "#f": 5 },
                ],
                "c-1": [
                  "\n",
                  "^You inject the adrenaline directly into your thigh. Your heart hammers in your chest, and your senses sharpen to a razor's edge. You feel faster, more agile.",
                  "\n",
                  "ev",
                  false,
                  "/ev",
                  { "VAR=": "has_adrenaline_shot", re: true },
                  "ev",
                  { "VAR?": "agility" },
                  1,
                  "+",
                  { "VAR=": "agility", re: true },
                  "/ev",
                  "ev",
                  { "f()": "update_combat_stats" },
                  "pop",
                  "/ev",
                  "\n",
                  '^<i>AI: "Unscheduled bio-stimulant administered. Agility permanently increased to ',
                  "ev",
                  { "VAR?": "agility" },
                  "out",
                  "/ev",
                  '^."</i>',
                  "\n",
                  { "->": ".^.^.^" },
                  { "#f": 5 },
                ],
                "c-2": [
                  "\n",
                  "^You take the resin-soaked wrappings Jed gave you and apply them to your club. The grip is more secure, and the added weight feels powerful. Your club has been upgraded!",
                  "\n",
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "club_is_upgraded", re: true },
                  "ev",
                  3,
                  "/ev",
                  { "VAR=": "club_power_slots", re: true },
                  "ev",
                  false,
                  "/ev",
                  { "VAR=": "has_club_upgrade_kit", re: true },
                  "ev",
                  { "f()": "update_combat_stats" },
                  "pop",
                  "/ev",
                  "\n",
                  { "->": ".^.^.^" },
                  { "#f": 5 },
                ],
                "c-3": [
                  "\n",
                  "^You use the heavy carapace and some scavenged straps to create a crude but effective breastplate. It's heavy, but it will offer significant protection.",
                  "\n",
                  "ev",
                  false,
                  "/ev",
                  { "VAR=": "has_titan_beetle_carapace", re: true },
                  { "->": ".^.^.^" },
                  { "#f": 5 },
                ],
                "c-4": [
                  "\n",
                  "ev",
                  { "^->": "scene_9c_final_preparations.manage_equipment" },
                  "/ev",
                  { "->": "crafting_options" },
                  { "#f": 5 },
                ],
                "c-5": ["\n", { "->": ".^.^.^.^" }, { "#f": 5 }],
              },
            ],
            { "#f": 3 },
          ],
          "#f": 3,
        },
      ],
      final_scavenge: [
        [
          "^You scour the nearby ruins for any last-minute supplies.",
          "\n",
          "ev",
          "str",
          "^Search for Glimmer Moss.",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 4 },
          "ev",
          "str",
          "^Search for Power Cells.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 4 },
          "ev",
          "str",
          "^Scavenge a collapsed hab-unit.",
          "/str",
          "/ev",
          { "*": ".^.c-2", flg: 4 },
          "ev",
          "str",
          "^Scavenge for materials to fletch arrows.",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Lena",
          "/str",
          "==",
          { "VAR?": "has_recurve_bow" },
          "&&",
          "/ev",
          { "*": ".^.c-3", flg: 5 },
          "ev",
          "str",
          "^Stop scavenging.",
          "/str",
          "/ev",
          { "*": ".^.c-4", flg: 4 },
          {
            "c-0": ["\n", { "->": ".^.^.^.final_scavenge_moss" }, { "#f": 5 }],
            "c-1": ["\n", { "->": ".^.^.^.final_scavenge_cells" }, { "#f": 5 }],
            "c-2": ["\n", { "->": ".^.^.^.scavenge_hab_unit" }, { "#f": 5 }],
            "c-3": ["\n", { "->": ".^.^.^.scavenge_for_arrows" }, { "#f": 5 }],
            "c-4": ["\n", { "->": "scene_9c_final_preparations" }, { "#f": 5 }],
          },
        ],
        {
          scavenge_for_arrows: [
            "^You shift your focus, looking not for bulky items, but for things that are long, straight, and sharp. You find a few thin pieces of rebar for shafts and some sharpened scrap metal for heads. With some tattered fabric for fletching, you can assemble a small quiver's worth of arrows.",
            "\n",
            "ev",
            2,
            4,
            "rnd",
            "/ev",
            { "temp=": "arrows_crafted" },
            "\n",
            "ev",
            { "VAR?": "scrap_arrow_count" },
            { "VAR?": "arrows_crafted" },
            "+",
            { "VAR=": "scrap_arrow_count", re: true },
            "/ev",
            "^You craft ",
            "ev",
            { "VAR?": "arrows_crafted" },
            "out",
            "/ev",
            "^ scrap arrows.",
            "\n",
            { "->": ".^.^" },
            { "#f": 1 },
          ],
          final_scavenge_moss: [
            "ev",
            1,
            3,
            "rnd",
            "/ev",
            { "temp=": "roll" },
            "\n",
            "ev",
            { "VAR?": "roll" },
            2,
            "<=",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You find another small patch of the glowing fungus.",
                  "\n",
                  "ev",
                  { "VAR?": "glimmer_moss_stack" },
                  1,
                  "+",
                  { "VAR=": "glimmer_moss_stack", re: true },
                  "/ev",
                  { "->": ".^.^.^.14" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  "^You find nothing. This area seems picked clean for now.",
                  "\n",
                  { "->": ".^.^.^.14" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "->": ".^.^" },
            { "#f": 1 },
          ],
          final_scavenge_cells: [
            "ev",
            1,
            4,
            "rnd",
            "/ev",
            { "temp=": "roll" },
            "\n",
            "ev",
            { "VAR?": "roll" },
            1,
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^Tucked inside a damaged console, you find another Degraded Power Cell.",
                  "\n",
                  "ev",
                  { "VAR?": "power_cell_stack" },
                  1,
                  "+",
                  { "VAR=": "power_cell_stack", re: true },
                  "/ev",
                  { "->": ".^.^.^.14" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  "^You find plenty of scrap, but no functional power sources.",
                  "\n",
                  { "->": ".^.^.^.14" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "->": ".^.^" },
            { "#f": 1 },
          ],
          scavenge_hab_unit: [
            "^You pry open the door of a collapsed hab-unit, hoping to find something valuable inside.",
            "\n",
            "ev",
            1,
            5,
            "rnd",
            "/ev",
            { "temp=": "roll" },
            "\n",
            [
              "ev",
              { "VAR?": "roll" },
              1,
              "==",
              "/ev",
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^A sharp hiss echoes from the darkness inside. A Slick-Skinned Skulker, disturbed from its nest, lunges at you!",
                  "\n",
                  { "->": ".^.^.^.^.setup_ambush_skulker_battle" },
                  { "->": ".^.^.^.14" },
                  null,
                ],
              },
            ],
            [
              "ev",
              { "VAR?": "roll" },
              2,
              "==",
              "/ev",
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  { "VAR?": "has_titan_beetle_carapace" },
                  "!",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        "^Inside, you find the remains of a massive, armored beetle. Its iridescent carapace is almost entirely intact. This could be fashioned into some sturdy armor.",
                        "\n",
                        "ev",
                        true,
                        "/ev",
                        { "VAR=": "has_titan_beetle_carapace", re: true },
                        "^You've found a Titan-Beetle Carapace.",
                        "\n",
                        { "->": ".^.^.^.7" },
                        null,
                      ],
                    },
                  ],
                  [
                    { "->": ".^.b" },
                    {
                      b: [
                        "\n",
                        "^You find another beetle husk, but this one has been picked clean. Nothing of value remains.",
                        "\n",
                        { "->": ".^.^.^.7" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  { "->": ".^.^.^.^" },
                  { "->": ".^.^.^.14" },
                  null,
                ],
              },
            ],
            [
              "ev",
              { "VAR?": "roll" },
              3,
              "==",
              "/ev",
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  { "VAR?": "has_adrenaline_shot" },
                  "!",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        '^You find a derelict first-aid station. Most of it is looted, but you find one sealed container with a single auto-injector inside, labeled "Adrenaline." This could permanently boost your reaction speed.',
                        "\n",
                        "ev",
                        true,
                        "/ev",
                        { "VAR=": "has_adrenaline_shot", re: true },
                        "^You've found an Adrenaline Shot.",
                        "\n",
                        { "->": ".^.^.^.7" },
                        null,
                      ],
                    },
                  ],
                  [
                    { "->": ".^.b" },
                    {
                      b: [
                        "\n",
                        "^You find a looted first-aid station, but it's completely empty.",
                        "\n",
                        { "->": ".^.^.^.7" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  { "->": ".^.^.^.^" },
                  { "->": ".^.^.^.14" },
                  null,
                ],
              },
            ],
            [
              "ev",
              { "VAR?": "roll" },
              4,
              "==",
              "/ev",
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  { "VAR?": "log_knows_ally_buff" },
                  "!",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        "^You find the skeletal remains of another contestant. Unlike the one in the bay, this one was clearly a strategist. Their datapad is still active, containing a single, encrypted Archivist Log about combat synergy.",
                        "\n",
                        { "->": ".^.^.^.^.^.^.decrypt_ally_buff_log" },
                        { "->": ".^.^.^.7" },
                        null,
                      ],
                    },
                  ],
                  [
                    { "->": ".^.b" },
                    {
                      b: [
                        "\n",
                        "^You find another looted body. Nothing of value remains.",
                        "\n",
                        { "->": ".^.^.^.^.^.^" },
                        { "->": ".^.^.^.7" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  { "->": ".^.^.^.14" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  "^The hab-unit is completely picked clean. There's nothing of value left.",
                  "\n",
                  { "->": ".^.^.^.^" },
                  { "->": ".^.^.^.14" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "#f": 1 },
          ],
          decrypt_ally_buff_log: [
            [
              "^You access the log. It appears to be a tactical analysis of multi-subject combat trials.",
              "\n",
              '^<i>AI: "Log is encrypted. High-level intelligence required."</i>',
              "\n",
              "ev",
              "str",
              "^Attempt to decrypt the log - Intelligence Check",
              "/str",
              "/ev",
              { "*": ".^.c-0", flg: 20 },
              "ev",
              "str",
              "^Leave it.",
              "/str",
              "/ev",
              { "*": ".^.c-1", flg: 20 },
              {
                "c-0": [
                  "\n",
                  "ev",
                  { "VAR?": "intelligence" },
                  8,
                  ">=",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        "^You find a weakness in the encryption and the file opens. It's a log detailing how to coordinate with an ally to exploit an enemy's blind spots. You've learned a new skill: **Targeted Analysis**.",
                        "\n",
                        "ev",
                        true,
                        "/ev",
                        { "VAR=": "log_knows_ally_buff", re: true },
                        "ev",
                        { "VAR?": "player_skills" },
                        { "VAR?": "TargetedAnalysis" },
                        "+",
                        { "VAR=": "player_skills", re: true },
                        "/ev",
                        { "->": ".^.^.^.^.^.^" },
                        { "->": ".^.^.^.8" },
                        null,
                      ],
                    },
                  ],
                  [
                    { "->": ".^.b" },
                    {
                      b: [
                        "\n",
                        "^The tactical jargon and complex data is beyond you. The log remains locked.",
                        "\n",
                        { "->": ".^.^.^.^.^.^" },
                        { "->": ".^.^.^.8" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  { "#f": 5 },
                ],
                "c-1": ["\n", { "->": ".^.^.^.^" }, { "#f": 5 }],
              },
            ],
            { "#f": 1 },
          ],
          setup_ambush_skulker_battle: [
            "ev",
            "str",
            "^Ambush Skulker",
            "/str",
            "/ev",
            { "VAR=": "current_enemy_name", re: true },
            "ev",
            15,
            "/ev",
            { "VAR=": "current_enemy_hp", re: true },
            "ev",
            8,
            "/ev",
            { "VAR=": "current_enemy_atk", re: true },
            "ev",
            2,
            "/ev",
            { "VAR=": "current_enemy_def", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "used_skill_in_battle", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "rival_will_miss_next_turn", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "enemy_is_poisoned", re: true },
            "ev",
            0,
            "/ev",
            { "VAR=": "poison_turns_remaining", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "is_overcharging", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "is_countering", re: true },
            { "->": "battle_loop" },
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      lair_horde_battle: [
        "^The smaller creatures are a chaotic mess, but you see two larger Skulkers coordinating their attacks, blocking the path forward.",
        "\n",
        "ev",
        { "VAR?": "jed_status" },
        "str",
        "^HELPED",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              '^Jed stands back-to-back with you. "Looks like we\'ve got trouble," he says, his voice steady. "I can give you some covering fire to soften them up, or I can give you this. Your call, but make it quick." He holds out a scavenged piece of armor plating.',
              "\n",
              { "->": ".^.^.^.jed_support_choice" },
              { "->": ".^.^.^.11" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "^This is your first major obstacle.",
              "\n",
              "ev",
              false,
              "/ev",
              { "->": "setup_two_skulker_battle" },
              { "->": ".^.^.^.11" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        {
          jed_support_choice: [
            [
              "ev",
              "str",
              "^Give me covering fire.",
              "/str",
              "/ev",
              { "*": ".^.c-0", flg: 20 },
              "ev",
              "str",
              "^I'll take the armor.",
              "/str",
              "/ev",
              { "*": ".^.c-1", flg: 20 },
              {
                "c-0": [
                  "\n",
                  '^"Right," Jed nods. "Stay low." He raises his weapon and lays down a burst of suppressing fire, forcing one of the Skulkers to duck behind cover. It will be easier to hit.',
                  "\n",
                  "ev",
                  true,
                  "/ev",
                  { "->": "setup_two_skulker_battle" },
                  { "#f": 5 },
                ],
                "c-1": [
                  "\n",
                  '^"Good choice," Jed says, handing you the heavy plating. "It\'s not pretty, but it\'ll stop a claw or two." You quickly strap it on. Your Defense has been temporarily boosted.',
                  "\n",
                  "ev",
                  { "VAR?": "def" },
                  2,
                  "+",
                  { "VAR=": "def", re: true },
                  "/ev",
                  "ev",
                  false,
                  "/ev",
                  { "->": "setup_two_skulker_battle" },
                  { "#f": 5 },
                ],
              },
            ],
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      setup_skulker_guard_battle: [
        "ev",
        "str",
        "^Skulker Guard",
        "/str",
        "/ev",
        { "VAR=": "current_enemy_name", re: true },
        "ev",
        40,
        "/ev",
        { "VAR=": "current_enemy_hp", re: true },
        "ev",
        9,
        "/ev",
        { "VAR=": "current_enemy_atk", re: true },
        "ev",
        3,
        "/ev",
        { "VAR=": "current_enemy_def", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "used_skill_in_battle", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "rival_will_miss_next_turn", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "enemy_is_poisoned", re: true },
        "ev",
        0,
        "/ev",
        { "VAR=": "poison_turns_remaining", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "is_overcharging", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "is_countering", re: true },
        { "->": "battle_loop" },
        { "#f": 1 },
      ],
      setup_alpha_skulker_battle: [
        "^You, ",
        "ev",
        { "VAR?": "rival_name" },
        "out",
        "/ev",
        "^, Jed, and two other skilled-looking contestants are the first to reach the platform. The Alpha Skulker Matriarch emerges from the derelict train car. It is immense, covered in glowing green scars, and it lets out a shriek that shakes the very foundations of the cavern. The final battle begins.",
        "\n",
        "ev",
        150,
        "/ev",
        { "VAR=": "alpha_skulker_max_hp", re: true },
        "ev",
        150,
        "/ev",
        { "VAR=": "alpha_skulker_hp", re: true },
        "ev",
        10,
        "/ev",
        { "VAR=": "alpha_skulker_atk", re: true },
        "ev",
        5,
        "/ev",
        { "VAR=": "alpha_skulker_def", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "alpha_is_berserk", re: true },
        "ev",
        0,
        "/ev",
        { "VAR=": "player_contribution", re: true },
        "ev",
        0,
        "/ev",
        { "VAR=": "rival_contribution", re: true },
        "ev",
        0,
        "/ev",
        { "VAR=": "jed_contribution", re: true },
        "ev",
        0,
        "/ev",
        { "VAR=": "ally_3_contribution", re: true },
        "ev",
        0,
        "/ev",
        { "VAR=": "ally_4_contribution", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "ally_3_is_down", re: true },
        "ev",
        false,
        "/ev",
        { "VAR=": "ally_4_is_down", re: true },
        { "->": "alpha_skulker_battle_loop" },
        { "#f": 1 },
      ],
      setup_two_skulker_battle: [
        { "temp=": "jed_is_providing_cover" },
        "ev",
        "str",
        "^Skulker Packmate Beta",
        "/str",
        "/ev",
        { "VAR=": "current_enemy_name", re: true },
        "ev",
        20,
        "/ev",
        { "VAR=": "current_enemy_hp", re: true },
        "ev",
        6,
        "/ev",
        { "VAR=": "current_enemy_atk", re: true },
        "ev",
        3,
        "/ev",
        { "VAR=": "current_enemy_def", re: true },
        "ev",
        { "VAR?": "jed_is_providing_cover" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              1,
              "/ev",
              { "VAR=": "current_enemy_def", re: true },
              { "->": ".^.^.^.23" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        "str",
        "^Skulker Packmate Gama",
        "/str",
        "/ev",
        { "VAR=": "enemy2_name", re: true },
        "ev",
        20,
        "/ev",
        { "VAR=": "enemy2_hp", re: true },
        "ev",
        6,
        "/ev",
        { "VAR=": "enemy2_atk", re: true },
        "ev",
        3,
        "/ev",
        { "VAR=": "enemy2_def", re: true },
        "ev",
        { "VAR?": "jed_max_hp" },
        "/ev",
        { "VAR=": "jed_hp", re: true },
        { "->": "battle_loop" },
        { "#f": 1 },
      ],
      alpha_skulker_battle_loop: [
        [
          "ev",
          { "VAR?": "alpha_skulker_hp" },
          { "VAR?": "alpha_skulker_max_hp" },
          10,
          "/",
          "<=",
          { "VAR?": "alpha_is_berserk" },
          "!",
          "&&",
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "ev",
                true,
                "/ev",
                { "VAR=": "alpha_is_berserk", re: true },
                "ev",
                15,
                "/ev",
                { "VAR=": "alpha_skulker_atk", re: true },
                "ev",
                2,
                "/ev",
                { "VAR=": "alpha_skulker_def", re: true },
                "^The Matriarch shrieks in agony and rage, its glowing scars flaring violently. It's gone berserk!",
                "\n",
                "ev",
                { "VAR?": "rival_relationship" },
                20,
                "<=",
                "/ev",
                [
                  { "->": ".^.b", c: true },
                  {
                    b: [
                      "\n",
                      "ev",
                      true,
                      "/ev",
                      { "VAR=": "rival_is_waiting_for_opening", re: true },
                      "^Seeing the boss falter, your Rival stops fighting. A cruel, opportunistic glint appears in their eyes. They back away from the fight, content to watch you and the creature tear each other apart.",
                      "\n",
                      "ev",
                      { "VAR?": "jed_status" },
                      "str",
                      "^HOSTILE",
                      "/str",
                      "==",
                      "/ev",
                      [
                        { "->": ".^.b", c: true },
                        {
                          b: [
                            "\n",
                            '^Jed joins them, a grim look on his face. "Sorry, kid. It\'s just business."',
                            "\n",
                            { "->": ".^.^.^.15" },
                            null,
                          ],
                        },
                      ],
                      "nop",
                      "\n",
                      { "->": ".^.^.^.21" },
                      null,
                    ],
                  },
                ],
                "nop",
                "\n",
                { "->": ".^.^.^.11" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "^The Alpha Skulker is at ",
          "ev",
          { "VAR?": "alpha_skulker_hp" },
          "out",
          "/ev",
          "^/",
          "ev",
          { "VAR?": "alpha_skulker_max_hp" },
          "out",
          "/ev",
          "^ HP. You are at ",
          "ev",
          { "VAR?": "hp" },
          "out",
          "/ev",
          "^/",
          "ev",
          { "VAR?": "max_hp" },
          "out",
          "/ev",
          "^ HP.",
          "\n",
          "ev",
          { "VAR?": "rival_is_in_danger" },
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                { "->": "rival_in_danger_event" },
                { "->": ".^.^.^.39" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "ev",
          "str",
          "^Attack",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 4 },
          "ev",
          "str",
          "^Use Skill",
          "/str",
          { "VAR?": "used_skill_in_battle" },
          "!",
          "/ev",
          { "*": ".^.c-1", flg: 5 },
          "ev",
          "str",
          "^Use Moss Poison (",
          "ev",
          { "VAR?": "has_moss_poison_vial" },
          "out",
          "/ev",
          "^ left)",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Aris",
          "/str",
          "==",
          { "VAR?": "has_moss_poison_vial" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-2", flg: 5 },
          "ev",
          "str",
          "^Use Poison Bomb",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Aris",
          "/str",
          "==",
          { "VAR?": "poison_bomb_stack" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-3", flg: 5 },
          "ev",
          "str",
          "^Use EMP Grenade",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Aris",
          "/str",
          "==",
          { "VAR?": "has_emp_grenade" },
          "&&",
          "/ev",
          { "*": ".^.c-4", flg: 5 },
          "ev",
          "str",
          "^Fire a Scrap Arrow (",
          "ev",
          { "VAR?": "scrap_arrow_count" },
          "out",
          "/ev",
          "^ left)",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Lena",
          "/str",
          "==",
          { "VAR?": "scrap_arrow_count" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-5", flg: 5 },
          "ev",
          "str",
          "^Fire a Silent Arrow (",
          "ev",
          { "VAR?": "silent_arrow_count" },
          "out",
          "/ev",
          "^ left)",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Lena",
          "/str",
          "==",
          { "VAR?": "silent_arrow_count" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-6", flg: 5 },
          "ev",
          "str",
          "^Fire a Shock Arrow (",
          "ev",
          { "VAR?": "shock_arrow_count" },
          "out",
          "/ev",
          "^ left)",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Lena",
          "/str",
          "==",
          { "VAR?": "shock_arrow_count" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-7", flg: 5 },
          "ev",
          "str",
          "^Use Kinetic Emitter (",
          "ev",
          { "VAR?": "emitter_charges" },
          "out",
          "/ev",
          "^ left)",
          "/str",
          { "VAR?": "emitter_equipped" },
          { "VAR?": "emitter_charges" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-8", flg: 21 },
          "ev",
          "str",
          "^Defend",
          "/str",
          "/ev",
          { "*": ".^.c-9", flg: 4 },
          "ev",
          "str",
          "^This is too much. Flee the battle.",
          "/str",
          { "VAR?": "hp" },
          { "VAR?": "max_hp" },
          4,
          "/",
          "<=",
          { "VAR?": "alpha_is_berserk" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-10", flg: 21 },
          {
            "c-0": [
              "\n",
              "ev",
              { "VAR?": "atk" },
              { "VAR?": "alpha_skulker_def" },
              "-",
              "/ev",
              { "temp=": "damage" },
              "ev",
              { "VAR?": "damage" },
              1,
              "<",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    1,
                    "/ev",
                    { "temp=": "damage", re: true },
                    { "->": ".^.^.^.13" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              { "VAR?": "alpha_skulker_hp" },
              { "VAR?": "damage" },
              "-",
              { "VAR=": "alpha_skulker_hp", re: true },
              "/ev",
              "ev",
              { "VAR?": "player_contribution" },
              { "VAR?": "damage" },
              "+",
              { "VAR=": "player_contribution", re: true },
              "/ev",
              "^You strike the Matriarch for ",
              "ev",
              { "VAR?": "damage" },
              "out",
              "/ev",
              "^ damage!",
              "\n",
              { "->": "allies_turn" },
              { "#f": 5 },
            ],
            "c-1": ["\n", { "->": "alpha_use_skill" }, { "#f": 5 }],
            "c-2": ["\n", { "->": "alpha_use_moss_poison" }, { "#f": 5 }],
            "c-3": ["\n", { "->": "alpha_use_poison_bomb" }, { "#f": 5 }],
            "c-4": ["\n", { "->": "alpha_use_emp_grenade" }, { "#f": 5 }],
            "c-5": ["\n", { "->": "alpha_fire_scrap_arrow" }, { "#f": 5 }],
            "c-6": ["\n", { "->": "alpha_fire_silent_arrow" }, { "#f": 5 }],
            "c-7": ["\n", { "->": "alpha_fire_shock_arrow" }, { "#f": 5 }],
            "c-8": ["\n", { "->": "player_use_emitter_on_alpha" }, { "#f": 5 }],
            "c-9": [
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "is_defending", re: true },
              "^You brace yourself for the Matriarch's onslaught.",
              "\n",
              { "->": "allies_turn" },
              { "#f": 5 },
            ],
            "c-10": ["\n", { "->": ".^.^.^.alpha_battle_flee" }, { "#f": 5 }],
          },
        ],
        {
          alpha_battle_flee: [
            '^The pain and the terror are too much. You turn your back on your allies and run, scrambling out of the cavern. Your Rival sees your cowardice, their face a mask of pure disgust. "Coward!" they scream after you.',
            "\n",
            "^You've abandoned the fight and sealed your fate in this zone. Your rival will never forget this betrayal.",
            "\n",
            "ev",
            0,
            "/ev",
            { "VAR=": "rival_relationship", re: true },
            { "->": "chapter_1_failure_ending" },
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      chapter_1_failure_ending: [
        "^You are left alone in the silent, cavernous lair. The Archive Gate closes, its energy fading into darkness. The Proctor's voice echoes one last time, not with triumph, but with cold, dismissive finality.",
        "\n",
        '^<i>PROCTOR: "Subject has failed to meet the primary objective parameters. Insufficient data acquired. Specimen is deemed substandard."</i>',
        "\n",
        '^<i>PROCTOR: "Initiating sanitation protocol."</i>',
        "\n",
        "^A low hum fills the air as hidden turrets descend from the ceiling, their targeting lasers painting your chest red. There is no escape. The Arena has found you wanting.",
        "\n",
        "end",
        { "#f": 1 },
      ],
      rival_final_blow: [
        "^With the battle reaching its desperate climax, your Rival sees their chance.",
        "\n",
        "ev",
        { "VAR?": "hp" },
        10,
        "<=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              '^As the Matriarch prepares to attack you, your Rival strikes you from behind. "Nothing personal," they whisper. The blow is fatal.',
              "\n",
              { "->": "game_over_death" },
              { "->": ".^.^.^.9" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "^As you prepare to land the final blow, your Rival darts past you, landing a cheap, opportunistic strike that fells the weakened Matriarch. They've stolen your kill and your glory.",
              "\n",
              "ev",
              { "VAR?": "rival_contribution" },
              20,
              "+",
              { "VAR=": "rival_contribution", re: true },
              "/ev",
              { "->": "alpha_skulker_defeated" },
              { "->": ".^.^.^.9" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      alpha_use_skill: [
        [
          "ev",
          true,
          "/ev",
          { "VAR=": "used_skill_in_battle", re: true },
          "ev",
          "str",
          "^Survivalist",
          "/str",
          { "VAR?": "player_skills" },
          { "VAR?": "Survivalist" },
          "?",
          "/ev",
          { "*": ".^.c-0", flg: 5 },
          "ev",
          "str",
          "^Bio can",
          "/str",
          { "VAR?": "player_skills" },
          { "VAR?": "BioScan" },
          "?",
          "/ev",
          { "*": ".^.c-1", flg: 5 },
          "ev",
          "str",
          "^Discerning Eye",
          "/str",
          { "VAR?": "player_skills" },
          { "VAR?": "DiscerningEye" },
          "?",
          "/ev",
          { "*": ".^.c-2", flg: 5 },
          "ev",
          "str",
          "^Use Heavy Hitter",
          "/str",
          { "VAR?": "player_skills" },
          { "VAR?": "HeavyHitter" },
          "?",
          "/ev",
          { "*": ".^.c-3", flg: 5 },
          "ev",
          "str",
          "^Use Overcharge",
          "/str",
          { "VAR?": "player_skills" },
          { "VAR?": "Overcharge" },
          "?",
          "/ev",
          { "*": ".^.c-4", flg: 5 },
          "ev",
          "str",
          "^Prepare to Counter Attack",
          "/str",
          { "VAR?": "player_skills" },
          { "VAR?": "CounterAttack" },
          "?",
          "/ev",
          { "*": ".^.c-5", flg: 5 },
          {
            "c-0": [
              "^ ",
              "\n",
              "^You roar, focusing your rage into a single, powerful strike. Your Attack is temporarily increased!",
              "\n",
              "ev",
              { "VAR?": "atk" },
              2,
              "+",
              { "VAR=": "atk", re: true },
              "/ev",
              { "->": "allies_turn" },
              { "#f": 5 },
            ],
            "c-1": [
              "^ ",
              "\n",
              "^You activate your bio-scanner, identifying a weak point in the creature's hide. Its Defense is lowered.",
              "\n",
              "ev",
              { "VAR?": "alpha_skulker_hp" },
              2,
              "-",
              { "VAR=": "alpha_skulker_hp", re: true },
              "/ev",
              "ev",
              { "VAR?": "alpha_skulker_hp" },
              0,
              "<",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    0,
                    "/ev",
                    { "VAR=": "alpha_skulker_hp", re: true },
                    { "->": ".^.^.^.16" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": "allies_turn" },
              { "#f": 5 },
            ],
            "c-2": [
              "^ ",
              "\n",
              "^You watch the creature's feral movements, predicting its lunge. You'll be able to easily dodge its next attack.",
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "rival_will_miss_next_turn", re: true },
              { "->": "allies_turn" },
              { "#f": 5 },
            ],
            "c-3": [
              "\n",
              "^You channel all your strength into a single, devastating blow. It's slow, but powerful.",
              "\n",
              "ev",
              { "VAR?": "atk" },
              1.5,
              "*",
              "INT",
              "/ev",
              { "temp=": "heavy_damage" },
              "\n",
              "ev",
              { "VAR?": "alpha_skulker_hp" },
              { "VAR?": "heavy_damage" },
              "-",
              { "VAR=": "alpha_skulker_hp", re: true },
              "/ev",
              "^You slam the Alpha Skulker for ",
              "ev",
              { "VAR?": "heavy_damage" },
              "out",
              "/ev",
              "^ damage!",
              "\n",
              "ev",
              { "VAR?": "current_enemy_hp" },
              0,
              "<=",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    { "->": "battle_won" },
                    { "->": ".^.^.^.31" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    { "->": "allies_turn" },
                    { "->": ".^.^.^.31" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "#f": 5 },
            ],
            "c-4": [
              "\n",
              "^You reroute power through the Kinetic Emitter, preparing to unleash an overloaded blast. Your next Emitter use will be massively amplified.",
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "is_overcharging", re: true },
              { "->": "allies_turn" },
              { "#f": 5 },
            ],
            "c-5": [
              "\n",
              "^You take a defensive stance, watching your enemy's every move, ready to strike the moment they commit to an attack.",
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "is_countering", re: true },
              { "->": "allies_turn" },
              { "#f": 5 },
            ],
          },
        ],
        { "#f": 1 },
      ],
      alpha_use_emitter: [
        "ev",
        { "f()": "use_emitter_charge" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              10,
              "/ev",
              { "temp=": "damage" },
              "ev",
              { "VAR?": "is_overcharging" },
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    30,
                    "/ev",
                    { "temp=": "damage", re: true },
                    "ev",
                    false,
                    "/ev",
                    { "VAR=": "is_overcharging", re: true },
                    { "->": ".^.^.^.9" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "^The emitter blast slams into the Matriarch!",
              "\n",
              "ev",
              { "VAR?": "alpha_skulker_hp" },
              { "VAR?": "damage" },
              "-",
              { "VAR=": "alpha_skulker_hp", re: true },
              "/ev",
              "ev",
              { "VAR?": "player_contribution" },
              { "VAR?": "damage" },
              "+",
              { "VAR=": "player_contribution", re: true },
              "/ev",
              { "->": ".^.^.^.4" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "->": "allies_turn" },
        { "#f": 1 },
      ],
      alpha_use_moss_poison: [
        "ev",
        { "VAR?": "has_moss_poison_vial" },
        1,
        "-",
        { "VAR=": "has_moss_poison_vial", re: true },
        "/ev",
        "^You apply the poison to your weapon. The Matriarch is now poisoned!",
        "\n",
        "ev",
        true,
        "/ev",
        { "VAR=": "enemy_is_poisoned", re: true },
        "ev",
        3,
        "/ev",
        { "VAR=": "poison_turns_remaining", re: true },
        { "->": "allies_turn" },
        { "#f": 1 },
      ],
      alpha_use_poison_bomb: [
        "ev",
        { "VAR?": "poison_bomb_stack" },
        1,
        "-",
        { "VAR=": "poison_bomb_stack", re: true },
        "/ev",
        "^The poison bomb explodes at the Matriarch's feet, dealing massive initial damage and applying a potent toxin.",
        "\n",
        "ev",
        { "VAR?": "atk" },
        3,
        "*",
        "/ev",
        { "temp=": "damage" },
        "ev",
        { "VAR?": "alpha_skulker_hp" },
        { "VAR?": "damage" },
        "-",
        { "VAR=": "alpha_skulker_hp", re: true },
        "/ev",
        "ev",
        { "VAR?": "player_contribution" },
        { "VAR?": "damage" },
        "+",
        { "VAR=": "player_contribution", re: true },
        "/ev",
        "ev",
        true,
        "/ev",
        { "VAR=": "enemy_is_poisoned", re: true },
        "ev",
        5,
        "/ev",
        { "VAR=": "poison_turns_remaining", re: true },
        { "->": "allies_turn" },
        { "#f": 1 },
      ],
      alpha_use_emp_grenade: [
        "ev",
        false,
        "/ev",
        { "VAR=": "has_emp_grenade", re: true },
        "^The EMP grenade detonates, causing the Matriarch to stagger and shriek in confusion. It will miss its next attack.",
        "\n",
        "ev",
        true,
        "/ev",
        { "VAR=": "rival_will_miss_next_turn", re: true },
        { "->": "allies_turn" },
        { "#f": 1 },
      ],
      alpha_fire_scrap_arrow: [
        "ev",
        { "VAR?": "scrap_arrow_count" },
        1,
        "-",
        { "VAR=": "scrap_arrow_count", re: true },
        "/ev",
        "^Your arrow finds a seam in the Matriarch's tough hide.",
        "\n",
        "ev",
        5,
        "/ev",
        { "temp=": "damage" },
        "ev",
        { "VAR?": "alpha_skulker_hp" },
        { "VAR?": "damage" },
        "-",
        { "VAR=": "alpha_skulker_hp", re: true },
        "/ev",
        "ev",
        { "VAR?": "player_contribution" },
        { "VAR?": "damage" },
        "+",
        { "VAR=": "player_contribution", re: true },
        "/ev",
        { "->": "allies_turn" },
        { "#f": 1 },
      ],
      alpha_fire_silent_arrow: [
        "ev",
        { "VAR?": "silent_arrow_count" },
        1,
        "-",
        { "VAR=": "silent_arrow_count", re: true },
        "/ev",
        "^Your silent arrow strikes a vulnerable, glowing scar. A critical hit!",
        "\n",
        "ev",
        9,
        "/ev",
        { "temp=": "damage" },
        "ev",
        { "VAR?": "alpha_skulker_hp" },
        { "VAR?": "damage" },
        "-",
        { "VAR=": "alpha_skulker_hp", re: true },
        "/ev",
        "ev",
        { "VAR?": "player_contribution" },
        { "VAR?": "damage" },
        "+",
        { "VAR=": "player_contribution", re: true },
        "/ev",
        { "->": "allies_turn" },
        { "#f": 1 },
      ],
      alpha_fire_shock_arrow: [
        "ev",
        { "VAR?": "shock_arrow_count" },
        1,
        "-",
        { "VAR=": "shock_arrow_count", re: true },
        "/ev",
        "^The shock arrow delivers a powerful jolt, causing the massive creature to seize up for a moment.",
        "\n",
        "ev",
        12,
        "/ev",
        { "temp=": "damage" },
        "ev",
        { "VAR?": "alpha_skulker_hp" },
        { "VAR?": "damage" },
        "-",
        { "VAR=": "alpha_skulker_hp", re: true },
        "/ev",
        "ev",
        { "VAR?": "player_contribution" },
        { "VAR?": "damage" },
        "+",
        { "VAR=": "player_contribution", re: true },
        "/ev",
        { "->": "allies_turn" },
        { "#f": 1 },
      ],
      player_use_emitter_on_alpha: [
        "ev",
        { "f()": "use_emitter_charge" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^You unleash a wave of pure force from the emitter. It slams into the ",
              "ev",
              { "VAR?": "current_enemy_name" },
              "out",
              "/ev",
              "^, sending it reeling.",
              "\n",
              "ev",
              15,
              "/ev",
              { "temp=": "emitter_damage" },
              "ev",
              { "VAR?": "is_overcharging" },
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    30,
                    "/ev",
                    { "temp=": "emitter_damage", re: true },
                    "ev",
                    false,
                    "/ev",
                    { "VAR=": "is_overcharging", re: true },
                    "^The Emitter shrieks as it discharges the excess energy.",
                    "\n",
                    { "->": ".^.^.^.16" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              { "VAR?": "alpha_skulker_hp" },
              { "VAR?": "emitter_damage" },
              "-",
              { "VAR=": "alpha_skulker_hp", re: true },
              "/ev",
              "ev",
              { "VAR?": "player_contribution" },
              { "VAR?": "emitter_damage" },
              "+",
              { "VAR=": "player_contribution", re: true },
              "/ev",
              { "->": ".^.^.^.4" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "alpha_skulker_hp" },
        0,
        "<=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              { "->": "alpha_skulker_defeated" },
              { "->": ".^.^.^.13" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              { "->": "alpha_skulker_turn" },
              { "->": ".^.^.^.13" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      allies_turn: [
        "ev",
        { "VAR?": "rival_is_waiting_for_opening" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              { "->": "alpha_skulker_turn" },
              { "->": ".^.^.^.4" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "jed_status" },
        "str",
        "^HOSTILE",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^Jed hangs back, taking pot-shots and contributing little.",
              "\n",
              "ev",
              { "VAR?": "jed_contribution" },
              2,
              "+",
              { "VAR=": "jed_contribution", re: true },
              "/ev",
              "ev",
              { "VAR?": "alpha_skulker_hp" },
              2,
              "-",
              { "VAR=": "alpha_skulker_hp", re: true },
              "/ev",
              { "->": ".^.^.^.15" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "^Jed fights beside you, a seasoned veteran. He lands a solid blow.",
              "\n",
              "ev",
              { "VAR?": "jed_contribution" },
              5,
              "+",
              { "VAR=": "jed_contribution", re: true },
              "/ev",
              "ev",
              { "VAR?": "alpha_skulker_hp" },
              5,
              "-",
              { "VAR=": "alpha_skulker_hp", re: true },
              "/ev",
              { "->": ".^.^.^.15" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        1,
        4,
        "rnd",
        "/ev",
        { "temp=": "rival_action_chance" },
        "\n",
        [
          "ev",
          { "VAR?": "rival_action_chance" },
          1,
          "==",
          { "VAR?": "rival_has_emitter" },
          "&&",
          { "VAR?": "rival_emitter_charges" },
          0,
          ">",
          "&&",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "rival_emitter_charges" },
              1,
              "-",
              { "VAR=": "rival_emitter_charges", re: true },
              "/ev",
              "^Your Rival unleashes a blast from their Kinetic Field Emitter! The raw force tears a huge chunk of armor from the Matriarch.",
              "\n",
              "ev",
              20,
              "/ev",
              { "temp=": "emitter_damage" },
              "ev",
              { "VAR?": "rival_contribution" },
              { "VAR?": "emitter_damage" },
              "+",
              { "VAR=": "rival_contribution", re: true },
              "/ev",
              "ev",
              { "VAR?": "alpha_skulker_hp" },
              { "VAR?": "emitter_damage" },
              "-",
              { "VAR=": "alpha_skulker_hp", re: true },
              "/ev",
              { "->": ".^.^.^.27" },
              null,
            ],
          },
        ],
        [
          "ev",
          { "VAR?": "rival_action_chance" },
          2,
          "<=",
          { "VAR?": "rival_used_combat_skill" },
          "!",
          "&&",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "rival_used_combat_skill", re: true },
              [
                "ev",
                { "VAR?": "rival_name" },
                "str",
                "^Xander",
                "/str",
                "==",
                "/ev",
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^Seeing the Matriarch wounded, Xander roars and leaps, bringing his weapon down in a devastating overhead strike.",
                    "\n",
                    "ev",
                    15,
                    "/ev",
                    { "temp=": "skill_damage" },
                    "ev",
                    { "VAR?": "rival_contribution" },
                    { "VAR?": "skill_damage" },
                    "+",
                    { "VAR=": "rival_contribution", re: true },
                    "/ev",
                    "ev",
                    { "VAR?": "alpha_skulker_hp" },
                    { "VAR?": "skill_damage" },
                    "-",
                    { "VAR=": "alpha_skulker_hp", re: true },
                    "/ev",
                    { "->": ".^.^.^.8" },
                    null,
                  ],
                },
              ],
              [
                "ev",
                { "VAR?": "rival_name" },
                "str",
                "^Jinx",
                "/str",
                "==",
                "/ev",
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    '^"Time for the light show!" Jinx yells, throwing an overloaded power cell into a puddle at the Matriarch\'s feet. An arc of electricity chains between the creature and a nearby ally!',
                    "\n",
                    "ev",
                    12,
                    "/ev",
                    { "temp=": "skill_damage2" },
                    "ev",
                    { "VAR?": "rival_contribution" },
                    { "VAR?": "skill_damage2" },
                    "+",
                    { "VAR=": "rival_contribution", re: true },
                    "/ev",
                    "ev",
                    { "VAR?": "alpha_skulker_hp" },
                    { "VAR?": "skill_damage2" },
                    "-",
                    { "VAR=": "alpha_skulker_hp", re: true },
                    "/ev",
                    "ev",
                    { "VAR?": "hp" },
                    4,
                    "-",
                    { "VAR=": "hp", re: true },
                    "/ev",
                    { "->": ".^.^.^.8" },
                    null,
                  ],
                },
              ],
              [
                "ev",
                { "VAR?": "rival_name" },
                "str",
                "^Isha",
                "/str",
                "==",
                "/ev",
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^Isha nocks a special arrow and waits for the perfect moment. She looses it, striking a vulnerable, glowing scar on the Matriarch's side. The creature shrieks as green energy bleeds from the wound.",
                    "\n",
                    "ev",
                    8,
                    "/ev",
                    { "temp=": "skill_damage3" },
                    "ev",
                    { "VAR?": "rival_contribution" },
                    { "VAR?": "skill_damage3" },
                    "+",
                    { "VAR=": "rival_contribution", re: true },
                    "/ev",
                    "ev",
                    { "VAR?": "alpha_skulker_hp" },
                    { "VAR?": "skill_damage3" },
                    "-",
                    { "VAR=": "alpha_skulker_hp", re: true },
                    "/ev",
                    { "->": ".^.^.^.8" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.27" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "^Your Rival fights with brutal efficiency, scoring a deep hit.",
              "\n",
              "ev",
              6,
              10,
              "rnd",
              "/ev",
              { "temp=": "rival_damage" },
              "\n",
              "ev",
              { "VAR?": "rival_contribution" },
              { "VAR?": "rival_damage" },
              "+",
              { "VAR=": "rival_contribution", re: true },
              "/ev",
              "ev",
              { "VAR?": "alpha_skulker_hp" },
              { "VAR?": "rival_damage" },
              "-",
              { "VAR=": "alpha_skulker_hp", re: true },
              "/ev",
              { "->": ".^.^.^.27" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "ally_3_is_down" },
        "!",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^The third contestant lands their attack.",
              "\n",
              "ev",
              { "VAR?": "ally_3_contribution" },
              4,
              "+",
              { "VAR=": "ally_3_contribution", re: true },
              "/ev",
              "ev",
              { "VAR?": "alpha_skulker_hp" },
              4,
              "-",
              { "VAR=": "alpha_skulker_hp", re: true },
              "/ev",
              { "->": ".^.^.^.34" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "ally_4_is_down" },
        "!",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^The fourth contestant lands their attack.",
              "\n",
              "ev",
              { "VAR?": "ally_4_contribution" },
              4,
              "+",
              { "VAR=": "ally_4_contribution", re: true },
              "/ev",
              "ev",
              { "VAR?": "alpha_skulker_hp" },
              4,
              "-",
              { "VAR=": "alpha_skulker_hp", re: true },
              "/ev",
              { "->": ".^.^.^.41" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "alpha_skulker_hp" },
        0,
        "<=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              { "->": "alpha_skulker_defeated" },
              { "->": ".^.^.^.50" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              { "->": "alpha_skulker_turn" },
              { "->": ".^.^.^.50" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      alpha_skulker_turn: [
        "ev",
        { "VAR?": "enemy_is_poisoned" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "poison_turns_remaining" },
              1,
              "-",
              { "VAR=": "poison_turns_remaining", re: true },
              "/ev",
              "ev",
              1,
              { "VAR?": "intelligence" },
              2,
              "/",
              "INT",
              "+",
              "/ev",
              { "temp=": "poison_damage" },
              "\n",
              "^The Matriach winces as the poison takes effect, dealing ",
              "ev",
              { "VAR?": "poison_damage" },
              "out",
              "/ev",
              "^ damage.",
              "\n",
              "ev",
              { "VAR?": "alpha_skulker_hp" },
              { "VAR?": "poison_damage" },
              "-",
              { "VAR=": "alpha_skulker_hp", re: true },
              "/ev",
              "ev",
              { "VAR?": "poison_turns_remaining" },
              0,
              "<=",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    false,
                    "/ev",
                    { "VAR=": "enemy_is_poisoned", re: true },
                    "^The poison wears off.",
                    "\n",
                    { "->": ".^.^.^.36" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.4" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "def" },
        "/ev",
        { "temp=": "current_player_def" },
        "ev",
        { "VAR?": "is_defending" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "def" },
              3,
              "+",
              "/ev",
              { "temp=": "current_player_def", re: true },
              { "->": ".^.^.^.14" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "rival_is_waiting_for_opening" },
        { "VAR?": "hp" },
        10,
        "<=",
        { "VAR?": "alpha_skulker_hp" },
        15,
        "<=",
        "||",
        "&&",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              { "->": "rival_final_blow" },
              { "->": ".^.^.^.28" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "^The Matriarch shrieks and attacks!",
        "\n",
        "ev",
        1,
        10,
        "rnd",
        "/ev",
        { "temp=": "aoe_roll" },
        "\n",
        "ev",
        { "VAR?": "aoe_roll" },
        2,
        "<=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^It unleashes a deafening sonic screech that hits everyone on the platform!",
              "\n",
              "ev",
              { "VAR?": "hp" },
              5,
              "-",
              { "VAR=": "hp", re: true },
              "/ev",
              "^You take 5 damage from the sonic blast!",
              "\n",
              { "->": ".^.^.^.46" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "^It lunges, claws flashing!",
              "\n",
              "ev",
              1,
              5,
              "rnd",
              "/ev",
              { "temp=": "target_roll" },
              "\n",
              "ev",
              { "VAR?": "alpha_skulker_atk" },
              { "VAR?": "def" },
              "-",
              "/ev",
              { "temp=": "damage" },
              "ev",
              { "VAR?": "damage" },
              1,
              "<",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    1,
                    "/ev",
                    { "temp=": "damage", re: true },
                    { "->": ".^.^.^.22" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              [
                "ev",
                { "VAR?": "target_roll" },
                1,
                "==",
                "/ev",
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    1,
                    100,
                    "rnd",
                    "/ev",
                    { "temp=": "player_dodge_roll" },
                    "\n",
                    "ev",
                    { "VAR?": "player_dodge_roll" },
                    { "VAR?": "dodge_chance" },
                    "<=",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "^You anticipate the attack and deftly move aside. It misses!",
                          "\n",
                          { "->": ".^.^.^.15" },
                          null,
                        ],
                      },
                    ],
                    [
                      { "->": ".^.b" },
                      {
                        b: [
                          "\n",
                          "ev",
                          { "VAR?": "rival_will_miss_next_turn" },
                          "/ev",
                          [
                            { "->": ".^.b", c: true },
                            {
                              b: [
                                "\n",
                                "ev",
                                false,
                                "/ev",
                                {
                                  "VAR=": "rival_will_miss_next_turn",
                                  re: true,
                                },
                                "^You anticipate the Alpha Skulker clumsy attack and easily step aside. It misses completely.",
                                "\n",
                                { "->": "allies_turn" },
                                { "->": ".^.^.^.6" },
                                null,
                              ],
                            },
                          ],
                          [
                            { "->": ".^.b" },
                            {
                              b: [
                                "\n",
                                "ev",
                                { "VAR?": "hp" },
                                { "VAR?": "damage" },
                                "-",
                                { "VAR=": "hp", re: true },
                                "/ev",
                                "^It attacks you for ",
                                "ev",
                                { "VAR?": "damage" },
                                "out",
                                "/ev",
                                "^ damage!",
                                "\n",
                                { "->": ".^.^.^.6" },
                                null,
                              ],
                            },
                          ],
                          "nop",
                          "\n",
                          { "->": ".^.^.^.15" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    { "->": ".^.^.^.29" },
                    null,
                  ],
                },
              ],
              [
                "ev",
                { "VAR?": "target_roll" },
                2,
                "==",
                "/ev",
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^The Matriarch slams into ",
                    "ev",
                    { "VAR?": "rival_name" },
                    "out",
                    "/ev",
                    "^, sending them flying. They're down, but not out of the fight.",
                    "\n",
                    { "->": ".^.^.^.29" },
                    null,
                  ],
                },
              ],
              [
                "ev",
                { "VAR?": "target_roll" },
                3,
                "==",
                "/ev",
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    { "VAR?": "jed_status" },
                    "str",
                    "^DEAD",
                    "/str",
                    "==",
                    "!",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "ev",
                          { "VAR?": "jed_hp" },
                          { "VAR?": "damage" },
                          "-",
                          { "VAR=": "jed_hp", re: true },
                          "/ev",
                          "^It attacks Jed for ",
                          "ev",
                          { "VAR?": "damage" },
                          "out",
                          "/ev",
                          "^ damage!",
                          "\n",
                          { "->": ".^.^.^.10" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    { "->": ".^.^.^.29" },
                    null,
                  ],
                },
              ],
              [
                "ev",
                { "VAR?": "target_roll" },
                4,
                "==",
                "/ev",
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    { "VAR?": "ally_3_is_down" },
                    "!",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "ev",
                          { "VAR?": "ally_3_hp" },
                          { "VAR?": "damage" },
                          "-",
                          { "VAR=": "ally_3_hp", re: true },
                          "/ev",
                          "^It attacks the third contestant for ",
                          "ev",
                          { "VAR?": "damage" },
                          "out",
                          "/ev",
                          "^ damage!",
                          "\n",
                          { "->": ".^.^.^.6" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    { "->": ".^.^.^.29" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "ev",
                    { "VAR?": "ally_4_is_down" },
                    "!",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "ev",
                          { "VAR?": "ally_4_hp" },
                          { "VAR?": "damage" },
                          "-",
                          { "VAR=": "ally_4_hp", re: true },
                          "/ev",
                          "^It attacks the fourth contestant for ",
                          "ev",
                          { "VAR?": "damage" },
                          "out",
                          "/ev",
                          "^ damage!",
                          "\n",
                          { "->": ".^.^.^.6" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    { "->": ".^.^.^.29" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.46" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "hp" },
        0,
        "<=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: ["\n", { "->": "game_over_death" }, { "->": ".^.^.^.54" }, null],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "jed_hp" },
        0,
        "<=",
        { "VAR?": "jed_status" },
        "str",
        "^DEAD",
        "/str",
        "!=",
        "&&",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^Jed is struck down by the Matriarch's attack and doesn't get back up.",
              "\n",
              "ev",
              "str",
              "^DEAD",
              "/str",
              "/ev",
              { "VAR=": "jed_status", re: true },
              { "->": ".^.^.^.68" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "ally_3_hp" },
        0,
        "<=",
        { "VAR?": "ally_3_is_down" },
        "!",
        "&&",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^The third contestant is overwhelmed and falls, unconscious.",
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "ally_3_is_down", re: true },
              { "->": ".^.^.^.79" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "ally_4_hp" },
        0,
        "<=",
        { "VAR?": "ally_4_is_down" },
        "!",
        "&&",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^The fourth contestant is thrown from the platform by a vicious blow.",
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "ally_4_is_down", re: true },
              { "->": ".^.^.^.90" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        1,
        4,
        "rnd",
        "/ev",
        { "temp=": "event_roll" },
        "\n",
        "ev",
        { "VAR?": "event_roll" },
        1,
        "==",
        { "VAR?": "rival_is_in_danger" },
        "!",
        "&&",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "rival_is_in_danger", re: true },
              { "->": ".^.^.^.108" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "->": "alpha_skulker_battle_loop" },
        { "#f": 1 },
      ],
      rival_in_danger_event: [
        [
          "^The Matriarch turns its attention, pinning your Rival against the derelict train car. They're trapped and vulnerable! This is your chance.",
          "\n",
          "ev",
          "str",
          "^Support your Rival",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 4 },
          "ev",
          "str",
          "^Sabotage your Rival",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 4 },
          "ev",
          "str",
          "^Do nothing",
          "/str",
          "/ev",
          { "*": ".^.c-2", flg: 4 },
          {
            "c-0": [
              "\n",
              "^You fire a shot or create a diversion, drawing the Matriarch's attention away from them. Your Rival gives you a surprised, grudging look of acknowledgement.",
              "\n",
              "ev",
              false,
              "/ev",
              { "VAR=": "rival_is_in_danger", re: true },
              "ev",
              { "VAR?": "rival_relationship" },
              10,
              "+",
              { "VAR=": "rival_relationship", re: true },
              "/ev",
              { "->": "alpha_skulker_turn" },
              { "#f": 5 },
            ],
            "c-1": [
              "\n",
              '^You "accidentally" kick a piece of debris in their way, causing them to stumble. The Matriarch\'s claws rake across their side, wounding them badly. They glare at you with pure hatred.',
              "\n",
              "ev",
              { "VAR?": "rival_contribution" },
              10,
              "-",
              { "VAR=": "rival_contribution", re: true },
              "/ev",
              "ev",
              { "VAR?": "rival_relationship" },
              15,
              "-",
              { "VAR=": "rival_relationship", re: true },
              "/ev",
              "ev",
              false,
              "/ev",
              { "VAR=": "rival_is_in_danger", re: true },
              { "->": "alpha_skulker_turn" },
              { "#f": 5 },
            ],
            "c-2": [
              "\n",
              "^You watch, impassive, as your Rival barely manages to fend off the attack on their own. They are wounded and exhausted from the effort.",
              "\n",
              "ev",
              false,
              "/ev",
              { "VAR=": "rival_is_in_danger", re: true },
              "ev",
              { "VAR?": "rival_relationship" },
              5,
              "-",
              { "VAR=": "rival_relationship", re: true },
              "/ev",
              { "->": "alpha_skulker_turn" },
              { "#f": 5 },
            ],
          },
        ],
        { "#f": 1 },
      ],
      alpha_skulker_defeated: [
        "^With a final, agonized shriek, the Alpha Skulker Matriarch collapses. The cavern falls silent, the only sound your own ragged breathing and the thumping of your heart.",
        "\n",
        '^<i>PROCTOR: "Apex Predator neutralized. Calculating combat contributions... Calculation complete."</i>',
        "\n",
        "^The Proctor's voice fills the cavern, as cold and vast as the space between worlds.",
        "\n",
        '^<i>PROCTOR: "Of the ten subjects who began this trial, five have demonstrated sufficient capability to proceed. The remainder have been culled or have proven substandard. An acceptable attrition rate."</i>',
        "\n",
        '^<i>PROCTOR: "Strength is not merely the ability to destroy. It is the ability to adapt, to overcome, and to endure in the face of overwhelming odds. You have demonstrated these qualities to a satisfactory degree."</i>',
        "\n",
        '^<i>PROCTOR: "The following subjects have earned the final Data Fragment and the right to ascend: Subject ',
        "ev",
        { "VAR?": "character_name" },
        "out",
        "/ev",
        "^, Subject ",
        "ev",
        { "VAR?": "rival_name" },
        "out",
        "/ev",
        '^, Subject Jedediah, Subject Jors and Subject Soff"</i>',
        "\n",
        "ev",
        { "VAR?": "data_fragments" },
        1,
        "+",
        { "VAR=": "data_fragments", re: true },
        "/ev",
        '^<i>PROCTOR: "As a reward for your exceptional performance, your biological and cybernetic parameters will now be optimized. Acknowledging... primary combat adaptation."</i>',
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Kaelen",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^You feel a surge of raw power course through your limbs, a deep, primal strength settling into your very bones.",
              "\n",
              "ev",
              { "VAR?": "strength" },
              1,
              "+",
              { "VAR=": "strength", re: true },
              "/ev",
              '^<i>AI: "Primary attribute Strength has been enhanced."</i>',
              "\n",
              { "->": ".^.^.^.38" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Aris",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^A wave of pure data washes through your mind, connections and calculations flowing faster than ever before. Your thoughts feel sharper, clearer.",
              "\n",
              "ev",
              { "VAR?": "intelligence" },
              1,
              "+",
              { "VAR=": "intelligence", re: true },
              "/ev",
              '^<i>AI: "Primary attribute Intelligence has been enhanced."</i>',
              "\n",
              { "->": ".^.^.^.48" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Lena",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^Your senses heighten, the dripping of water in the far corners of the cavern and the subtle scent of ozone becoming crystal clear. You feel more aware, more connected to your surroundings.",
              "\n",
              "ev",
              { "VAR?": "perception" },
              1,
              "+",
              { "VAR=": "perception", re: true },
              "/ev",
              '^<i>AI: "Primary attribute Perception has been enhanced."</i>',
              "\n",
              { "->": ".^.^.^.58" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "f()": "update_combat_stats" },
        "pop",
        "/ev",
        "\n",
        '^<i>PROCTOR: "The Archive Gate is now active. Proceed to the next phase of the experiment. The true test has just begun."</i>',
        "\n",
        "ev",
        { "VAR?": "data_fragments" },
        3,
        ">=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^The massive gate on the platform hums to life, a swirling vortex of green energy. Your Rival gives you a final, unreadable look before stepping through. Jed, if he is still with you, gives a weary nod and follows.",
              "\n",
              "ev",
              "str",
              "^Step through the Archive Gate and leave this place behind.",
              "/str",
              "/ev",
              { "*": ".^.c-0", flg: 20 },
              { "->": ".^.^.^.74" },
              {
                "c-0": [
                  "\n",
                  "^You take a deep breath and step into the unknown, ready for whatever comes next.",
                  "\n",
                  "end",
                  { "#f": 5 },
                ],
              },
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "^The massive gate on the platform hums to life, but a red barrier shimmers in front of you, blocking your path. The other successful contestants step through, leaving you behind.",
              "\n",
              { "->": "chapter_1_failure_ending" },
              { "->": ".^.^.^.74" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      scene_3_ai_query: [
        [
          "^The AI's calm voice is a presence in your mind.",
          "\n",
          "ev",
          "str",
          '^Ask: "Where am I?"',
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          "ev",
          "str",
          "^Ask: \"What is this place? What is the 'Arena'?\"",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 20 },
          "ev",
          "str",
          '^Ask: "Who is the Proctor?"',
          "/str",
          "/ev",
          { "*": ".^.c-2", flg: 20 },
          "ev",
          "str",
          '^Ask: "Why am I here?"',
          "/str",
          "/ev",
          { "*": ".^.c-3", flg: 20 },
          "ev",
          "str",
          "^That's enough for now.",
          "/str",
          "/ev",
          { "*": ".^.c-4", flg: 20 },
          {
            "c-0": [
              "\n",
              "^<i>AI: \"You are within Test Site Echo-7, located on a fragment of a planetary body designated 'The Shattered World'. My designation for this zone is the 'Ruined City-Isle'.\"</i>",
              "\n",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-1": [
              "\n",
              "^<i>AI: \"This 'Arena' is a controlled environment for a series of trials conducted by my creators, the Archivists. The objective is to test species' capacity for survival and adaptation.\"</i>",
              "\n",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-2": [
              "\n",
              '^<i>AI: "The Proctor is the master control AI for this entire experiment. My function is to guide and observe subjects. The Proctor\'s function is to administrate the trials."</i>',
              "\n",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-3": [
              "\n",
              '^<i>AI: "Your selection criteria are not available in my data banks. Your purpose is to survive and demonstrate mastery of the environment. That is all the data I can provide on the subject."</i>',
              "\n",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-4": ["\n", { "->": "scene_3_choices" }, { "#f": 5 }],
          },
        ],
        { "#f": 1 },
      ],
      scene_5_ai_query: [
        [
          "^The AI is silent for a moment before responding.",
          "\n",
          "ev",
          "str",
          '^Ask: "Who was that?"',
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          "ev",
          "str",
          "^Ask: \"What is this 'Supply Drop'?\"",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 20 },
          "ev",
          "str",
          "^That's all I need.",
          "/str",
          "/ev",
          { "*": ".^.c-2", flg: 20 },
          {
            "c-0": [
              "\n",
              '^<i>AI: "That was the Proctor. It administrates the trials."</i>',
              "\n",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-1": [
              "\n",
              '^<i>AI: "A resource distribution event. It is designed to test subjects\' risk-assessment and acquisition capabilities under pressure."</i>',
              "\n",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-2": ["\n", { "->": "scene_5_crossroads" }, { "#f": 5 }],
          },
        ],
        { "#f": 1 },
      ],
      scene_8_ai_query: [
        [
          '^<i>AI: "Awaiting query."</i>',
          "\n",
          "ev",
          "str",
          '^Ask: "What is this structure?"',
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 4 },
          "ev",
          "str",
          '^Ask: "How many other contestants are in this tower?"',
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 4 },
          "ev",
          "str",
          '^Ask: "What is the nature of this test?"',
          "/str",
          "/ev",
          { "*": ".^.c-2", flg: 4 },
          "ev",
          "str",
          "^That's all for now.",
          "/str",
          "/ev",
          { "*": ".^.c-3", flg: 20 },
          {
            "c-0": [
              "\n",
              '^<i>AI: "This is a primary communications relay tower, designated Spire-3. It was part of the former inhabitants\' global network."</i>',
              "\n",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-1": [
              "\n",
              '^<i>AI: "My sensors indicate four other active subjects within the immediate structure, excluding yourself. Proctor protocols designate this as a competitive trial."</i>',
              "\n",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-2": [
              "\n",
              '^<i>AI: "The objective is to retrieve the Data Fragment from the peak. The Proctor is monitoring subject performance based on speed, efficiency, and obstacle neutralization."</i>',
              "\n",
              { "->": ".^.^.^" },
              { "#f": 5 },
            ],
            "c-3": ["\n", { "->": "scene_8_the_tower" }, { "#f": 5 }],
          },
        ],
        { "#f": 1 },
      ],
      analyze_items: [
        [
          "^You take a moment to examine your findings.",
          "\n",
          "ev",
          "str",
          "^Analyze the Degraded Power Cell.",
          "/str",
          { "VAR?": "power_cell_stack" },
          0,
          ">",
          { "CNT?": ".^.^.analyze_power_cell" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-0", flg: 21 },
          "ev",
          "str",
          "^Analyze the Glimmer Moss Sample.",
          "/str",
          { "VAR?": "glimmer_moss_stack" },
          0,
          ">",
          { "VAR?": "analyzed_moss_type" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-1", flg: 21 },
          "ev",
          "str",
          "^Analyze the Archivist Log ",
          "#",
          "^77-B",
          "/#",
          "/str",
          { "VAR?": "found_first_log" },
          "/ev",
          { "*": ".^.c-2", flg: 21 },
          "ev",
          "str",
          "^Analyze Neuro-Stim",
          "/str",
          { "VAR?": "neuro_stim_state" },
          "str",
          "^AVAILABLE",
          "/str",
          "==",
          { "VAR?": "analyzed_neuro_stim_type" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-3", flg: 21 },
          "ev",
          "str",
          "^Analyze Skulker Venom Gland",
          "/str",
          { "VAR?": "skulker_venom_gland_stack" },
          0,
          ">",
          { "VAR?": "analyzed_venom_gland_type" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-4", flg: 21 },
          "ev",
          "str",
          "^Analyze Moss Poison Vial",
          "/str",
          { "VAR?": "has_moss_poison_vial" },
          0,
          ">",
          { "VAR?": "analyzed_poison_vial_type" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-5", flg: 21 },
          "ev",
          "str",
          "^Analyze Calming Poultice Recipe",
          "/str",
          { "VAR?": "knows_calming_poultice_recipe" },
          { "VAR?": "analyzed_poultice_recipe_type" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-6", flg: 21 },
          "ev",
          "str",
          "^Analyze Titan-Beetle Carapace",
          "/str",
          { "VAR?": "has_titan_beetle_carapace" },
          { "VAR?": "analyzed_beetle_carapace_type" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-7", flg: 21 },
          "ev",
          "str",
          "^Analyze Adrenaline Shot",
          "/str",
          { "VAR?": "has_adrenaline_shot" },
          { "VAR?": "analyzed_adrenaline_shot_type" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-8", flg: 21 },
          "ev",
          "str",
          "^Done analyzing.",
          "/str",
          "/ev",
          { "*": ".^.c-9", flg: 4 },
          {
            "c-0": [
              "\n",
              "ev",
              { "^->": "analyze_items.analyze_power_cell" },
              "/ev",
              { "->": ".^.^.^.do_analyze" },
              { "#f": 5 },
            ],
            "c-1": [
              "\n",
              "ev",
              { "^->": "analyze_items.analyze_moss" },
              "/ev",
              { "->": ".^.^.^.do_analyze" },
              { "#f": 5 },
            ],
            "c-2": ["\n", { "->": "decrypt_first_log" }, { "#f": 5 }],
            "c-3": [
              "^ ",
              "ev",
              { "^->": "analyze_items.analyze_neuro_stim" },
              "/ev",
              { "->": ".^.^.^.do_analyze" },
              "\n",
              { "#f": 5 },
            ],
            "c-4": [
              "^ ",
              "ev",
              { "^->": "analyze_items.analyze_venom_gland" },
              "/ev",
              { "->": ".^.^.^.do_analyze" },
              "\n",
              { "#f": 5 },
            ],
            "c-5": [
              "^ ",
              "ev",
              { "^->": "analyze_items.analyze_poison_vial" },
              "/ev",
              { "->": ".^.^.^.do_analyze" },
              "\n",
              { "#f": 5 },
            ],
            "c-6": [
              "^ ",
              "ev",
              { "^->": "analyze_items.analyze_poultice_recipe" },
              "/ev",
              { "->": ".^.^.^.do_analyze" },
              "\n",
              { "#f": 5 },
            ],
            "c-7": [
              "^ ",
              "ev",
              { "^->": "analyze_items.analyze_beetle_carapace" },
              "/ev",
              { "->": ".^.^.^.do_analyze" },
              "\n",
              { "#f": 5 },
            ],
            "c-8": [
              "^ ",
              "ev",
              { "^->": "analyze_items.analyze_adrenaline_shot" },
              "/ev",
              { "->": ".^.^.^.do_analyze" },
              "\n",
              { "#f": 5 },
            ],
            "c-9": ["\n", { "->": "scene_3_choices" }, { "#f": 5 }],
          },
        ],
        {
          do_analyze: [
            { "temp=": "item_specific_text" },
            { "->": "item_specific_text", var: true },
            "ev",
            1,
            4,
            "rnd",
            "/ev",
            { "temp=": "stat_roll" },
            "\n",
            [
              "ev",
              { "VAR?": "stat_roll" },
              1,
              "==",
              "/ev",
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  { "VAR?": "strength" },
                  1,
                  "+",
                  { "VAR=": "strength", re: true },
                  "/ev",
                  "^<i>Your Strength has permanently increased by 1!</i>",
                  "\n",
                  { "->": ".^.^.^.13" },
                  null,
                ],
              },
            ],
            [
              "ev",
              { "VAR?": "stat_roll" },
              2,
              "==",
              "/ev",
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  { "VAR?": "intelligence" },
                  1,
                  "+",
                  { "VAR=": "intelligence", re: true },
                  "/ev",
                  "^<i>Your Intelligence has permanently increased by 1!</i>",
                  "\n",
                  { "->": ".^.^.^.13" },
                  null,
                ],
              },
            ],
            [
              "ev",
              { "VAR?": "stat_roll" },
              3,
              "==",
              "/ev",
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  { "VAR?": "agility" },
                  1,
                  "+",
                  { "VAR=": "agility", re: true },
                  "/ev",
                  "^<i>Your Agility has permanently increased by 1!</i>",
                  "\n",
                  { "->": ".^.^.^.13" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  "ev",
                  { "VAR?": "perception" },
                  1,
                  "+",
                  { "VAR=": "perception", re: true },
                  "/ev",
                  "^<i>Your Perception has permanently increased by 1!</i>",
                  "\n",
                  { "->": ".^.^.^.13" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "f()": "update_combat_stats" },
            "pop",
            "/ev",
            "\n",
            { "->": ".^.^" },
            { "#f": 1 },
          ],
          analyze_power_cell: [
            "ev",
            true,
            "/ev",
            { "VAR=": "analyzed_power_cell_type", re: true },
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Kaelen",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You tap the power cell against a metal strut. It sparks weakly. Looks like it has a little juice, but it feels unstable. Probably best not to hit it too hard.",
                  "\n",
                  { "->": ".^.^.^.12" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Aris",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  '^You run your multi-tool over the power cell. <i>AI: "Lithium-ion architecture, heavily degraded. Output is unstable, fluctuating between 3 and 19 volts. Could be repurposed for a directed electromagnetic pulse, with a moderate chance of catastrophic failure."</i> Fascinating.',
                  "\n",
                  { "->": ".^.^.^.22" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Lena",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^It's a standard power cell, the kind used in old maintenance drones. Heavy. You notice a small crack in the casing near the positive terminal. It might be volatile.",
                  "\n",
                  { "->": ".^.^.^.32" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "->": ".^.^.grant_random_stat_boost" },
            { "#f": 3 },
          ],
          analyze_moss: [
            "ev",
            true,
            "/ev",
            { "VAR=": "analyzed_moss_type", re: true },
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Kaelen",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You rub the moss between your fingers. It's cool to the touch and leaves a faint glowing residue. Doesn't seem very useful, but it smells... sweet.",
                  "\n",
                  { "->": ".^.^.^.12" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Aris",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  '^You analyze the sample. <i>AI: "Fungal sample contains a unique bioluminescent enzyme and a mild coagulant. Spores are airborne. High probability of attracting local fauna."</i> Useful. The coagulant could be refined into a basic healing agent.',
                  "\n",
                  { "->": ".^.^.^.22" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Lena",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^The moss glows, but the light is faint. You recall seeing similar fungi in deep-cave infiltration missions. The spores are light enough to travel on air currents; anything that hunts by scent would be drawn to this.",
                  "\n",
                  { "->": ".^.^.^.32" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "->": ".^.^.grant_random_stat_boost" },
            { "#f": 3 },
          ],
          analyze_neuro_stim: [
            "ev",
            true,
            "/ev",
            { "VAR=": "analyzed_neuro_stim_type", re: true },
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Kaelen",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You study the injector. It's military-grade, designed for battlefield medics. Understanding its simple, rugged construction gives you an idea of how to better maintain your own gear.",
                  "\n",
                  { "->": ".^.^.^.12" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Aris",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You recognize the chemical compound in the vial. It's a complex cocktail of synaptic enhancers. Just tracing the molecular structure in your mind feels like a complex and rewarding puzzle.",
                  "\n",
                  { "->": ".^.^.^.22" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Lena",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You've seen stims like this before, used by extraction targets who needed to be alert. Deconstructing how it works gives you a better understanding of how to counter their effects... or use them to your advantage.",
                  "\n",
                  { "->": ".^.^.^.32" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "->": ".^.^.grant_random_stat_boost" },
            { "#f": 3 },
          ],
          analyze_venom_gland: [
            "ev",
            true,
            "/ev",
            { "VAR=": "analyzed_venom_gland_type", re: true },
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Kaelen",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You handle the pulsating sac carefully. It's a reminder of a tough fight, a trophy. Understanding what it took to defeat the creature that grew this makes you feel stronger.",
                  "\n",
                  { "->": ".^.^.^.12" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Aris",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^Fascinating! The neurotoxin is incredibly complex. Analyzing how it attacks the nervous system gives you a flash of insight into how to better protect your own.",
                  "\n",
                  { "->": ".^.^.^.22" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Lena",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You study the gland, noting its weak points and how quickly the venom degrades when exposed to air. Knowledge of an enemy's biology is a weapon in itself.",
                  "\n",
                  { "->": ".^.^.^.32" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "->": ".^.^.grant_random_stat_boost" },
            { "#f": 3 },
          ],
          analyze_poison_vial: [
            "ev",
            true,
            "/ev",
            { "VAR=": "analyzed_poison_vial_type", re: true },
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Kaelen",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^It's a simple weapon, but effective. Thinking about how to apply it in a fight makes you reconsider your own combat techniques, looking for new openings.",
                  "\n",
                  { "->": ".^.^.^.12" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Aris",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^Your refined poison is stable and potent. Reviewing your own work, you see a slight improvement you could make to the formula, a small but satisfying intellectual victory.",
                  "\n",
                  { "->": ".^.^.^.22" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Lena",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You practice handling the vial, figuring out the fastest way to apply it to a blade or an arrow without making a sound. The economy of motion is a skill you're always refining.",
                  "\n",
                  { "->": ".^.^.^.32" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "->": ".^.^.grant_random_stat_boost" },
            { "#f": 3 },
          ],
          analyze_poultice_recipe: [
            "ev",
            true,
            "/ev",
            { "VAR=": "analyzed_poultice_recipe_type", re: true },
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Kaelen",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^Jed's knowledge is old-world, practical. It's not about complex science, but about knowing the land. Thinking about that simple, effective wisdom makes you feel more grounded.",
                  "\n",
                  { "->": ".^.^.^.12" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Aris",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You analyze the chemical properties of the weed Jed pointed out. He was right—it contains a mild sedative. The folk knowledge is surprisingly accurate, a fascinating data point.",
                  "\n",
                  { "->": ".^.^.^.22" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Lena",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^A recipe based on observation, not technology. It's a reminder that the environment itself can be a tool, or an arsenal. A valuable lesson in situational awareness.",
                  "\n",
                  { "->": ".^.^.^.32" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "->": ".^.^.grant_random_stat_boost" },
            { "#f": 3 },
          ],
          analyze_beetle_carapace: [
            "ev",
            true,
            "/ev",
            { "VAR=": "analyzed_beetle_carapace_type", re: true },
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Kaelen",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^The carapace is incredibly dense, a natural shield. Studying how the layers are formed gives you ideas on how to better brace for impact in a fight.",
                  "\n",
                  { "->": ".^.^.^.12" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Aris",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^The chitinous plating is a marvel of bio-engineering. The way it distributes kinetic force is incredibly efficient. You've learned something new about structural integrity.",
                  "\n",
                  { "->": ".^.^.^.22" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Lena",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You examine the joints in the plating, the places where movement is possible. Understanding its weak points is just as important as understanding its strengths.",
                  "\n",
                  { "->": ".^.^.^.32" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "->": ".^.^.grant_random_stat_boost" },
            { "#f": 3 },
          ],
          analyze_adrenaline_shot: [
            "ev",
            true,
            "/ev",
            { "VAR=": "analyzed_adrenaline_shot_type", re: true },
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Kaelen",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^A shot of pure adrenaline. A soldier's tool for a last stand. Just holding it makes your heart beat faster, your muscles tense in anticipation.",
                  "\n",
                  { "->": ".^.^.^.12" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Aris",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^It's a crude but powerful stimulant. Analyzing its effect on the human body reminds you of the delicate balance of your own biochemistry, and how easily it can be pushed.",
                  "\n",
                  { "->": ".^.^.^.22" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Lena",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "^You know what this is for: pushing past your limits when escape is the only option. Thinking about the desperate situations that would require it sharpens your survival instincts.",
                  "\n",
                  { "->": ".^.^.^.32" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "->": ".^.^.grant_random_stat_boost" },
            { "#f": 3 },
          ],
          grant_random_stat_boost: [
            "ev",
            1,
            4,
            "rnd",
            "/ev",
            { "temp=": "stat_roll" },
            "\n",
            [
              "ev",
              { "VAR?": "stat_roll" },
              1,
              "==",
              "/ev",
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  { "VAR?": "strength" },
                  1,
                  "+",
                  { "VAR=": "strength", re: true },
                  "/ev",
                  "^<i>Your Strength has permanently increased by 1!</i>",
                  "\n",
                  { "->": ".^.^.^.11" },
                  null,
                ],
              },
            ],
            [
              "ev",
              { "VAR?": "stat_roll" },
              2,
              "==",
              "/ev",
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  { "VAR?": "intelligence" },
                  1,
                  "+",
                  { "VAR=": "intelligence", re: true },
                  "/ev",
                  "^<i>Your Intelligence has permanently increased by 1!</i>",
                  "\n",
                  { "->": ".^.^.^.11" },
                  null,
                ],
              },
            ],
            [
              "ev",
              { "VAR?": "stat_roll" },
              3,
              "==",
              "/ev",
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  "ev",
                  { "VAR?": "agility" },
                  1,
                  "+",
                  { "VAR=": "agility", re: true },
                  "/ev",
                  "^<i>Your Agility has permanently increased by 1!</i>",
                  "\n",
                  { "->": ".^.^.^.11" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  "ev",
                  { "VAR?": "perception" },
                  1,
                  "+",
                  { "VAR=": "perception", re: true },
                  "/ev",
                  "^<i>Your Perception has permanently increased by 1!</i>",
                  "\n",
                  { "->": ".^.^.^.11" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            "ev",
            { "f()": "update_combat_stats" },
            "pop",
            "/ev",
            "\n",
            { "->": ".^.^" },
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      battle_loop: [
        [
          "^You have ",
          "ev",
          { "VAR?": "hp" },
          "out",
          "/ev",
          "^/",
          "ev",
          { "VAR?": "max_hp" },
          "out",
          "/ev",
          "^ HP.",
          "\n",
          "ev",
          { "VAR?": "jed_status" },
          "str",
          "^HELPED",
          "/str",
          "==",
          { "VAR?": "jed_hp" },
          0,
          ">",
          "&&",
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "^Jed has ",
                "ev",
                { "VAR?": "jed_hp" },
                "out",
                "/ev",
                "^/",
                "ev",
                { "VAR?": "jed_max_hp" },
                "out",
                "/ev",
                "^ HP.",
                "\n",
                { "->": ".^.^.^.24" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "ev",
          { "VAR?": "current_enemy_hp" },
          0,
          ">",
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "^The ",
                "ev",
                { "VAR?": "current_enemy_name" },
                "out",
                "/ev",
                "^ has ",
                "ev",
                { "VAR?": "current_enemy_hp" },
                "out",
                "/ev",
                "^ HP.",
                "\n",
                { "->": ".^.^.^.32" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "ev",
          { "VAR?": "enemy2_hp" },
          0,
          ">",
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "^The ",
                "ev",
                { "VAR?": "enemy2_name" },
                "out",
                "/ev",
                "^ has ",
                "ev",
                { "VAR?": "enemy2_hp" },
                "out",
                "/ev",
                "^ HP.",
                "\n",
                { "->": ".^.^.^.40" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "ev",
          "str",
          "^Attack the first ",
          "ev",
          { "VAR?": "current_enemy_name" },
          "out",
          "/ev",
          "/str",
          { "VAR?": "current_enemy_hp" },
          0,
          ">",
          { "VAR?": "enemy2_hp" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-0", flg: 5 },
          "ev",
          "str",
          "^Attack the second ",
          "ev",
          { "VAR?": "enemy2_name" },
          "out",
          "/ev",
          "/str",
          { "VAR?": "current_enemy_hp" },
          0,
          ">",
          { "VAR?": "enemy2_hp" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-1", flg: 5 },
          "ev",
          "str",
          "^Attack the ",
          "ev",
          { "VAR?": "current_enemy_name" },
          "out",
          "/ev",
          "/str",
          { "VAR?": "current_enemy_hp" },
          0,
          ">",
          { "VAR?": "enemy2_hp" },
          0,
          "<=",
          "&&",
          "/ev",
          { "*": ".^.c-2", flg: 5 },
          "ev",
          "str",
          "^Attack the ",
          "ev",
          { "VAR?": "enemy2_name" },
          "out",
          "/ev",
          "/str",
          { "VAR?": "current_enemy_hp" },
          0,
          "<=",
          { "VAR?": "enemy2_hp" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-3", flg: 5 },
          "ev",
          "str",
          "^Defend",
          "/str",
          "/ev",
          { "*": ".^.c-4", flg: 4 },
          "ev",
          "str",
          "^Slot a Power Cell into the Club (",
          "ev",
          { "VAR?": "club_power_slots" },
          "out",
          "/ev",
          "^ slots left)",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Kaelen",
          "/str",
          "==",
          { "VAR?": "club_is_upgraded" },
          "&&",
          { "VAR?": "club_is_charged" },
          "!",
          "&&",
          { "VAR?": "club_power_slots" },
          0,
          ">",
          "&&",
          { "VAR?": "power_cell_stack" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-5", flg: 5 },
          "ev",
          "str",
          "^Use Kinetic Emitter (",
          "ev",
          { "VAR?": "emitter_charges" },
          "out",
          "/ev",
          "^ left)",
          "/str",
          { "VAR?": "emitter_equipped" },
          { "VAR?": "emitter_charges" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-6", flg: 5 },
          "ev",
          "str",
          "^Use Skill",
          "/str",
          { "VAR?": "used_skill_in_battle" },
          "!",
          "/ev",
          { "*": ".^.c-7", flg: 5 },
          "ev",
          "str",
          "^Use Moss Poison (",
          "ev",
          { "VAR?": "has_moss_poison_vial" },
          "out",
          "/ev",
          "^ left)",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Aris",
          "/str",
          "==",
          { "VAR?": "has_moss_poison_vial" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-8", flg: 5 },
          "ev",
          "str",
          "^Use Poison Bomb",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Aris",
          "/str",
          "==",
          { "VAR?": "poison_bomb_stack" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-9", flg: 5 },
          "ev",
          "str",
          "^Use EMP Grenade",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Aris",
          "/str",
          "==",
          { "VAR?": "has_emp_grenade" },
          "&&",
          "/ev",
          { "*": ".^.c-10", flg: 5 },
          "ev",
          "str",
          "^Fire a Scrap Arrow (",
          "ev",
          { "VAR?": "scrap_arrow_count" },
          "out",
          "/ev",
          "^ left)",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Lena",
          "/str",
          "==",
          { "VAR?": "scrap_arrow_count" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-11", flg: 5 },
          "ev",
          "str",
          "^Fire a Silent Arrow (",
          "ev",
          { "VAR?": "silent_arrow_count" },
          "out",
          "/ev",
          "^ left)",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Lena",
          "/str",
          "==",
          { "VAR?": "silent_arrow_count" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-12", flg: 5 },
          "ev",
          "str",
          "^Fire a Shock Arrow (",
          "ev",
          { "VAR?": "shock_arrow_count" },
          "out",
          "/ev",
          "^ left)",
          "/str",
          { "VAR?": "character_name" },
          "str",
          "^Lena",
          "/str",
          "==",
          { "VAR?": "shock_arrow_count" },
          0,
          ">",
          "&&",
          "/ev",
          { "*": ".^.c-13", flg: 5 },
          "ev",
          "str",
          "^Run Away",
          "/str",
          "/ev",
          { "*": ".^.c-14", flg: 4 },
          {
            "c-0": [
              "\n",
              "ev",
              { "^->": "jed_turn" },
              false,
              "/ev",
              { "->": "player_attack" },
              { "#f": 5 },
            ],
            "c-1": [
              "\n",
              "ev",
              { "^->": "jed_turn" },
              true,
              "/ev",
              { "->": "player_attack" },
              { "#f": 5 },
            ],
            "c-2": [
              "\n",
              "ev",
              { "^->": "jed_turn" },
              false,
              "/ev",
              { "->": "player_attack" },
              { "#f": 5 },
            ],
            "c-3": [
              "\n",
              "ev",
              { "^->": "jed_turn" },
              true,
              "/ev",
              { "->": "player_attack" },
              { "#f": 5 },
            ],
            "c-4": [
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "is_defending", re: true },
              "^You brace for an attack, increasing your defense for this turn.",
              "\n",
              { "->": "jed_turn" },
              { "#f": 5 },
            ],
            "c-5": ["\n", { "->": "kaelen_charge_club" }, { "#f": 5 }],
            "c-6": ["\n", { "->": "player_use_emitter" }, { "#f": 5 }],
            "c-7": ["\n", { "->": "battle_use_skill" }, { "#f": 5 }],
            "c-8": ["\n", { "->": "player_use_moss_poison" }, { "#f": 5 }],
            "c-9": ["\n", { "->": "player_use_poison_bomb" }, { "#f": 5 }],
            "c-10": ["\n", { "->": "player_use_emp_grenade" }, { "#f": 5 }],
            "c-11": ["\n", { "->": "player_fire_scrap_arrow" }, { "#f": 5 }],
            "c-12": ["\n", { "->": "player_fire_silent_arrow" }, { "#f": 5 }],
            "c-13": ["\n", { "->": "player_fire_shock_arrow" }, { "#f": 5 }],
            "c-14": ["\n", { "->": "battle_fled" }, { "#f": 5 }],
          },
        ],
        { "#f": 1 },
      ],
      player_attack: [
        { "temp=": "is_second_enemy" },
        { "temp=": "return_point" },
        "ev",
        0,
        "/ev",
        { "temp=": "target_hp" },
        "ev",
        0,
        "/ev",
        { "temp=": "target_def" },
        "ev",
        "str",
        "^",
        "/str",
        "/ev",
        { "temp=": "target_name" },
        "ev",
        { "VAR?": "is_second_enemy" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "enemy2_hp" },
              "/ev",
              { "temp=": "target_hp", re: true },
              "ev",
              { "VAR?": "enemy2_def" },
              "/ev",
              { "temp=": "target_def", re: true },
              "ev",
              { "VAR?": "enemy2_name" },
              "/ev",
              { "temp=": "target_name", re: true },
              { "->": ".^.^.^.21" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "current_enemy_hp" },
              "/ev",
              { "temp=": "target_hp", re: true },
              "ev",
              { "VAR?": "current_enemy_def" },
              "/ev",
              { "temp=": "target_def", re: true },
              "ev",
              { "VAR?": "current_enemy_name" },
              "/ev",
              { "temp=": "target_name", re: true },
              { "->": ".^.^.^.21" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        1,
        100,
        "rnd",
        "/ev",
        { "temp=": "enemy_dodge_roll" },
        "\n",
        "ev",
        { "VAR?": "enemy_dodge_roll" },
        15,
        "<=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^You lunge, but the ",
              "ev",
              { "VAR?": "target_name" },
              "out",
              "/ev",
              "^ is surprisingly quick, sidestepping your attack. It misses!",
              "\n",
              { "->": "return_point", var: true },
              { "->": ".^.^.^.36" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        8,
        12,
        "rnd",
        10.0,
        "/",
        "/ev",
        { "temp=": "p_multiplier" },
        "\n",
        "ev",
        { "VAR?": "atk" },
        { "VAR?": "target_def" },
        "-",
        "/ev",
        { "temp=": "damage" },
        "ev",
        { "VAR?": "damage" },
        1,
        "<",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              1,
              "/ev",
              { "temp=": "damage", re: true },
              { "->": ".^.^.^.59" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "damage" },
        { "VAR?": "p_multiplier" },
        "*",
        "INT",
        "/ev",
        { "temp=": "final_dmg" },
        "\n",
        "ev",
        { "VAR?": "club_is_charged" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              false,
              "/ev",
              { "VAR=": "club_is_charged", re: true },
              "^You swing the energized club in a wide, brutal arc. It connects with a deafening CRACK of displaced energy!",
              "\n",
              "ev",
              { "VAR?": "final_dmg" },
              8,
              "+",
              { "temp=": "final_dmg", re: true },
              "/ev",
              { "->": ".^.^.^.73" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "target_hp" },
        { "VAR?": "final_dmg" },
        "-",
        { "temp=": "target_hp", re: true },
        "/ev",
        "ev",
        { "VAR?": "is_second_enemy" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "target_hp" },
              "/ev",
              { "VAR=": "enemy2_hp", re: true },
              { "->": ".^.^.^.86" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "target_hp" },
              "/ev",
              { "VAR=": "current_enemy_hp", re: true },
              { "->": ".^.^.^.86" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "^You attack the ",
        "ev",
        { "VAR?": "target_name" },
        "out",
        "/ev",
        "^ for ",
        "ev",
        { "VAR?": "final_dmg" },
        "out",
        "/ev",
        "^ damage!",
        "\n",
        "ev",
        { "VAR?": "target_hp" },
        0,
        "<=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^The ",
              "ev",
              { "VAR?": "target_name" },
              "out",
              "/ev",
              "^ collapses, defeated!",
              "\n",
              { "->": ".^.^.^.106" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "target_name" },
        { "VAR?": "current_enemy_name" },
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              0,
              "/ev",
              { "VAR=": "current_enemy_hp", re: true },
              { "->": ".^.^.^.115" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "ev",
              0,
              "/ev",
              { "VAR=": "enemy2_hp", re: true },
              { "->": ".^.^.^.115" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "->": "return_point", var: true },
        { "#f": 1 },
      ],
      kaelen_charge_club: [
        "^You jam a Degraded Power Cell into one of the crude slots on your club. It sparks violently, and the weapon hums with a dangerous, unstable energy. Your next attack will be devastating.",
        "\n",
        "ev",
        { "VAR?": "power_cell_stack" },
        1,
        "-",
        { "VAR=": "power_cell_stack", re: true },
        "/ev",
        "ev",
        { "VAR?": "club_power_slots" },
        1,
        "-",
        { "VAR=": "club_power_slots", re: true },
        "/ev",
        "ev",
        true,
        "/ev",
        { "VAR=": "club_is_charged", re: true },
        { "->": "jed_turn" },
        { "#f": 1 },
      ],
      player_fire_shock_arrow: [
        "ev",
        { "VAR?": "shock_arrow_count" },
        1,
        "-",
        { "VAR=": "shock_arrow_count", re: true },
        "/ev",
        "^The arrow crackles with stored energy as you loose it. It hits the ",
        "ev",
        { "VAR?": "current_enemy_name" },
        "out",
        "/ev",
        "^ with a shower of sparks, delivering a powerful electric shock.",
        "\n",
        "ev",
        12,
        "/ev",
        { "temp=": "arrow_damage" },
        "ev",
        { "VAR?": "current_enemy_hp" },
        { "VAR?": "arrow_damage" },
        "-",
        { "VAR=": "current_enemy_hp", re: true },
        "/ev",
        "^It hits for ",
        "ev",
        { "VAR?": "arrow_damage" },
        "out",
        "/ev",
        "^ damage!",
        "\n",
        "ev",
        { "VAR?": "current_enemy_hp" },
        0,
        "<=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          { b: ["\n", { "->": "battle_won" }, { "->": ".^.^.^.37" }, null] },
        ],
        [
          { "->": ".^.b" },
          { b: ["\n", { "->": "jed_turn" }, { "->": ".^.^.^.37" }, null] },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      player_fire_scrap_arrow: [
        "ev",
        { "VAR?": "scrap_arrow_count" },
        1,
        "-",
        { "VAR=": "scrap_arrow_count", re: true },
        "/ev",
        "^You loose a crudely made arrow. It flies true, striking the ",
        "ev",
        { "VAR?": "current_enemy_name" },
        "out",
        "/ev",
        "^ for moderate damage.",
        "\n",
        "ev",
        5,
        "/ev",
        { "temp=": "arrow_damage" },
        "ev",
        { "VAR?": "current_enemy_hp" },
        { "VAR?": "arrow_damage" },
        "-",
        { "VAR=": "current_enemy_hp", re: true },
        "/ev",
        "^It hits for ",
        "ev",
        { "VAR?": "arrow_damage" },
        "out",
        "/ev",
        "^ damage!",
        "\n",
        "ev",
        { "VAR?": "current_enemy_hp" },
        0,
        "<=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          { b: ["\n", { "->": "battle_won" }, { "->": ".^.^.^.37" }, null] },
        ],
        [
          { "->": ".^.b" },
          { b: ["\n", { "->": "jed_turn" }, { "->": ".^.^.^.37" }, null] },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      player_fire_silent_arrow: [
        "ev",
        { "VAR?": "silent_arrow_count" },
        1,
        "-",
        { "VAR=": "silent_arrow_count", re: true },
        "/ev",
        "^The silent arrow whispers through the air, finding a weak point in the ",
        "ev",
        { "VAR?": "current_enemy_name" },
        "out",
        "/ev",
        "^'s defense. A critical hit!",
        "\n",
        "ev",
        9,
        "/ev",
        { "temp=": "arrow_damage" },
        "ev",
        { "VAR?": "current_enemy_hp" },
        { "VAR?": "arrow_damage" },
        "-",
        { "VAR=": "current_enemy_hp", re: true },
        "/ev",
        "^It hits for ",
        "ev",
        { "VAR?": "arrow_damage" },
        "out",
        "/ev",
        "^ damage!",
        "\n",
        "ev",
        { "VAR?": "current_enemy_hp" },
        0,
        "<=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          { b: ["\n", { "->": "battle_won" }, { "->": ".^.^.^.37" }, null] },
        ],
        [
          { "->": ".^.b" },
          { b: ["\n", { "->": "jed_turn" }, { "->": ".^.^.^.37" }, null] },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      player_use_moss_poison: [
        "ev",
        { "VAR?": "has_moss_poison_vial" },
        1,
        "-",
        { "VAR=": "has_moss_poison_vial", re: true },
        "/ev",
        "^You coat a small dart with the paralytic poison.",
        "\n",
        "ev",
        { "VAR?": "enemy2_hp" },
        0,
        ">",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              "str",
              "^Target the first ",
              "ev",
              { "VAR?": "current_enemy_name" },
              "out",
              "/ev",
              "^.",
              "/str",
              "/ev",
              { "*": ".^.c-0", flg: 20 },
              "ev",
              "str",
              "^Target the second ",
              "ev",
              { "VAR?": "enemy2_name" },
              "out",
              "/ev",
              "^.",
              "/str",
              "/ev",
              { "*": ".^.c-1", flg: 20 },
              { "->": ".^.^.^.15" },
              {
                "c-0": [
                  "\n",
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "enemy_is_poisoned", re: true },
                  "ev",
                  3,
                  "/ev",
                  { "VAR=": "poison_turns_remaining", re: true },
                  "^The dart finds its mark. The first ",
                  "ev",
                  { "VAR?": "current_enemy_name" },
                  "out",
                  "/ev",
                  "^ is now poisoned!",
                  "\n",
                  { "->": "jed_turn" },
                  { "#f": 5 },
                ],
                "c-1": [
                  "\n",
                  "ev",
                  true,
                  "/ev",
                  { "VAR=": "enemy2_is_poisoned", re: true },
                  "ev",
                  3,
                  "/ev",
                  { "VAR=": "poison2_turns_remaining", re: true },
                  "^The dart finds its mark. The second ",
                  "ev",
                  { "VAR?": "enemy2_name" },
                  "out",
                  "/ev",
                  "^ is now poisoned!",
                  "\n",
                  { "->": "jed_turn" },
                  { "#f": 5 },
                ],
              },
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "^The ",
              "ev",
              { "VAR?": "current_enemy_name" },
              "out",
              "/ev",
              "^ is now poisoned!",
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "enemy_is_poisoned", re: true },
              "ev",
              3,
              "/ev",
              { "VAR=": "poison_turns_remaining", re: true },
              { "->": "jed_turn" },
              { "->": ".^.^.^.15" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      player_use_poison_bomb: [
        "ev",
        { "VAR?": "poison_bomb_stack" },
        1,
        "-",
        { "VAR=": "poison_bomb_stack", re: true },
        "/ev",
        "^The bomb shatters, releasing a cloud of aerosolized neurotoxin that engulfs all enemies!",
        "\n",
        "ev",
        { "VAR?": "atk" },
        3,
        "*",
        "/ev",
        { "temp=": "bomb_damage" },
        "^The ",
        "ev",
        { "VAR?": "current_enemy_name" },
        "out",
        "/ev",
        "^ takes ",
        "ev",
        { "VAR?": "bomb_damage" },
        "out",
        "/ev",
        "^ initial damage!",
        "\n",
        "ev",
        { "VAR?": "current_enemy_hp" },
        { "VAR?": "bomb_damage" },
        "-",
        { "VAR=": "current_enemy_hp", re: true },
        "/ev",
        "ev",
        true,
        "/ev",
        { "VAR=": "enemy_is_poisoned", re: true },
        "ev",
        5,
        "/ev",
        { "VAR=": "poison_turns_remaining", re: true },
        "ev",
        { "VAR?": "enemy2_hp" },
        0,
        ">",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^The ",
              "ev",
              { "VAR?": "enemy2_name" },
              "out",
              "/ev",
              "^ also takes ",
              "ev",
              { "VAR?": "bomb_damage" },
              "out",
              "/ev",
              "^ initial damage!",
              "\n",
              "ev",
              { "VAR?": "enemy2_hp" },
              { "VAR?": "bomb_damage" },
              "-",
              { "VAR=": "enemy2_hp", re: true },
              "/ev",
              "ev",
              true,
              "/ev",
              { "VAR=": "enemy2_is_poisoned", re: true },
              "ev",
              5,
              "/ev",
              { "VAR=": "poison2_turns_remaining", re: true },
              { "->": ".^.^.^.46" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "current_enemy_hp" },
        0,
        "<=",
        { "VAR?": "enemy2_hp" },
        0,
        "<=",
        { "VAR?": "enemy2_name" },
        "str",
        "^",
        "/str",
        "==",
        "||",
        "&&",
        "/ev",
        [
          { "->": ".^.b", c: true },
          { b: ["\n", { "->": "battle_won" }, { "->": ".^.^.^.65" }, null] },
        ],
        [
          { "->": ".^.b" },
          { b: ["\n", { "->": "jed_turn" }, { "->": ".^.^.^.65" }, null] },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      player_use_emp_grenade: [
        "ev",
        false,
        "/ev",
        { "VAR=": "has_emp_grenade", re: true },
        "^You hurl the jury-rigged power cell. It detonates with a sharp crackle and a flash of blue light, releasing a powerful electromagnetic pulse. The ",
        "ev",
        { "VAR?": "current_enemy_name" },
        "out",
        "/ev",
        "^ seizes up, disoriented by the assault on its nervous system. It will be stunned for its next turn.",
        "\n",
        "ev",
        true,
        "/ev",
        { "VAR=": "rival_will_miss_next_turn", re: true },
        "ev",
        { "VAR?": "enemy2_hp" },
        0,
        ">",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "enemy2_will_miss_next_turn", re: true },
              { "->": ".^.^.^.21" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "->": "jed_turn" },
        { "#f": 1 },
      ],
      battle_fled: [
        "ev",
        false,
        "/ev",
        { "VAR=": "used_skill_in_battle", re: true },
        "^You take a chance and disengage, turning to flee. The ",
        "ev",
        { "VAR?": "current_enemy_name" },
        "out",
        "/ev",
        "^ lets out a cry of frustration but doesn't pursue.",
        "\n",
        "ev",
        { "VAR?": "resolve" },
        5,
        "-",
        { "VAR=": "resolve", re: true },
        "/ev",
        [
          "ev",
          { "VAR?": "current_enemy_name" },
          "str",
          "^Slick-Skinned Skulker",
          "/str",
          "==",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^You've escaped, but you've lost your chance to achieve your objective here.",
              "\n",
              '^<i>AI: "Subject has disengaged. Data Fragment unretrievable."</i>',
              "\n",
              { "->": "scene_8_the_tower" },
              { "->": ".^.^.^.21" },
              null,
            ],
          },
        ],
        [
          "ev",
          { "VAR?": "current_enemy_name" },
          "str",
          "^The Brute",
          "/str",
          "==",
          { "VAR?": "current_enemy_name" },
          "str",
          "^The Tinkerer",
          "/str",
          "==",
          "||",
          { "VAR?": "current_enemy_name" },
          "str",
          "^The Veteran",
          "/str",
          "==",
          "||",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^You're forced back down the stairs, floor by floor, until you're back at the base of the communications spire. The contestant you fled from now holds the high ground, and there's no way back up. You've failed this test.",
              "\n",
              { "->": "scene_9_the_bargain" },
              { "->": ".^.^.^.21" },
              null,
            ],
          },
        ],
        [
          "ev",
          { "VAR?": "current_enemy_name" },
          "str",
          "^Skulker Guard",
          "/str",
          "==",
          { "VAR?": "current_enemy_name" },
          "str",
          "^Skulker Packmate",
          "/str",
          "==",
          "||",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^The pack is too thick. You are forced back from the main platform, swallowed by the horde. You've lost your chance to face the Alpha and prove your worth.",
              "\n",
              { "->": "chapter_1_failure_ending" },
              { "->": ".^.^.^.21" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "^You escape, but the opportunity is lost.",
              "\n",
              { "->": "scene_9_the_bargain" },
              { "->": ".^.^.^.21" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      battle_use_skill: [
        [
          "ev",
          true,
          "/ev",
          { "VAR=": "used_skill_in_battle", re: true },
          "ev",
          { "VAR?": "player_skills" },
          { "VAR?": "Survivalist" },
          "?",
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "^You roar, focusing your rage into a single, powerful strike. Your Attack is temporarily increased!",
                "\n",
                "ev",
                { "VAR?": "atk" },
                2,
                "+",
                { "VAR=": "atk", re: true },
                "/ev",
                "ev",
                { "^->": "jed_turn" },
                false,
                "/ev",
                { "->": "player_attack" },
                { "->": ".^.^.^.10" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "ev",
          { "VAR?": "player_skills" },
          { "VAR?": "BioScan" },
          "?",
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "^You activate your bio-scanner, identifying a weak point in the creature's hide. Its Defense is lowered.",
                "\n",
                "ev",
                { "VAR?": "current_enemy_def" },
                2,
                "-",
                { "VAR=": "current_enemy_def", re: true },
                "/ev",
                "ev",
                { "VAR?": "current_enemy_def" },
                0,
                "<",
                "/ev",
                [
                  { "->": ".^.b", c: true },
                  {
                    b: [
                      "\n",
                      "ev",
                      0,
                      "/ev",
                      { "VAR=": "current_enemy_def", re: true },
                      { "->": ".^.^.^.15" },
                      null,
                    ],
                  },
                ],
                "nop",
                "\n",
                { "->": "jed_turn" },
                { "->": ".^.^.^.18" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "ev",
          { "VAR?": "player_skills" },
          { "VAR?": "DiscerningEye" },
          "?",
          "/ev",
          [
            { "->": ".^.b", c: true },
            {
              b: [
                "\n",
                "^You watch the opponent's movements, predicting its lunge. You'll be able to easily dodge its next attac.",
                "\n",
                "ev",
                true,
                "/ev",
                { "VAR=": "rival_will_miss_next_turn", re: true },
                { "->": "jed_turn" },
                { "->": ".^.^.^.26" },
                null,
              ],
            },
          ],
          "nop",
          "\n",
          "ev",
          "str",
          "^Use Heavy Hitter",
          "/str",
          { "VAR?": "player_skills" },
          { "VAR?": "HeavyHitter" },
          "?",
          "/ev",
          { "*": ".^.c-0", flg: 21 },
          "ev",
          "str",
          "^Use Overcharge",
          "/str",
          { "VAR?": "player_skills" },
          { "VAR?": "Overcharge" },
          "?",
          "/ev",
          { "*": ".^.c-1", flg: 21 },
          "ev",
          "str",
          "^Prepare to Counter Attack",
          "/str",
          { "VAR?": "player_skills" },
          { "VAR?": "CounterAttack" },
          "?",
          "/ev",
          { "*": ".^.c-2", flg: 21 },
          "ev",
          "str",
          "^Use Targeted Analysis on Jed",
          "/str",
          { "VAR?": "player_skills" },
          { "VAR?": "TargetedAnalysis" },
          "?",
          { "VAR?": "jed_status" },
          "str",
          "^HELPED",
          "/str",
          "==",
          "&&",
          { "VAR?": "jed_hp" },
          0,
          ">",
          "&&",
          { "VAR?": "jed_is_buffed" },
          "!",
          "&&",
          "/ev",
          { "*": ".^.c-3", flg: 21 },
          {
            "c-0": [
              "\n",
              "^You channel all your strength into a single, devastating blow. It's slow, but powerful.",
              "\n",
              "ev",
              { "VAR?": "atk" },
              1.5,
              "*",
              "INT",
              "/ev",
              { "temp=": "heavy_damage" },
              "\n",
              "ev",
              { "VAR?": "current_enemy_hp" },
              { "VAR?": "heavy_damage" },
              "-",
              { "VAR=": "current_enemy_hp", re: true },
              "/ev",
              "^You slam the ",
              "ev",
              { "VAR?": "current_enemy_name" },
              "out",
              "/ev",
              "^ for ",
              "ev",
              { "VAR?": "heavy_damage" },
              "out",
              "/ev",
              "^ damage!",
              "\n",
              "ev",
              { "VAR?": "current_enemy_hp" },
              0,
              "<=",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    { "->": "battle_won" },
                    { "->": ".^.^.^.36" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: ["\n", { "->": "jed_turn" }, { "->": ".^.^.^.36" }, null],
                },
              ],
              "nop",
              "\n",
              { "#f": 5 },
            ],
            "c-1": [
              "\n",
              "^You reroute power through the Kinetic Emitter, preparing to unleash an overloaded blast. Your next Emitter use will be massively amplified.",
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "is_overcharging", re: true },
              { "->": "jed_turn" },
              { "#f": 5 },
            ],
            "c-2": [
              "\n",
              "^You take a defensive stance, watching your enemy's every move, ready to strike the moment they commit to an attack.",
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "is_countering", re: true },
              { "->": "jed_turn" },
              { "#f": 5 },
            ],
            "c-3": [
              "\n",
              '^You analyze the enemy\'s movements and call out a weak point to Jed. "Jed, the left side! Now!"',
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "jed_is_buffed", re: true },
              { "->": "jed_turn" },
              { "#f": 5 },
            ],
          },
        ],
        { "#f": 1 },
      ],
      player_use_emitter: [
        "ev",
        { "f()": "use_emitter_charge" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^You unleash a wave of pure force from the emitter. It slams into the ",
              "ev",
              { "VAR?": "current_enemy_name" },
              "out",
              "/ev",
              "^, sending it reeling.",
              "\n",
              "ev",
              15,
              "/ev",
              { "temp=": "emitter_damage" },
              "ev",
              { "VAR?": "is_overcharging" },
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    30,
                    "/ev",
                    { "temp=": "emitter_damage", re: true },
                    "ev",
                    false,
                    "/ev",
                    { "VAR=": "is_overcharging", re: true },
                    "^The Emitter shrieks as it discharges the excess energy.",
                    "\n",
                    { "->": ".^.^.^.16" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              { "VAR?": "current_enemy_hp" },
              { "VAR?": "emitter_damage" },
              "-",
              { "VAR=": "current_enemy_hp", re: true },
              "/ev",
              "^It hit for ",
              "ev",
              { "VAR?": "emitter_damage" },
              "out",
              "/ev",
              "^ damage!",
              "\n",
              { "->": ".^.^.^.4" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "current_enemy_hp" },
        0,
        "<=",
        "/ev",
        [
          { "->": ".^.b", c: true },
          { b: ["\n", { "->": "battle_won" }, { "->": ".^.^.^.13" }, null] },
        ],
        [
          { "->": ".^.b" },
          { b: ["\n", { "->": "jed_turn" }, { "->": ".^.^.^.13" }, null] },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      enemy_turn: [
        "ev",
        { "VAR?": "enemy_is_poisoned" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "poison_turns_remaining" },
              1,
              "-",
              { "VAR=": "poison_turns_remaining", re: true },
              "/ev",
              "ev",
              1,
              { "VAR?": "intelligence" },
              2,
              "/",
              "INT",
              "+",
              "/ev",
              { "temp=": "poison_damage" },
              "\n",
              "^The poison seeps into the ",
              "ev",
              { "VAR?": "current_enemy_name" },
              "out",
              "/ev",
              "^, dealing ",
              "ev",
              { "VAR?": "poison_damage" },
              "out",
              "/ev",
              "^ damage.",
              "\n",
              "ev",
              { "VAR?": "current_enemy_hp" },
              { "VAR?": "poison_damage" },
              "-",
              { "VAR=": "current_enemy_hp", re: true },
              "/ev",
              "ev",
              { "VAR?": "poison_turns_remaining" },
              0,
              "<=",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    false,
                    "/ev",
                    { "VAR=": "enemy_is_poisoned", re: true },
                    "^The poison on the ",
                    "ev",
                    { "VAR?": "current_enemy_name" },
                    "out",
                    "/ev",
                    "^ has run its course.",
                    "\n",
                    { "->": ".^.^.^.41" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.4" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "enemy2_is_poisoned" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "poison2_turns_remaining" },
              1,
              "-",
              { "VAR=": "poison2_turns_remaining", re: true },
              "/ev",
              "ev",
              1,
              { "VAR?": "intelligence" },
              2,
              "/",
              "INT",
              "+",
              "/ev",
              { "temp=": "poison_damage2" },
              "\n",
              "^The poison seeps into the ",
              "ev",
              { "VAR?": "enemy2_name" },
              "out",
              "/ev",
              "^, dealing ",
              "ev",
              { "VAR?": "poison_damage2" },
              "out",
              "/ev",
              "^ damage.",
              "\n",
              "ev",
              { "VAR?": "enemy2_hp" },
              { "VAR?": "poison_damage2" },
              "-",
              { "VAR=": "enemy2_hp", re: true },
              "/ev",
              "ev",
              { "VAR?": "poison2_turns_remaining" },
              0,
              "<=",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    false,
                    "/ev",
                    { "VAR=": "enemy2_is_poisoned", re: true },
                    "^The poison on the ",
                    "ev",
                    { "VAR?": "enemy2_name" },
                    "out",
                    "/ev",
                    "^ has run its course.",
                    "\n",
                    { "->": ".^.^.^.41" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.10" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "rival_will_miss_next_turn" },
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              false,
              "/ev",
              { "VAR=": "rival_will_miss_next_turn", re: true },
              "^You anticipate the enemy's clumsy attack and easily step aside. It misses completely.",
              "\n",
              { "->": "battle_loop" },
              { "->": ".^.^.^.17" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "def" },
              "/ev",
              { "temp=": "current_player_def" },
              "ev",
              { "VAR?": "is_defending" },
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    { "VAR?": "def" },
                    3,
                    "+",
                    "/ev",
                    { "temp=": "current_player_def", re: true },
                    { "->": ".^.^.^.9" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              { "VAR?": "current_enemy_hp" },
              0,
              ">",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^The ",
                    "ev",
                    { "VAR?": "current_enemy_name" },
                    "out",
                    "/ev",
                    "^ attacks!",
                    "\n",
                    "ev",
                    1,
                    100,
                    "rnd",
                    "/ev",
                    { "temp=": "player_dodge_roll" },
                    "\n",
                    "ev",
                    { "VAR?": "player_dodge_roll" },
                    { "VAR?": "dodge_chance" },
                    "<=",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "^You anticipate the attack and deftly move aside. It misses!",
                          "\n",
                          { "->": ".^.^.^.22" },
                          null,
                        ],
                      },
                    ],
                    [
                      { "->": ".^.b" },
                      {
                        b: [
                          "\n",
                          "ev",
                          { "VAR?": "jed_status" },
                          "str",
                          "^HELPED",
                          "/str",
                          "==",
                          { "VAR?": "jed_hp" },
                          0,
                          ">",
                          "&&",
                          "/ev",
                          [
                            { "->": ".^.b", c: true },
                            {
                              b: [
                                "\n",
                                "ev",
                                1,
                                2,
                                "rnd",
                                "/ev",
                                { "temp=": "target_roll" },
                                "\n",
                                [
                                  "ev",
                                  { "VAR?": "target_roll" },
                                  1,
                                  "==",
                                  "/ev",
                                  { "->": ".^.b", c: true },
                                  {
                                    b: [
                                      "\n",
                                      "ev",
                                      8,
                                      12,
                                      "rnd",
                                      10.0,
                                      "/",
                                      "/ev",
                                      { "temp=": "p_multiplier" },
                                      "\n",
                                      "ev",
                                      { "VAR?": "current_enemy_atk" },
                                      { "VAR?": "current_player_def" },
                                      "-",
                                      "/ev",
                                      { "temp=": "damage" },
                                      "ev",
                                      { "VAR?": "damage" },
                                      1,
                                      "<",
                                      "/ev",
                                      [
                                        { "->": ".^.b", c: true },
                                        {
                                          b: [
                                            "\n",
                                            "ev",
                                            1,
                                            "/ev",
                                            { "temp=": "damage", re: true },
                                            { "->": ".^.^.^.22" },
                                            null,
                                          ],
                                        },
                                      ],
                                      "nop",
                                      "\n",
                                      "ev",
                                      { "VAR?": "damage" },
                                      { "VAR?": "p_multiplier" },
                                      "*",
                                      "INT",
                                      "/ev",
                                      { "temp=": "final_dmg" },
                                      "\n",
                                      "ev",
                                      { "VAR?": "hp" },
                                      { "VAR?": "final_dmg" },
                                      "-",
                                      { "VAR=": "hp", re: true },
                                      "/ev",
                                      "^It hits you for ",
                                      "ev",
                                      { "VAR?": "final_dmg" },
                                      "out",
                                      "/ev",
                                      "^ damage!",
                                      "\n",
                                      { "->": ".^.^.^.10" },
                                      null,
                                    ],
                                  },
                                ],
                                [
                                  { "->": ".^.b" },
                                  {
                                    b: [
                                      "\n",
                                      "ev",
                                      8,
                                      12,
                                      "rnd",
                                      10.0,
                                      "/",
                                      "/ev",
                                      { "temp=": "p_multiplier2" },
                                      "\n",
                                      "ev",
                                      { "VAR?": "current_enemy_atk" },
                                      { "VAR?": "jed_def" },
                                      "-",
                                      "/ev",
                                      { "temp=": "damage3" },
                                      "ev",
                                      { "VAR?": "damage3" },
                                      1,
                                      "<",
                                      "/ev",
                                      [
                                        { "->": ".^.b", c: true },
                                        {
                                          b: [
                                            "\n",
                                            "ev",
                                            1,
                                            "/ev",
                                            { "temp=": "damage3", re: true },
                                            { "->": ".^.^.^.22" },
                                            null,
                                          ],
                                        },
                                      ],
                                      "nop",
                                      "\n",
                                      "ev",
                                      { "VAR?": "damage3" },
                                      { "VAR?": "p_multiplier2" },
                                      "*",
                                      "INT",
                                      "/ev",
                                      { "temp=": "final_dmg2" },
                                      "\n",
                                      "ev",
                                      { "VAR?": "jed_hp" },
                                      { "VAR?": "final_dmg2" },
                                      "-",
                                      { "VAR=": "jed_hp", re: true },
                                      "/ev",
                                      "^It hits Jed for ",
                                      "ev",
                                      { "VAR?": "final_dmg2" },
                                      "out",
                                      "/ev",
                                      "^ damage!",
                                      "\n",
                                      { "->": ".^.^.^.10" },
                                      null,
                                    ],
                                  },
                                ],
                                "nop",
                                "\n",
                                { "->": ".^.^.^.14" },
                                null,
                              ],
                            },
                          ],
                          [
                            { "->": ".^.b" },
                            {
                              b: [
                                "\n",
                                "ev",
                                8,
                                12,
                                "rnd",
                                10.0,
                                "/",
                                "/ev",
                                { "temp=": "p_multiplier3" },
                                "\n",
                                "ev",
                                { "VAR?": "current_enemy_atk" },
                                { "VAR?": "current_player_def" },
                                "-",
                                "/ev",
                                { "temp=": "damage4" },
                                "ev",
                                { "VAR?": "damage4" },
                                1,
                                "<",
                                "/ev",
                                [
                                  { "->": ".^.b", c: true },
                                  {
                                    b: [
                                      "\n",
                                      "ev",
                                      1,
                                      "/ev",
                                      { "temp=": "damage4", re: true },
                                      { "->": ".^.^.^.22" },
                                      null,
                                    ],
                                  },
                                ],
                                "nop",
                                "\n",
                                "ev",
                                { "VAR?": "damage4" },
                                { "VAR?": "p_multiplier3" },
                                "*",
                                "INT",
                                "/ev",
                                { "temp=": "final_dmg3" },
                                "\n",
                                "ev",
                                { "VAR?": "hp" },
                                { "VAR?": "final_dmg3" },
                                "-",
                                { "VAR=": "hp", re: true },
                                "/ev",
                                "^It hits you for ",
                                "ev",
                                { "VAR?": "final_dmg3" },
                                "out",
                                "/ev",
                                "^ damage!",
                                "\n",
                                { "->": ".^.^.^.14" },
                                null,
                              ],
                            },
                          ],
                          "nop",
                          "\n",
                          { "->": ".^.^.^.22" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    { "->": ".^.^.^.17" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              { "VAR?": "enemy2_hp" },
              0,
              ">",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^The second ",
                    "ev",
                    { "VAR?": "enemy2_name" },
                    "out",
                    "/ev",
                    "^ attacks!",
                    "\n",
                    "ev",
                    1,
                    100,
                    "rnd",
                    "/ev",
                    { "temp=": "player_dodge_roll2" },
                    "\n",
                    "ev",
                    { "VAR?": "player_dodge_roll2" },
                    { "VAR?": "dodge_chance" },
                    "<=",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "^You anticipate the attack and deftly move aside. It misses!",
                          "\n",
                          { "->": ".^.^.^.22" },
                          null,
                        ],
                      },
                    ],
                    [
                      { "->": ".^.b" },
                      {
                        b: [
                          "\n",
                          "ev",
                          { "VAR?": "jed_status" },
                          "str",
                          "^HELPED",
                          "/str",
                          "==",
                          { "VAR?": "jed_hp" },
                          0,
                          ">",
                          "&&",
                          "/ev",
                          [
                            { "->": ".^.b", c: true },
                            {
                              b: [
                                "\n",
                                "ev",
                                1,
                                2,
                                "rnd",
                                "/ev",
                                { "temp=": "target_roll2" },
                                "\n",
                                [
                                  "ev",
                                  { "VAR?": "target_roll2" },
                                  1,
                                  "==",
                                  "/ev",
                                  { "->": ".^.b", c: true },
                                  {
                                    b: [
                                      "\n",
                                      "ev",
                                      8,
                                      12,
                                      "rnd",
                                      10.0,
                                      "/",
                                      "/ev",
                                      { "temp=": "p_multiplier4" },
                                      "\n",
                                      "ev",
                                      { "VAR?": "enemy2_atk" },
                                      { "VAR?": "current_player_def" },
                                      "-",
                                      "/ev",
                                      { "temp=": "damage5" },
                                      "ev",
                                      { "VAR?": "damage5" },
                                      1,
                                      "<",
                                      "/ev",
                                      [
                                        { "->": ".^.b", c: true },
                                        {
                                          b: [
                                            "\n",
                                            "ev",
                                            1,
                                            "/ev",
                                            { "temp=": "damage5", re: true },
                                            { "->": ".^.^.^.22" },
                                            null,
                                          ],
                                        },
                                      ],
                                      "nop",
                                      "\n",
                                      "ev",
                                      { "VAR?": "hp" },
                                      { "VAR?": "damage5" },
                                      "-",
                                      { "VAR=": "hp", re: true },
                                      "/ev",
                                      "ev",
                                      { "VAR?": "damage5" },
                                      { "VAR?": "p_multiplier4" },
                                      "*",
                                      "INT",
                                      "/ev",
                                      { "temp=": "final_dmg4" },
                                      "\n",
                                      "^It hits you for ",
                                      "ev",
                                      { "VAR?": "final_dmg4" },
                                      "out",
                                      "/ev",
                                      "^ damage!",
                                      "\n",
                                      { "->": ".^.^.^.10" },
                                      null,
                                    ],
                                  },
                                ],
                                [
                                  { "->": ".^.b" },
                                  {
                                    b: [
                                      "\n",
                                      "ev",
                                      8,
                                      12,
                                      "rnd",
                                      10.0,
                                      "/",
                                      "/ev",
                                      { "temp=": "p_multiplier5" },
                                      "\n",
                                      "ev",
                                      { "VAR?": "enemy2_atk" },
                                      { "VAR?": "jed_def" },
                                      "-",
                                      "/ev",
                                      { "temp=": "damage2" },
                                      "ev",
                                      { "VAR?": "damage2" },
                                      1,
                                      "<",
                                      "/ev",
                                      [
                                        { "->": ".^.b", c: true },
                                        {
                                          b: [
                                            "\n",
                                            "ev",
                                            1,
                                            "/ev",
                                            { "temp=": "damage2", re: true },
                                            { "->": ".^.^.^.22" },
                                            null,
                                          ],
                                        },
                                      ],
                                      "nop",
                                      "\n",
                                      "ev",
                                      { "VAR?": "jed_hp" },
                                      { "VAR?": "damage2" },
                                      "-",
                                      { "VAR=": "jed_hp", re: true },
                                      "/ev",
                                      "ev",
                                      { "VAR?": "damage2" },
                                      { "VAR?": "p_multiplier5" },
                                      "*",
                                      "INT",
                                      "/ev",
                                      { "temp=": "final_dmg5" },
                                      "\n",
                                      "^It hits Jed for ",
                                      "ev",
                                      { "VAR?": "final_dmg5" },
                                      "out",
                                      "/ev",
                                      "^ damage!",
                                      "\n",
                                      { "->": ".^.^.^.10" },
                                      null,
                                    ],
                                  },
                                ],
                                "nop",
                                "\n",
                                { "->": ".^.^.^.14" },
                                null,
                              ],
                            },
                          ],
                          [
                            { "->": ".^.b" },
                            {
                              b: [
                                "\n",
                                "ev",
                                8,
                                12,
                                "rnd",
                                10.0,
                                "/",
                                "/ev",
                                { "temp=": "p_multiplier6" },
                                "\n",
                                "ev",
                                { "VAR?": "enemy2_atk" },
                                { "VAR?": "current_player_def" },
                                "-",
                                "/ev",
                                { "temp=": "damage6" },
                                "ev",
                                { "VAR?": "damage6" },
                                1,
                                "<",
                                "/ev",
                                [
                                  { "->": ".^.b", c: true },
                                  {
                                    b: [
                                      "\n",
                                      "ev",
                                      1,
                                      "/ev",
                                      { "temp=": "damage6", re: true },
                                      { "->": ".^.^.^.22" },
                                      null,
                                    ],
                                  },
                                ],
                                "nop",
                                "\n",
                                "ev",
                                { "VAR?": "damage6" },
                                { "VAR?": "p_multiplier6" },
                                "*",
                                "INT",
                                "/ev",
                                { "temp=": "final_dmg6" },
                                "\n",
                                "ev",
                                { "VAR?": "hp" },
                                { "VAR?": "final_dmg6" },
                                "-",
                                { "VAR=": "hp", re: true },
                                "/ev",
                                "^It hits you for ",
                                "ev",
                                { "VAR?": "final_dmg6" },
                                "out",
                                "/ev",
                                "^ damage!",
                                "\n",
                                { "->": ".^.^.^.14" },
                                null,
                              ],
                            },
                          ],
                          "nop",
                          "\n",
                          { "->": ".^.^.^.22" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    { "->": ".^.^.^.25" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              { "VAR?": "is_countering" },
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    false,
                    "/ev",
                    { "VAR=": "is_countering", re: true },
                    "^As the enemy strikes, you pivot and deliver a vicious counter blow!",
                    "\n",
                    "ev",
                    { "VAR?": "atk" },
                    0.75,
                    "*",
                    "INT",
                    "/ev",
                    { "temp=": "counter_damage" },
                    "\n",
                    "^You counter for ",
                    "ev",
                    { "VAR?": "counter_damage" },
                    "out",
                    "/ev",
                    "^ damage!",
                    "\n",
                    "ev",
                    { "VAR?": "current_enemy_hp" },
                    { "VAR?": "enemy2_hp" },
                    ">",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "ev",
                          { "VAR?": "current_enemy_hp" },
                          { "VAR?": "counter_damage" },
                          "-",
                          { "VAR=": "current_enemy_hp", re: true },
                          "/ev",
                          { "->": ".^.^.^.29" },
                          null,
                        ],
                      },
                    ],
                    [
                      { "->": ".^.b" },
                      {
                        b: [
                          "\n",
                          "ev",
                          { "VAR?": "enemy2_hp" },
                          { "VAR?": "counter_damage" },
                          "-",
                          { "VAR=": "enemy2_hp", re: true },
                          "/ev",
                          { "->": ".^.^.^.29" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    "ev",
                    { "VAR?": "current_enemy_hp" },
                    0,
                    "<=",
                    { "VAR?": "enemy2_hp" },
                    0,
                    "<=",
                    { "VAR?": "enemy2_name" },
                    "str",
                    "^",
                    "/str",
                    "==",
                    "||",
                    "&&",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          { "->": "battle_won" },
                          { "->": ".^.^.^.47" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    { "->": ".^.^.^.31" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              false,
              "/ev",
              { "VAR=": "is_defending", re: true },
              "ev",
              { "VAR?": "hp" },
              0,
              "<=",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    { "->": "game_over_death" },
                    { "->": ".^.^.^.43" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              { "VAR?": "jed_hp" },
              0,
              "<=",
              { "VAR?": "jed_status" },
              "str",
              "^DEAD",
              "/str",
              "!=",
              "&&",
              { "VAR?": "current_enemy_name" },
              "str",
              "^Skulker Guard",
              "/str",
              "==",
              "&&",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^The creature lands a fatal blow on Jed. He collapses to the ground, his weapon clattering on the metal floor. He's gone.",
                    "\n",
                    "ev",
                    "str",
                    "^DEAD",
                    "/str",
                    "/ev",
                    { "VAR=": "jed_status", re: true },
                    { "->": ".^.^.^.63" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              { "VAR?": "current_enemy_hp" },
              0,
              "<=",
              { "VAR?": "enemy2_hp" },
              0,
              "<=",
              { "VAR?": "enemy2_name" },
              "str",
              "^",
              "/str",
              "==",
              "||",
              "&&",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    { "->": "battle_won" },
                    { "->": ".^.^.^.82" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    { "->": "battle_loop" },
                    { "->": ".^.^.^.82" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.17" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      battle_won: [
        "ev",
        false,
        "/ev",
        { "VAR=": "used_skill_in_battle", re: true },
        [
          "ev",
          { "VAR?": "hp" },
          { "VAR?": "max_hp" },
          4,
          "/",
          "<=",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "is_injured", re: true },
              "ev",
              false,
              "/ev",
              { "VAR=": "is_fatigued", re: true },
              "^You are badly wounded from the fight.",
              "\n",
              { "->": ".^.^.^.7" },
              null,
            ],
          },
        ],
        [
          "ev",
          { "VAR?": "hp" },
          { "VAR?": "max_hp" },
          2,
          "/",
          "<=",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              false,
              "/ev",
              { "VAR=": "is_injured", re: true },
              "ev",
              true,
              "/ev",
              { "VAR=": "is_fatigued", re: true },
              "^The fight has left you bruised and exhausted.",
              "\n",
              { "->": ".^.^.^.7" },
              null,
            ],
          },
        ],
        [
          { "->": ".^.b" },
          {
            b: [
              "\n",
              "ev",
              false,
              "/ev",
              { "VAR=": "is_injured", re: true },
              "ev",
              false,
              "/ev",
              { "VAR=": "is_fatigued", re: true },
              { "->": ".^.^.^.7" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        "^The ",
        "ev",
        { "VAR?": "current_enemy_name" },
        "out",
        "/ev",
        "^ collapses. You are victorious.",
        "\n",
        [
          "ev",
          { "VAR?": "character_name" },
          "str",
          "^Kaelen",
          "/str",
          "==",
          { "VAR?": "current_enemy_name" },
          "str",
          "^The Brute",
          "/str",
          "==",
          "&&",
          { "VAR?": "player_skills" },
          { "VAR?": "HeavyHitter" },
          "?",
          "!",
          "&&",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "player_skills" },
              { "VAR?": "HeavyHitter" },
              "+",
              { "VAR=": "player_skills", re: true },
              "/ev",
              "^Watching the Brute fight, you learned something about raw, unrestrained power. You've learned the **Heavy Hitter** skill!",
              "\n",
              '^<i>AI: "Subject Kaelen has adapted to superior physical force by mimicking it. A predictable, yet effective, development."</i>',
              "\n",
              { "->": ".^.^.^.21" },
              null,
            ],
          },
        ],
        [
          "ev",
          { "VAR?": "character_name" },
          "str",
          "^Aris",
          "/str",
          "==",
          { "VAR?": "current_enemy_name" },
          "str",
          "^The Tinkerer",
          "/str",
          "==",
          "&&",
          { "VAR?": "player_skills" },
          { "VAR?": "Overcharge" },
          "?",
          "!",
          "&&",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "player_skills" },
              { "VAR?": "Overcharge" },
              "+",
              { "VAR=": "player_skills", re: true },
              "/ev",
              "^By observing the Tinkerer's unstable technology, you've figured out how to apply their reckless principles to your own gear. You've learned the **Overcharge** skill!",
              "\n",
              '^<i>AI: "Subject Aris has reverse-engineered a crude but effective energy modulation technique. Logical progression."</i>',
              "\n",
              { "->": ".^.^.^.21" },
              null,
            ],
          },
        ],
        [
          "ev",
          { "VAR?": "character_name" },
          "str",
          "^Lena",
          "/str",
          "==",
          { "VAR?": "current_enemy_name" },
          "str",
          "^The Veteran",
          "/str",
          "==",
          "&&",
          { "VAR?": "player_skills" },
          { "VAR?": "CounterAttack" },
          "?",
          "!",
          "&&",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "player_skills" },
              { "VAR?": "CounterAttack" },
              "+",
              { "VAR=": "player_skills", re: true },
              "/ev",
              "^The Veteran's calm, precise fighting style was a lesson in efficiency. You've adapted their technique into a new skill: **Counter Attack**.",
              "\n",
              '^<i>AI: "Subject Lena has assimilated a more efficient combat doctrine based on observation. Optimal."</i>',
              "\n",
              { "->": ".^.^.^.21" },
              null,
            ],
          },
        ],
        [
          "ev",
          { "VAR?": "current_enemy_name" },
          "str",
          "^Skulker Packmate Beta",
          "/str",
          "==",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^You've cleared the first wave of Skulkers. The path to the main platform is closer, but a larger guard now blocks your way.",
              "\n",
              { "->": "setup_skulker_guard_battle" },
              { "->": ".^.^.^.21" },
              null,
            ],
          },
        ],
        [
          "ev",
          { "VAR?": "current_enemy_name" },
          "str",
          "^Skulker Guard",
          "/str",
          "==",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^The guard collapses, and you spot a small, sealed vial attached to its makeshift belt. It's a Regen Serum.",
              "\n",
              "ev",
              true,
              "/ev",
              { "VAR=": "has_regen_serum", re: true },
              "ev",
              "str",
              "^Use the Regen-Serum",
              "/str",
              "/ev",
              { "*": ".^.c-0", flg: 20 },
              "ev",
              "str",
              "^Do not use the Regen-Serum",
              "/str",
              "/ev",
              { "*": ".^.c-1", flg: 20 },
              { "->": ".^.^.^.21" },
              {
                "c-0": [
                  "\n",
                  { "->": ".^.^.^.^.use_regen_serum" },
                  { "#f": 5 },
                ],
                "c-1": [
                  "\n",
                  "ev",
                  { "VAR?": "character_name" },
                  "str",
                  "^Aris",
                  "/str",
                  "==",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        { "->": "battle_won.loot_skulker_guard" },
                        { "->": ".^.^.^.10" },
                        null,
                      ],
                    },
                  ],
                  [
                    { "->": ".^.b" },
                    {
                      b: [
                        "\n",
                        "^You defeat the guard, and the rest of the horde scatters, clearing the path to the main platform.",
                        "\n",
                        { "->": "setup_alpha_skulker_battle" },
                        { "->": ".^.^.^.10" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  "ev",
                  { "VAR?": "jed_status" },
                  "str",
                  "^HELPED",
                  "/str",
                  "==",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        '^Jed stands back-to-back with you. "You take the one on the left?" he asks, readying his own weapon.',
                        "\n",
                        { "->": ".^.^.^.20" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  "^This is your first major obstacle.",
                  "\n",
                  "ev",
                  true,
                  "/ev",
                  { "->": "setup_two_skulker_battle" },
                  { "#f": 5 },
                ],
              },
            ],
          },
        ],
        "nop",
        "\n",
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Aris",
        "/str",
        "==",
        { "VAR?": "current_enemy_name" },
        "str",
        "^Slick-Skinned Skulker",
        "/str",
        "==",
        "&&",
        "/ev",
        [
          { "->": ".^.b", c: true },
          { b: ["\n", { "->": "loot_skulker" }, { "->": ".^.^.^.37" }, null] },
        ],
        "nop",
        "\n",
        [
          "ev",
          { "VAR?": "current_enemy_name" },
          "str",
          "^Slick-Skinned Skulker",
          "/str",
          "==",
          "/ev",
          { "->": ".^.b", c: true },
          { b: ["\n", { "->": "skulker_win" }, { "->": ".^.^.^.46" }, null] },
        ],
        [
          "ev",
          { "VAR?": "current_enemy_name" },
          "str",
          "^Ambush Skulker",
          "/str",
          "==",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "character_name" },
              "str",
              "^Aris",
              "/str",
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    { "->": "battle_won.loot_ambush_skulker" },
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "^You dispatch the creature and catch your breath. The hab-unit is now clear.",
                    "\n",
                    { "->": "final_scavenge" },
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.46" },
              null,
            ],
          },
        ],
        [
          "ev",
          { "VAR?": "current_enemy_name" },
          "str",
          "^Skulker Guard",
          "/str",
          "==",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "character_name" },
              "str",
              "^Aris",
              "/str",
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    { "->": "battle_won.loot_skulker_guard" },
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "^You defeat the guard, and the rest of the horde scatters, clearing the path to the main platform.",
                    "\n",
                    { "->": "setup_alpha_skulker_battle" },
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.46" },
              null,
            ],
          },
        ],
        [
          "ev",
          { "VAR?": "current_enemy_name" },
          "str",
          "^The Brute",
          "/str",
          "==",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^You step over the unconscious form of the Brute and head for the stairs.",
              "\n",
              "ev",
              { "^->": "tower_floor_3" },
              "/ev",
              { "->": "tower_buffer_room" },
              { "->": ".^.^.^.46" },
              null,
            ],
          },
        ],
        [
          "ev",
          { "VAR?": "current_enemy_name" },
          "str",
          "^The Tinkerer",
          "/str",
          "==",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^The Tinkerer's device sputters and dies, and they surrender immediately. You move to the next floor.",
              "\n",
              "ev",
              { "^->": "tower_floor_5" },
              "/ev",
              { "->": "tower_buffer_room" },
              { "->": ".^.^.^.46" },
              null,
            ],
          },
        ],
        [
          "ev",
          { "VAR?": "current_enemy_name" },
          "str",
          "^The Veteran",
          "/str",
          "==",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^The Veteran gives a single, respectful nod as they fall back, defeated. The path to the top is clear.",
              "\n",
              { "->": "tower_top" },
              { "->": ".^.^.^.46" },
              null,
            ],
          },
        ],
        [{ "->": ".^.b" }, { b: ["\n", "end", { "->": ".^.^.^.46" }, null] }],
        "nop",
        "\n",
        {
          use_regen_serum: [
            "^You inject the serum. A wave of warmth spreads through your body, knitting tissues back together and washing away all fatigue and pain. You feel completely restored.",
            "\n",
            "ev",
            { "VAR?": "max_hp" },
            "/ev",
            { "VAR=": "hp", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "is_injured", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "is_fatigued", re: true },
            "ev",
            false,
            "/ev",
            { "VAR=": "has_regen_serum", re: true },
            "ev",
            { "VAR?": "character_name" },
            "str",
            "^Aris",
            "/str",
            "==",
            "/ev",
            [
              { "->": ".^.b", c: true },
              {
                b: [
                  "\n",
                  { "->": ".^.^.^.^.loot_skulker_guard" },
                  { "->": ".^.^.^.27" },
                  null,
                ],
              },
            ],
            [
              { "->": ".^.b" },
              {
                b: [
                  "\n",
                  "^You defeat the guard, and the rest of the horde scatters, clearing the path to the main platform.",
                  "\n",
                  { "->": "setup_alpha_skulker_battle" },
                  { "->": ".^.^.^.27" },
                  null,
                ],
              },
            ],
            "nop",
            "\n",
            { "#f": 1 },
          ],
          loot_ambush_skulker: [
            [
              "^As the Skulker lies defeated, your bio-scanner detects a potent neurotoxin still active in its venom glands.",
              "\n",
              "ev",
              "str",
              "^Harvest the Venom Gland",
              "/str",
              "/ev",
              { "*": ".^.c-0", flg: 4 },
              "ev",
              "str",
              "^Leave it.",
              "/str",
              "/ev",
              { "*": ".^.c-1", flg: 4 },
              {
                "c-0": [
                  "\n",
                  "ev",
                  1,
                  3,
                  "rnd",
                  "/ev",
                  { "temp=": "roll" },
                  "\n",
                  "ev",
                  { "VAR?": "roll" },
                  1,
                  "==",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        "^You carefully extract the gland, a pulsating sac of green fluid. This could be a powerful ingredient.",
                        "\n",
                        "ev",
                        { "VAR?": "skulker_venom_gland_stack" },
                        1,
                        "+",
                        { "VAR=": "skulker_venom_gland_stack", re: true },
                        "/ev",
                        { "->": ".^.^.^.15" },
                        null,
                      ],
                    },
                  ],
                  [
                    { "->": ".^.b" },
                    {
                      b: [
                        "\n",
                        "^You try to extract the gland, but your hands are unsteady from the fight. You accidentally puncture the sac, and the venom spills uselessly onto the floor, dissolving into a foul-smelling vapor. The opportunity is lost.",
                        "\n",
                        { "->": ".^.^.^.15" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  { "->": ".^.^.g-0" },
                  { "#f": 5 },
                ],
                "c-1": ["\n", { "->": ".^.^.g-0" }, { "#f": 5 }],
                "g-0": [
                  "^The hab-unit is now clear.",
                  "\n",
                  { "->": "final_scavenge" },
                  { "#f": 5 },
                ],
              },
            ],
            { "#f": 1 },
          ],
          loot_skulker_guard: [
            [
              "^As the Skulker Guard lies defeated, your bio-scanner detects a potent neurotoxin still active in its venom glands.",
              "\n",
              "ev",
              "str",
              "^Harvest the Venom Gland",
              "/str",
              "/ev",
              { "*": ".^.c-0", flg: 20 },
              "ev",
              "str",
              "^Leave it.",
              "/str",
              "/ev",
              { "*": ".^.c-1", flg: 20 },
              {
                "c-0": [
                  "\n",
                  "ev",
                  1,
                  3,
                  "rnd",
                  "/ev",
                  { "temp=": "roll" },
                  "\n",
                  "ev",
                  { "VAR?": "roll" },
                  1,
                  "==",
                  { "VAR?": "roll" },
                  2,
                  "==",
                  "||",
                  "/ev",
                  [
                    { "->": ".^.b", c: true },
                    {
                      b: [
                        "\n",
                        "^You carefully extract the gland from the larger creature. This is a prime sample.",
                        "\n",
                        "ev",
                        { "VAR?": "skulker_venom_gland_stack" },
                        2,
                        "+",
                        { "VAR=": "skulker_venom_gland_stack", re: true },
                        "/ev",
                        { "->": ".^.^.^.19" },
                        null,
                      ],
                    },
                  ],
                  [
                    { "->": ".^.b" },
                    {
                      b: [
                        "\n",
                        "^You try to extract the gland, but the creature's thick hide makes it difficult. You accidentally puncture the sac, and the venom spills uselessly onto the floor.",
                        "\n",
                        { "->": ".^.^.^.19" },
                        null,
                      ],
                    },
                  ],
                  "nop",
                  "\n",
                  { "->": ".^.^.g-0" },
                  { "#f": 5 },
                ],
                "c-1": ["\n", { "->": ".^.^.g-0" }, { "#f": 5 }],
                "g-0": [
                  "^You defeat the guard, and the rest of the horde scatters, clearing the path to the main platform.",
                  "\n",
                  { "->": "setup_alpha_skulker_battle" },
                  { "#f": 5 },
                ],
              },
            ],
            { "#f": 1 },
          ],
          "#f": 1,
        },
      ],
      skulker_defeated_hub: [
        "ev",
        { "VAR?": "character_name" },
        "str",
        "^Aris",
        "/str",
        "==",
        "/ev",
        [
          { "->": ".^.b", c: true },
          { b: ["\n", { "->": "loot_skulker" }, { "->": ".^.^.^.8" }, null] },
        ],
        "nop",
        "\n",
        { "->": "skulker_win" },
        { "#f": 1 },
      ],
      loot_skulker: [
        [
          "^As the Skulker lies defeated, your bio-scanner detects a potent neurotoxin still active in its venom glands.",
          "\n",
          "ev",
          "str",
          "^Harvest the Venom Gland",
          "/str",
          "/ev",
          { "*": ".^.c-0", flg: 20 },
          "ev",
          "str",
          "^Leave it.",
          "/str",
          "/ev",
          { "*": ".^.c-1", flg: 20 },
          {
            "c-0": [
              "\n",
              "ev",
              1,
              3,
              "rnd",
              "/ev",
              { "temp=": "roll" },
              "\n",
              "ev",
              { "VAR?": "roll" },
              1,
              "==",
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "^You carefully extract the gland, a pulsating sac of green fluid. This could be a powerful ingredient.",
                    "\n",
                    "ev",
                    { "VAR?": "skulker_venom_gland_stack" },
                    1,
                    "+",
                    { "VAR=": "skulker_venom_gland_stack", re: true },
                    "/ev",
                    { "->": ".^.^.^.15" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "^You try to extract the gland, but your hands are unsteady from the fight. You accidentally puncture the sac, and the venom spills uselessly onto the floor, dissolving into a foul-smelling vapor. The opportunity is lost.",
                    "\n",
                    { "->": ".^.^.^.15" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.g-0" },
              { "#f": 5 },
            ],
            "c-1": ["\n", { "->": ".^.^.g-0" }, { "#f": 5 }],
            "g-0": [
              "^The path to the terminal is clear..",
              "\n",
              { "->": "skulker_win" },
              { "#f": 5 },
            ],
          },
        ],
        { "#f": 1 },
      ],
      battle_lost: [
        "ev",
        false,
        "/ev",
        { "VAR=": "used_skill_in_battle", re: true },
        [
          "ev",
          { "VAR?": "current_enemy_name" },
          "str",
          "^Slick-Skinned Skulker",
          "/str",
          "==",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^Your vision fades to black as the creature's final blow lands.",
              "\n",
              { "->": "scene_6c_skulker_lair" },
              { "->": ".^.^.^.7" },
              null,
            ],
          },
        ],
        [
          "ev",
          { "VAR?": "current_enemy_name" },
          "str",
          "^The Brute",
          "/str",
          "==",
          { "VAR?": "current_enemy_name" },
          "str",
          "^The Tinkerer",
          "/str",
          "==",
          "||",
          { "VAR?": "current_enemy_name" },
          "str",
          "^The Veteran",
          "/str",
          "==",
          "||",
          "/ev",
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "^Your vision fades to black as the opponent's final blow lands.",
              "\n",
              { "->": "game_over_death" },
              { "->": ".^.^.^.7" },
              null,
            ],
          },
        ],
        [{ "->": ".^.b" }, { b: ["\n", "end", { "->": ".^.^.^.7" }, null] }],
        "nop",
        "\n",
        { "#f": 1 },
      ],
      skulker_win: [
        "^You download the **first Data Fragment**.",
        "\n",
        "ev",
        { "VAR?": "data_fragments" },
        1,
        "+",
        { "VAR=": "data_fragments", re: true },
        "/ev",
        { "->": "scene_7_the_fragment" },
        { "#f": 1 },
      ],
      skulker_lose: [
        "^You have been defeated by the Skulker.",
        "\n",
        "end",
        { "#f": 1 },
      ],
      jed_turn: [
        "ev",
        { "VAR?": "jed_status" },
        "str",
        "^HELPED",
        "/str",
        "==",
        { "VAR?": "jed_hp" },
        0,
        ">",
        "&&",
        "/ev",
        [
          { "->": ".^.b", c: true },
          {
            b: [
              "\n",
              "ev",
              { "VAR?": "jed_atk" },
              "/ev",
              { "temp=": "current_jed_atk" },
              "ev",
              { "VAR?": "jed_is_buffed" },
              "/ev",
              [
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    { "VAR?": "jed_atk" },
                    4,
                    "+",
                    "/ev",
                    { "temp=": "current_jed_atk", re: true },
                    "ev",
                    false,
                    "/ev",
                    { "VAR=": "jed_is_buffed", re: true },
                    "^Jed follows your instructions perfectly, striking the weak point you identified!",
                    "\n",
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "^Jed fights alongside you. He attacks an enemy!",
                    "\n",
                    { "->": ".^.^.^.10" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              "ev",
              1,
              2,
              "rnd",
              "/ev",
              { "temp=": "target_roll" },
              "\n",
              [
                "ev",
                { "VAR?": "target_roll" },
                1,
                "==",
                { "VAR?": "current_enemy_hp" },
                0,
                ">",
                "&&",
                "/ev",
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    { "VAR?": "current_enemy_hp" },
                    { "VAR?": "current_jed_atk" },
                    "-",
                    { "VAR=": "current_enemy_hp", re: true },
                    "/ev",
                    "^Jed hits the first ",
                    "ev",
                    { "VAR?": "current_enemy_name" },
                    "out",
                    "/ev",
                    "^ for ",
                    "ev",
                    { "VAR?": "current_jed_atk" },
                    "out",
                    "/ev",
                    "^ damage!",
                    "\n",
                    "ev",
                    { "VAR?": "current_enemy_hp" },
                    0,
                    "<=",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "^The first ",
                          "ev",
                          { "VAR?": "current_enemy_name" },
                          "out",
                          "/ev",
                          "^ is defeated!",
                          "\n",
                          "ev",
                          0,
                          "/ev",
                          { "VAR=": "current_enemy_hp", re: true },
                          { "->": ".^.^.^.25" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    { "->": ".^.^.^.22" },
                    null,
                  ],
                },
              ],
              [
                "ev",
                { "VAR?": "enemy2_hp" },
                0,
                ">",
                "/ev",
                { "->": ".^.b", c: true },
                {
                  b: [
                    "\n",
                    "ev",
                    { "VAR?": "enemy2_hp" },
                    { "VAR?": "current_jed_atk" },
                    "-",
                    { "VAR=": "enemy2_hp", re: true },
                    "/ev",
                    "^Jed hits the second ",
                    "ev",
                    { "VAR?": "enemy2_name" },
                    "out",
                    "/ev",
                    "^ for ",
                    "ev",
                    { "VAR?": "current_jed_atk" },
                    "out",
                    "/ev",
                    "^ damage!",
                    "\n",
                    "ev",
                    { "VAR?": "enemy2_hp" },
                    0,
                    "<=",
                    "/ev",
                    [
                      { "->": ".^.b", c: true },
                      {
                        b: [
                          "\n",
                          "^The second ",
                          "ev",
                          { "VAR?": "enemy2_name" },
                          "out",
                          "/ev",
                          "^ is defeated!",
                          "\n",
                          "ev",
                          0,
                          "/ev",
                          { "VAR=": "enemy2_hp", re: true },
                          { "->": ".^.^.^.25" },
                          null,
                        ],
                      },
                    ],
                    "nop",
                    "\n",
                    { "->": ".^.^.^.22" },
                    null,
                  ],
                },
              ],
              [
                { "->": ".^.b" },
                {
                  b: [
                    "\n",
                    "ev",
                    { "VAR?": "current_enemy_hp" },
                    { "VAR?": "current_jed_atk" },
                    "-",
                    { "VAR=": "current_enemy_hp", re: true },
                    "/ev",
                    "^Jed hits the first ",
                    "ev",
                    { "VAR?": "current_enemy_name" },
                    "out",
                    "/ev",
                    "^ for ",
                    "ev",
                    { "VAR?": "current_jed_atk" },
                    "out",
                    "/ev",
                    "^ damage!",
                    "\n",
                    { "->": ".^.^.^.22" },
                    null,
                  ],
                },
              ],
              "nop",
              "\n",
              { "->": ".^.^.^.12" },
              null,
            ],
          },
        ],
        "nop",
        "\n",
        { "->": "enemy_turn" },
        { "#f": 3 },
      ],
      game_over_death: [
        "^Your vision fades to black as the final blow lands. The last thing you hear is the AI's detached voice in your mind.",
        "\n",
        '^<i>AI: "Subject\'s vital signs have ceased. Test failed."</i>',
        "\n",
        "end",
        { "#f": 1 },
      ],
      "global decl": [
        "ev",
        0,
        { "VAR=": "strength" },
        0,
        { "VAR=": "intelligence" },
        0,
        { "VAR=": "agility" },
        0,
        { "VAR=": "perception" },
        100,
        { "VAR=": "resolve" },
        false,
        { "VAR=": "is_fatigued" },
        false,
        { "VAR=": "is_injured" },
        "str",
        "^",
        "/str",
        { "VAR=": "character_name" },
        "str",
        "^",
        "/str",
        { "VAR=": "rival_name" },
        50,
        { "VAR=": "rival_relationship" },
        false,
        { "VAR=": "rival_is_waiting_for_opening" },
        0,
        { "VAR=": "rival_emitter_charges" },
        false,
        { "VAR=": "rival_used_non_combat_skill" },
        false,
        { "VAR=": "rival_used_combat_skill" },
        0,
        { "VAR=": "power_cell_stack" },
        0,
        { "VAR=": "glimmer_moss_stack" },
        false,
        { "VAR=": "has_kinetic_emitter" },
        0,
        { "VAR=": "emitter_charges" },
        "str",
        "^NONE",
        "/str",
        { "VAR=": "neuro_stim_state" },
        false,
        { "VAR=": "analyzed_power_cell" },
        false,
        { "VAR=": "analyzed_glimmer_moss" },
        false,
        { "VAR=": "studied_emitter" },
        { list: {}, origins: ["AllSkills"] },
        { "VAR=": "AllSkills" },
        { list: {} },
        { "VAR=": "player_skills" },
        "str",
        "^UNKNOWN",
        "/str",
        { "VAR=": "jed_status" },
        "str",
        "^",
        "/str",
        { "VAR=": "scene_4_debuff_stat" },
        false,
        { "VAR=": "rival_has_emitter" },
        false,
        { "VAR=": "rival_owes_favour" },
        0,
        { "VAR=": "data_fragments" },
        0,
        { "VAR=": "max_hp" },
        0,
        { "VAR=": "hp" },
        0,
        { "VAR=": "atk" },
        0,
        { "VAR=": "def" },
        0,
        { "VAR=": "dodge_chance" },
        "str",
        "^",
        "/str",
        { "VAR=": "current_enemy_name" },
        0,
        { "VAR=": "current_enemy_hp" },
        0,
        { "VAR=": "current_enemy_atk" },
        0,
        { "VAR=": "current_enemy_def" },
        false,
        { "VAR=": "is_defending" },
        0,
        { "VAR=": "rival_hp" },
        0,
        { "VAR=": "rival_max_hp" },
        0,
        { "VAR=": "rival_atk" },
        0,
        { "VAR=": "rival_def" },
        false,
        { "VAR=": "used_skill_in_battle" },
        false,
        { "VAR=": "rival_will_miss_next_turn" },
        false,
        { "VAR=": "rival_is_defending" },
        false,
        { "VAR=": "used_survivalist_in_bay" },
        false,
        { "VAR=": "used_bioscan_in_bay" },
        false,
        { "VAR=": "used_discerningeye_in_bay" },
        false,
        { "VAR=": "found_first_log" },
        false,
        { "VAR=": "know_skulker_weakness" },
        false,
        { "VAR=": "log_knows_site_purpose" },
        false,
        { "VAR=": "log_knows_shattered_world_cause" },
        false,
        { "VAR=": "log_knows_proctor_ethics" },
        false,
        { "VAR=": "has_reinforced_club" },
        false,
        { "VAR=": "has_recurve_bow" },
        false,
        { "VAR=": "has_emp_grenade" },
        false,
        { "VAR=": "has_metal_pipe" },
        false,
        { "VAR=": "has_thick_wiring" },
        false,
        { "VAR=": "has_flexible_polymer" },
        false,
        { "VAR=": "has_tensile_cable" },
        false,
        { "VAR=": "has_copper_wiring" },
        false,
        { "VAR=": "has_magnetic_coil" },
        0,
        { "VAR=": "skulker_venom_gland_stack" },
        0,
        { "VAR=": "has_moss_poison_vial" },
        false,
        { "VAR=": "poison_bomb_stack" },
        false,
        { "VAR=": "enemy_is_poisoned" },
        0,
        { "VAR=": "poison_turns_remaining" },
        false,
        { "VAR=": "enemy2_is_poisoned" },
        0,
        { "VAR=": "poison2_turns_remaining" },
        false,
        { "VAR=": "emitter_equipped" },
        false,
        { "VAR=": "club_equipped" },
        false,
        { "VAR=": "bow_equipped" },
        false,
        { "VAR=": "is_overcharging" },
        false,
        { "VAR=": "is_countering" },
        false,
        { "VAR=": "enemy2_will_miss_next_turn" },
        false,
        { "VAR=": "has_club_upgrade_kit" },
        false,
        { "VAR=": "knows_calming_poultice_recipe" },
        0,
        { "VAR=": "silent_arrow_count" },
        0,
        { "VAR=": "scrap_arrow_count" },
        0,
        { "VAR=": "shock_arrow_count" },
        false,
        { "VAR=": "club_is_upgraded" },
        0,
        { "VAR=": "club_power_slots" },
        false,
        { "VAR=": "club_is_charged" },
        false,
        { "VAR=": "has_titan_beetle_carapace" },
        false,
        { "VAR=": "has_adrenaline_shot" },
        false,
        { "VAR=": "has_regen_serum" },
        0,
        { "VAR=": "player_contribution" },
        0,
        { "VAR=": "rival_contribution" },
        0,
        { "VAR=": "jed_contribution" },
        0,
        { "VAR=": "ally_3_contribution" },
        0,
        { "VAR=": "ally_4_contribution" },
        20,
        { "VAR=": "ally_3_hp" },
        20,
        { "VAR=": "ally_4_hp" },
        false,
        { "VAR=": "ally_3_is_down" },
        false,
        { "VAR=": "ally_4_is_down" },
        0,
        { "VAR=": "alpha_skulker_hp" },
        0,
        { "VAR=": "alpha_skulker_max_hp" },
        0,
        { "VAR=": "alpha_skulker_atk" },
        0,
        { "VAR=": "alpha_skulker_def" },
        false,
        { "VAR=": "alpha_is_berserk" },
        false,
        { "VAR=": "rival_is_in_danger" },
        "str",
        "^",
        "/str",
        { "VAR=": "enemy2_name" },
        0,
        { "VAR=": "enemy2_hp" },
        0,
        { "VAR=": "enemy2_atk" },
        0,
        { "VAR=": "enemy2_def" },
        0,
        { "VAR=": "jed_hp" },
        20,
        { "VAR=": "jed_max_hp" },
        6,
        { "VAR=": "jed_atk" },
        4,
        { "VAR=": "jed_def" },
        false,
        { "VAR=": "analyzed_power_cell_type" },
        false,
        { "VAR=": "analyzed_moss_type" },
        false,
        { "VAR=": "analyzed_emitter_type" },
        false,
        { "VAR=": "analyzed_neuro_stim_type" },
        false,
        { "VAR=": "analyzed_club_type" },
        false,
        { "VAR=": "analyzed_bow_type" },
        false,
        { "VAR=": "analyzed_emp_grenade_type" },
        false,
        { "VAR=": "analyzed_metal_pipe_type" },
        false,
        { "VAR=": "analyzed_thick_wiring_type" },
        false,
        { "VAR=": "analyzed_polymer_type" },
        false,
        { "VAR=": "analyzed_cable_type" },
        false,
        { "VAR=": "analyzed_copper_wiring_type" },
        false,
        { "VAR=": "analyzed_magnetic_coil_type" },
        false,
        { "VAR=": "analyzed_venom_gland_type" },
        false,
        { "VAR=": "analyzed_poison_vial_type" },
        false,
        { "VAR=": "analyzed_poison_bomb_type" },
        false,
        { "VAR=": "analyzed_club_upgrade_type" },
        false,
        { "VAR=": "analyzed_poultice_recipe_type" },
        false,
        { "VAR=": "analyzed_silent_arrows_type" },
        false,
        { "VAR=": "analyzed_scrap_arrows_type" },
        false,
        { "VAR=": "analyzed_shock_arrows_type" },
        false,
        { "VAR=": "analyzed_beetle_carapace_type" },
        false,
        { "VAR=": "analyzed_adrenaline_shot_type" },
        false,
        { "VAR=": "analyzed_first_log" },
        false,
        { "VAR=": "log_knows_ally_buff" },
        false,
        { "VAR=": "jed_is_buffed" },
        "/ev",
        "end",
        null,
      ],
      "#f": 1,
    },
  ],
  listDefs: {
    AllSkills: {
      Survivalist: 1,
      BioScan: 2,
      DiscerningEye: 3,
      HeavyHitter: 4,
      Overcharge: 5,
      CounterAttack: 6,
      TargetedAnalysis: 7,
    },
  },
};
